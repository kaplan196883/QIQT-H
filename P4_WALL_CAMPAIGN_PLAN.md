...
### Phase 4 — unbounded self-adjoint operators + Stone's theorem  *(✅ DONE — see the Phase-4-CLOSED entry in the progress log; K, P, X=A_edge are all self-adjoint, axiom-free)*
...
Phase 4 (Stone) is ✅ **DONE** (K, P, X=A_edge all self-adjoint, axiom-free — audit 2026-06-29).  **Phase 5 (the
dual-weight trace / Type II) is now the sole remaining analytic frontier**; Phase 6 is the payoff (and Route 2 /
P4-MICRO already ships to GR).
...
**(Was) the remaining wall of 4.2:** `Dense (stoneGen U).domain` (Gårding/mollified-vector density) +
essential self-adjointness (`Range(A±i)` dense) + the Cayley transform.

### Phase 4 — essential self-adjointness + Stone CLOSED ✅✅ (audit 2026-06-29 — the named "Mathlib frontier" is DONE)
**The wall above is closed (axiom-free, budget 0).** `QIQTH/Spectral/Garding.lean` proves the Gårding-density /
deficiency-index route to **essential self-adjointness** for any C₀ unitary group: **`stoneGen_isSelfAdjoint`**
(`IsSelfAdjoint (stoneGen U)` = `K = K†` in the genuine `LinearPMap` sense), from the hard surjectivity
**`stoneGen_add_I_surjective`** (`Range(K+i)=H`, Gårding density) + `ker_adjoint_add_I_trivial`, plus the Cayley
bijection **`stoneGen_add_I_bijective`** / **`cayleyEquiv`** and the bounded spectral theorem
**`SpectralTheorem.PVM_of_selfAdjoint`** + **`PVM.boundedFC_mul`**. All three campaign C₀ generators are therefore
genuine **self-adjoint** operators (verified `#print axioms` = standard 3): the modular Hamiltonian
**`ModularGenerator.modularGen_isSelfAdjoint`** (K), the momentum **`MomentumGenerator.momentumOp_isSelfAdjoint`**
(P), and the **clock energy `X = A_edge`** **`CrossedProductGenerator.clockEnergy_isSelfAdjoint`**. So Phase 4 is
COMPLETE and `Increment 1c` (the dressed generator `K̃ = K + X`) is **no longer gated on Stone** — QIQT-H has a
self-contained unbounded Stone theorem + spectral theory that Mathlib still lacks. (The per-file "carried frontier
/ NOT claimed" headers were stale; corrected this session.) The genuine remaining frontier is **Phase 5** (the
