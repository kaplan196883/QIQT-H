...
\begin{table}[ht]
\centering
\caption{Instrument ablation: faults detected (of 5 seeded), by loop configuration.}
\begin{tabularx}{\linewidth}{@{}YYYYY@{}}
\toprule
\textbf{Configuration} & \textbf{Faults detected} & \textbf{False axiom} & \textbf{Circular/over-strong} & \textbf{Sound controls misflagged} \\
\midrule
Compiler (green build) & 0/5 (all build; \texttt{sorry} only warns) & no & no & 0/2 \\
+ axiom budget & 1 outright (\texttt{sorry}); 3 more flagged as \emph{added axioms} (disclosure, not falsity); misses the over-strong one & flagged as "an axiom", not as false & \textbf{no} & 0/2 \\
+ vacuity lint & 1/5 (the \texttt{:= True} predicate; the \texttt{True}-\emph{binder} form is not matched) & no & no & 0/2 \\
+ independent reviewer (blind) & \textbf{5/5} & \textbf{yes} & \textbf{yes} & \textbf{0/2} \\
\bottomrule
\end{tabularx}
\end{table}

The point is not the headline 5/5 but the \emph{profile}. The structural instruments catch structural
faults — \texttt{sorry} and added axioms (budget), \texttt{:= True} bodies (lint) — but they cannot judge semantic
soundness: a false-but-well-typed axiom is flagged by the budget only as "an axiom" (the same signal a
\emph{legitimate} labelled assumption emits), and a circular/over-strong hypothesis adds no axiom and no
