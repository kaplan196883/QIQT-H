...
\begin{itemize}
  ...
  \item \textbf{Headline result: the Einstein field equations, machine-checked and axiom-free, from a finite-information bound.} Running the loop on a researcher's own framework (QIQT-H, not a benchmark and not a published lemma), we obtained a single end-to-end Lean theorem deriving the Einstein field equations $a\,T_{\mu\nu}=G_{\mu\nu}+\Lambda g_{\mu\nu}$ from the framework's Bekenstein-type capacity bound: a Jacobson-style equation of state in which the thermodynamic (area-law) input is \emph{derived} from the framework rather than assumed, with only Bisognano-Wichmann wedge-modular flux and Raychaudhuri focusing cited as explicit hypotheses and all the differential geometry machine-checked. The same axiom-free development also carries a no-collapse measurement core (driving the project's axiom count 57 to 0); in total 192 modules, about 2,010 theorems, 0 project axioms, no \texttt{sorry} (§4).
  ...
  \item \textbf{A verified human-readable bridge.} A blueprint whose formalized tags are mechanically linked to kernel-checked declarations, making an AI-formalized result auditable by domain experts who do not read Lean (§3.6).
