...
2,010 theorems, 0 project-specific axioms, no \texttt{sorry}. We are explicit about scope: verification
...
\begin{itemize}
  \item \textbf{An axiom-budget discipline as a soundness instrument.} A continuously maintained, CI-enforced
\end{itemize}
...
\begin{itemize}
  \item **Headline result: the Einstein field equations, machine-checked and axiom-free, from a
\end{itemize}
  ...
  0, with no \texttt{sorry}, over 192 modules and about 2,010 theorems (§4).
\begin{itemize}
  \item \textbf{Two documented soundness saves and a third instrument.} Two concrete, kernel-checkable
\end{itemize}
...
\begin{itemize}
  \item \textbf{A verified human-readable bridge.} A blueprint whose formalized tags are mechanically linked
\end{itemize}
  to kernel-checked declarations, making an AI-formalized result auditable by domain experts who do
  not read Lean (§3.6).
\begin{itemize}
  \item \textbf{A reproducible artifact.} A public repository, the axiom audit, and the blueprint, so that
\end{itemize}
...
The axiom budget governs assumptions declared as \texttt{axiom}, but a result can also rest on assumptions
carried as ordinary \emph{hypotheses} on its statement; since a hypothesis is an axiom local to one
...
the absence of \texttt{sorry} are the continuous-integration result; the vacuity-lint outcome is a script;
...
information, bounded by a holographic capacity \cite{r40}. The same finite-capacity premise reaches in two
...
across-run frequency rather than a per-run probability \cite{r32,r33,r34}. Toward gravity: a saturated
...
finite-information line of approaches to measurement \cite{r41}; it is set out in a companion paper \cite{r42},
and the Lean development we report on is public \cite{r43}. Whether these physical claims are true is not
...
from a finite-information capacity bound, by way of a Jacobson-style equation of state \cite{r46}. The Lean
top theorem (\texttt{qiqt\_bekenstein\_gives\_gr}) takes a smooth metric, a stress tensor, and a local
causal-horizon congruence; from the QIQT-H capacity bound $S \le \eta A$ together with its saturation
and Klein positivity it \emph{derives} the differential first law $\delta\langle K\rangle = \eta\,\delta A$;
it then combines this with two explicitly cited physics inputs — the Bisognano-Wichmann
boost/modular flux \cite{r47,r48} and Raychaudhuri null focusing \cite{r49} — to obtain the Jacobson per-null
...
The point for this paper is not the physics but the \emph{shape of the certificate}. Table 1 records the
...
bound is the framework's own postulate. Verification certifies the conditional implication — \emph{given}
...
\begin{table}[ht]
\centering
\caption{Status of each link in the QIQT-H → Einstein chain.}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.60\linewidth}Y@{}}
\toprule
\textbf{Link in the chain} & \textbf{Status} \\
\midrule
Finite-information capacity bound $S \le \eta A$ (QIQT-H postulate) & framework postulate \\
Saturation + Klein positivity ⟹ differential first law $\delta\langle K\rangle=\eta\,\delta A$ & \textbf{derived (machine-checked)} \\
Boost/modular flux $\delta\langle K\rangle = \tfrac{2\pi}{\hbar}\!\int T_{\mu\nu}\,\xi^\mu d\Sigma^\nu$ (Bisognano-Wichmann) & cited input \cite{r47,r48} \\
Raychaudhuri null focusing $\delta A = -\!\int R_{\mu\nu}k^\mu k^\nu\,\lambda\,d\lambda\,dA$ & cited input \cite{r49} \\
Jacobson per-null premise (Clausius $\delta Q = T\,\delta S$ on local horizons) & \textbf{derived (machine-checked)} \\
Second + contracted Bianchi, $\nabla^\mu G_{\mu\nu}=0$ & \textbf{machine-checked} \\
Null-cone-pointwise ⟹ tensor equation; $\Lambda$ constant & \textbf{machine-checked} \\
Einstein field equations $a\,T_{\mu\nu}=G_{\mu\nu}+\Lambda g_{\mu\nu}$ & \textbf{machine-checked conclusion} \\
\bottomrule
\end{tabularx}
\end{table}

This scope statement is read off the theorem by the hypothesis-audit scripts of §3.5, which anyone
can re-run. The ledger reports that \texttt{qiqt\_bekenstein\_gives\_gr} takes 14 data binders and 23
...
modules and approximately 2,010 theorems and lemmas\textbf{, with }830 \texttt{\#print axioms} directives** in a
dedicated audit module. It builds green, contains \textbf{no \texttt{sorry}}, and depends on **zero
...
adversarial reviewer, a \texttt{\#print axioms}-based proved/conditional/cited audit with a ratcheting
...
directly on the headline result. The machine-checked Einstein-equation derivation is a \emph{conditional}
one: it depends on the QIQT-H capacity bound as a postulate and on two explicitly cited physics
inputs — the Bisognano-Wichmann boost/modular flux \cite{r47,r48} and Raychaudhuri null focusing \cite{r49} —
...
the two checks practitioners habitually run, "it builds" and "it has no \texttt{sorry}," cannot tell the
...
fifty-seven project-specific axioms to zero, with no \texttt{sorry}, across 192 modules and roughly 2,010
