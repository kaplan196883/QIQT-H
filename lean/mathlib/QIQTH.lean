...
import QIQTH.HDConvGateCensusReduce
...
-- J4-1012 (ChartGeneralChangeVarEvalSlot): the GENERIC (non-Gaussian-specialized) weighted CoV,
-- generalizing ChartGaussianChangeVar.chart_gaussian_change_variables from the hard-wired integrand
-- gaussDdim τ to an arbitrary φ, instantiated at the eval-slot chart (general interior q₀, via
-- J4-1011's IFT package) and TARGETED at φ := gaussDdim τ ∘ terminalVelAt … x (evalSlot_terminalVel_
-- weighted_CoV): transports ∫ z in ball x ρ, gaussDdim τ (T_x (W_x z)) · B z onto the chart-image
-- integral, item (a) of nb's STEP-4c residual (r6) Sol flagged as remaining. Domain reconciliation
-- (ball x ρ / W_x''(ball x ρ) vs ball 0 R, item (b)) and composition with the evenness link / literal
...
-- (same-direction i twice) to two directions i,j via the SAME mechanical ContDiffAt.fderiv_right bridge
-- (no new derivative-existence theory needed). Gives PI/PJ/Q in the exact hJetVi/hJetVj/hJetQ shapes of
-- ChartJetHessianMixed / HCompNearCarryKPrimeGateRestrictedCoV, RESTRICTED to a near-0 neighbourhood
-- (hJetVi/hJetVj) and the field centre 0 (hJetQ). Does NOT close the literal ∀y-unrestricted, ∀z∈K
-- uniform-in-fixed-x hypothesis (chart-coverage across K is a separate unestablished geometric fact,
-- per gpt-5.6-sol high consult) -- so cp983's r2 diagnosis for the FULL hypothesis stands -- but confirms
-- the LOCAL calculus piece is mechanical/free, correcting the earlier 5-dispatch size estimate for that
...
-- that this pointwise-germ form (not a stronger "simultaneous" packaging) is the shape the
...
-- s ↦ Function.update ξ.2 k s (base point ξ.1 frozen, output component jj), producing the exact
...
-- next dispatch (J4-1128) targets a concrete Pfield/Pifield/Pjfield/Qfield representative with
-- explicit fderiv-based derivative coefficients (not classical-choice gluing, for tractable
...
-- bookkeeping, per J4-1151's flagged optional item — does NOT touch the `hWmeas` wall. std-3.
-- NOT a₁=R/6 (STRICTLY CONDITIONAL on {hDuhamel,hDConv,hCConv}, UNCHANGED).
import QIQTH.TripleHEmeasV5
