...
| `QIQTH.NoConcentration` | **Central audit:** Linear unitary measurement-decoherence preserves branch weights — it does NOT concentrate amplitudes to 0/1. Equal-superposition input ψ = (\|0⟩+\|1⟩)/√2 leaves weights (½,½). Theorems 1/4 ("single-record per-run") therefore do NOT arise by any amplitude-trimming or dynamics modification: the global wave function stays exactly unitary. The single resident record comes from two non-dynamical ingredients — the (FQ) resolution-equivalence on regional states (a finite-capacity residence/individuation constraint, not an operation on amplitudes) plus microscopic-IC selection of which record the actual per-run universe carries. |
