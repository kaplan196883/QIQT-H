...
/-!
# JET-5 phase 4 (J4-648): the D5 two-point crux, staged

Brick **J4-648** of the JET-5 campaign toward the truly-unconditional `a₁ = R/6`
(`docs/qg_roadmap/HEAT_KERNEL_GAP_PLAN.md`): the staged J5-3 crux.

* **(i)** `expJet4Val_v_two_pt_Icc_const` — the `[0,1]`-uniform FOURTH-variation two-point
  Lipschitz bound: the identical 51-sub-term `ρ₄`-telescope of the banked
  `expJet4Val_v_two_pt_diff` (`ExpJet4ValFull.lean`), re-run with the uniform Grönwall
  `gronwall_vec_residual_Icc` in place of the `t = 1` `gronwall_vec_residual` — exactly the
  `expJet3Val_v_two_pt_diff ↦ expJet3Val_v_two_pt_Icc_const` move one Fréchet order up.
  This is the lower-jet Lipschitz FEEDER the jet-5 `(4+1)` telescope blocks consume.
* **(ii)** `expJet5_v_residual_hasDerivWithinAt` — the Jet₅ `R_v − R_w` residual ODE identity
  with the 51-term source difference `Θ₅_v − Θ₅_w`.
* **(iii-a)** `expJet5Val_v_two_pt_diff_gronwall` — the residual-Grönwall application carrying
  the genuine `ρ₅`-residual bound as an explicit hypothesis, + curved-witness non-vacuity gate.

## Honest firewall (binding)

**What is NOT closed here:** the concrete 202-sub-term `ρ₅`-telescope discharging `hrbound`
(hence the full `expJet5Val_v_two_pt_diff` + `expJet5VtpConst` packaging — stage (iii-b)),
the J5-4 `expJetD5` packaging, J5-5/J5-6.  This does NOT establish `exp_p ∈ C⁵`, does NOT
...
the Jet-5 completion + the Duhamel carry + fat-K carriers + capstone co-instantiation).

Non-vacuity: stage (i) carries the IDENTICAL hypothesis list of the banked
`expJet4Val_v_two_pt_diff` (jointly satisfied on the mainline `ExpJet4DFull` consumption
chain); stages (ii)/(iii-a) are gated below at the genuinely curved witness
(`curvedRNCMetric (−1)`).
-/
...
/-! ### Generic multilinear CLM-application norm bounds and telescoping identities
(local private copies — the `ExpJet4ValFull` originals are `private`) -/
...
/-! ### Stage (i): the `[0,1]`-uniform fourth-variation two-point bound -/

set_option maxHeartbeats 6400000 in
/-- **(J4-648 stage (i)) The `[0,1]`-uniform fourth-variation two-point Lipschitz bound.**
    The `∀ t ∈ [0,1]` mirror of `expJet4Val_v_two_pt_diff` (its `t = 1` endpoint): the
    IDENTICAL fifty-one-sub-term `ρ₄`-telescope, with the `t = 1` vector Grönwall
    (`gronwall_vec_residual`) replaced by the uniform `gronwall_vec_residual_Icc` — exactly
    the `expJet3Val_v_two_pt_diff ↦ expJet3Val_v_two_pt_Icc_const` move one Fréchet order up.
    THE reusable ingredient the jet-5 `(4+1)` `ρ₅`-telescope consumes (its differenced
    fourth-variation factor `Q^{····}_v − Q^{····}_w`). -/
theorem expJet4Val_v_two_pt_Icc_const
  ...
  have hgron := gronwall_vec_residual_Icc (fun s => Rv s - Rw s)
    (fun t => (fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
      ...
      + (expJet4Rhs g gi hC p v Φv Qhkv Qhlv Qhmv Qklv Qkmv Qlmv Qhklv Qhkmv Qhlmv Qklmv h k l m t
         - expJet4Rhs g gi hC p w Φw Qhkw Qhlw Qhmw Qklw Qkmw Qlmw Qhklw Qhkmw Qhlmw Qklmw h k l m t))
    (fun t => fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t))
...
/-! ### Stage (ii): the Jet₅ residual ODE identity -/

set_option maxHeartbeats 1000000 in
/-- **(J4-648 stage (ii)) The Jet₅ `R_v − R_w` parameter-residual ODE (residual identity).**
    With `Rv`,`Rw` the Jet₅ fifth-variation witnesses for base parameters `v`,`w` (each with its
    own propagator `Φ·`, ten pair `Q··`, ten triple `Q···` and five quadruple `Q····` variations,
    and derivative law `R'_· = DF(Y_· t)(R_·) + Θ₅^{hklmr}_·(t)`), the parameter residual
    `D(t) = Rv(t) − Rw(t)` obeys, on `[0,1]`,
    `D'(t) = DF(Y_v t)(D t) + ([DF(Y_v t) − DF(Y_w t)](Rw t) + (Θ₅_v(t) − Θ₅_w(t)))`.
    The exact mirror of `expJet4_v_residual_hasDerivWithinAt`, one Fréchet order up. -/
theorem expJet5_v_residual_hasDerivWithinAt (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
    (p v w : Point n)
    (Φv Φw : ℝ → ((Point n × Point n) →L[ℝ] (Point n × Point n)))
    (Qhkv Qhlv Qhmv Qhrv Qklv Qkmv Qkrv Qlmv Qlrv Qmrv Qhklv Qhkmv Qhkrv Qhlmv Qhlrv Qhmrv Qklmv Qklrv Qkmrv Qlmrv Qhklmv Qhklrv Qhkmrv Qhlmrv Qklmrv : ℝ → (Point n × Point n))
    (Qhkw Qhlw Qhmw Qhrw Qklw Qkmw Qkrw Qlmw Qlrw Qmrw Qhklw Qhkmw Qhkrw Qhlmw Qhlrw Qhmrw Qklmw Qklrw Qkmrw Qlmrw Qhklmw Qhklrw Qhkmrw Qhlmrw Qklmrw : ℝ → (Point n × Point n))
    (Rv Rw : ℝ → (Point n × Point n)) (h k l m r : Point n)
    (hRvd : ∀ t ∈ Set.Icc (0 : ℝ) 1, HasDerivWithinAt Rv
      ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)) (Rv t)
        + expJet5Rhs g gi hC p v Φv Qhkv Qhlv Qhmv Qhrv Qklv Qkmv Qkrv Qlmv Qlrv Qmrv Qhklv Qhkmv Qhkrv Qhlmv Qhlrv Qhmrv Qklmv Qklrv Qkmrv Qlmrv Qhklmv Qhklrv Qhkmrv Qhlmrv Qklmrv h k l m r t) (Set.Icc (0 : ℝ) 1) t)
    (hRwd : ∀ t ∈ Set.Icc (0 : ℝ) 1, HasDerivWithinAt Rw
      ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)) (Rw t)
        + expJet5Rhs g gi hC p w Φw Qhkw Qhlw Qhmw Qhrw Qklw Qkmw Qkrw Qlmw Qlrw Qmrw Qhklw Qhkmw Qhkrw Qhlmw Qhlrw Qhmrw Qklmw Qklrw Qkmrw Qlmrw Qhklmw Qhklrw Qhkmrw Qhlmrw Qklmrw h k l m r t) (Set.Icc (0 : ℝ) 1) t)
      ...
      ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)) (Rv t - Rw t)
        + ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
           ...
           + (expJet5Rhs g gi hC p v Φv Qhkv Qhlv Qhmv Qhrv Qklv Qkmv Qkrv Qlmv Qlrv Qmrv Qhklv Qhkmv Qhkrv Qhlmv Qhlrv Qhmrv Qklmv Qklrv Qkmrv Qlmrv Qhklmv Qhklrv Qhkmrv Qhlmrv Qklmrv h k l m r t
              - expJet5Rhs g gi hC p w Φw Qhkw Qhlw Qhmw Qhrw Qklw Qkmw Qkrw Qlmw Qlrw Qmrw Qhklw Qhkmw Qhkrw Qhlmw Qhlrw Qhmrw Qklmw Qklrw Qkmrw Qlmrw Qhklmw Qhklrw Qhkmrw Qhlmrw Qklmrw h k l m r t)))
      (Set.Icc (0 : ℝ) 1) t := by
  ...
  have heq :
      ...
      = ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)) (Rv t)
          + expJet5Rhs g gi hC p v Φv Qhkv Qhlv Qhmv Qhrv Qklv Qkmv Qkrv Qlmv Qlrv Qmrv Qhklv Qhkmv Qhkrv Qhlmv Qhlrv Qhmrv Qklmv Qklrv Qkmrv Qlmrv Qhklmv Qhklrv Qhkmrv Qhlmrv Qklmrv h k l m r t)
        - ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)) (Rw t)
          + expJet5Rhs g gi hC p w Φw Qhkw Qhlw Qhmw Qhrw Qklw Qkmw Qkrw Qlmw Qlrw Qmrw Qhklw Qhkmw Qhkrw Qhlmw Qhlrw Qhmrw Qklmw Qklrw Qkmrw Qlmrw Qhklmw Qhklrw Qhkmrw Qhlmrw Qklmrw h k l m r t) := by
    simp only [map_sub, ContinuousLinearMap.sub_apply]
    abel
  rw [heq]
  exact hcomb

/-! ### Stage (iii-a): the residual-Grönwall application -/

set_option maxHeartbeats 1000000 in
/-- **(J4-648 stage (iii-a)) The jet-5 value two-point difference — residual-Grönwall
    application.**  Given the Jet₅ fifth-variation witnesses `Rv`, `Rw` (each solving its
    `R'_· = DF(Y_·)(R_·) + Θ₅_·` ODE with `R_·(0) = 0`), the `[0,1]`-bound `Kstar` on
    `‖DF(Y_v t)‖`, and — carried as an explicit, genuine hypothesis — the `ρ₅`-residual bound
    `‖[DF(Y_v)−DF(Y_w)](R_w) + (Θ₅_v − Θ₅_w)‖ ≤ ρ₅` on `[0,1]`, the parameter residual
    `Rv − Rw` (which solves `D' = DF(Y_v)(D) + ρ₅`, `expJet5_v_residual_hasDerivWithinAt`)
    satisfies, via the vector Grönwall (`gronwall_vec_residual`),
    `‖Rv 1 − Rw 1‖ ≤ ρ₅·e^{Kstar}`.  This is the full `expJet5Val_v_two_pt_diff` MINUS the
    202-sub-term algebraic `ρ₅`-telescope that discharges `hrbound` with the concrete `ρ₅`;
    the exact mirror of `expJet4Val_v_two_pt_diff_gronwall` one Fréchet order up. -/
theorem expJet5Val_v_two_pt_diff_gronwall (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
    ...
    (hrbound : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖(fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
           - fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)) (Rw t)
         + (expJet5Rhs g gi hC p v Φv Qhkv Qhlv Qhmv Qhrv Qklv Qkmv Qkrv Qlmv Qlrv Qmrv Qhklv Qhkmv Qhkrv Qhlmv Qhlrv Qhmrv Qklmv Qklrv Qkmrv Qlmrv Qhklmv Qhklrv Qhkmrv Qhlmrv Qklmrv h k l m r t
            - expJet5Rhs g gi hC p w Φw Qhkw Qhlw Qhmw Qhrw Qklw Qkmw Qkrw Qlmw Qlrw Qmrw Qhklw Qhkmw Qhkrw Qhlmw Qhlrw Qhmrw Qklmw Qklrw Qkmrw Qlmrw Qhklmw Qhklrw Qhkmrw Qhlmrw Qklmrw h k l m r t)‖ ≤ ρ5) :
    ‖Rv 1 - Rw 1‖ ≤ ρ5 * Real.exp Kstar :=
  gronwall_vec_residual (fun s => Rv s - Rw s)
    (fun t => (fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
      ...
      + (expJet5Rhs g gi hC p v Φv Qhkv Qhlv Qhmv Qhrv Qklv Qkmv Qkrv Qlmv Qlrv Qmrv Qhklv Qhkmv Qhkrv Qhlmv Qhlrv Qhmrv Qklmv Qklrv Qkmrv Qlmrv Qhklmv Qhklrv Qhkmrv Qhlmrv Qklmrv h k l m r t
         - expJet5Rhs g gi hC p w Φw Qhkw Qhlw Qhmw Qhrw Qklw Qkmw Qkrw Qlmw Qlrw Qmrw Qhklw Qhkmw Qhkrw Qhlmw Qhlrw Qhmrw Qklmw Qklrw Qkmrw Qlmrw Qhklmw Qhklrw Qhkmrw Qhlmrw Qklmrw h k l m r t))
    (fun t => fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t))
    Kstar ρ5 hKstar0 hρ50
    (by simp only [hRv0, hRw0, sub_zero])
    (fun t ht => expJet5_v_residual_hasDerivWithinAt g gi hC p v w Φv Φw
      Qhkv Qhlv Qhmv Qhrv Qklv Qkmv Qkrv Qlmv Qlrv Qmrv Qhklv Qhkmv Qhkrv Qhlmv Qhlrv Qhmrv Qklmv Qklrv Qkmrv Qlmrv Qhklmv Qhklrv Qhkmrv Qhlmrv Qklmrv
      Qhkw Qhlw Qhmw Qhrw Qklw Qkmw Qkrw Qlmw Qlrw Qmrw Qhklw Qhkmw Qhkrw Qhlmw Qhlrw Qhmrw Qklmw Qklrw Qkmrw Qlmrw Qhklmw Qhklrw Qhkmrw Qhlmrw Qklmrw Rv Rw h k l m r hRvd hRwd t ht)
    (fun t ht => hKstarv t ht)
    (fun t ht => hrbound t ht)
...
/-- **Non-vacuity gate for stages (ii)/(iii-a): all antecedents jointly discharged at the
    genuinely curved witness.**  At `g^κ = curvedRNCMetric (−1)` (`p = v = w = 0`, `Φ = 0`,
    zero lower variations), the `expJet5Fund` witness `R` (taken for BOTH parameter slots)
    satisfies every hypothesis of `expJet5Val_v_two_pt_diff_gronwall` — the residual source
    difference vanishes identically (`v = w`), so `ρ₅ = 0` closes `hrbound` — and the theorem
    fires: `‖R 1 − R 1‖ ≤ 0·e^{Kstar}`.  This certifies the antecedent set of the D5
    two-point Grönwall layer is satisfiable at curved data (NOT a curvature computation and
    NOT `a₁ = R/6`). -/
theorem expJet5Val_v_two_pt_diff_gronwall_gate (h k l m r : Point n) :
    ∃ (R : ℝ → (Point n × Point n)) (Kstar : ℝ),
      ‖R 1 - R 1‖ ≤ 0 * Real.exp Kstar := by
  ...
  obtain ⟨Kstar, hKstar0, hKstar⟩ := expJet_fderiv_tube_bddAbove (n := n)
    (curvedRNCMetric (-1)) (curvedRNCInv (-1)) (curvedRNC_hChr (-1) (by norm_num)) 0 0 hv
  obtain ⟨R, hR0, hRcont, hRint, hRderiv⟩ :=
    expJet5Fund (n := n) (curvedRNCMetric (-1)) (curvedRNCInv (-1))
      ...
      h k l m r
  refine ⟨R, Kstar, ?_⟩
  exact expJet5Val_v_two_pt_diff_gronwall (curvedRNCMetric (-1)) (curvedRNCInv (-1))
    (curvedRNC_hChr (-1) (by norm_num)) 0 0 0 Kstar hKstar0 (fun _ => 0) (fun _ => 0)
    (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0)
    (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0) (fun _ => 0)
    R R h k l m r hR0 hR0 hRderiv hRderiv hKstar 0 le_rfl
    (fun t ht => by
      simp only [sub_self, ContinuousLinearMap.zero_apply, add_zero, norm_zero, le_refl])
