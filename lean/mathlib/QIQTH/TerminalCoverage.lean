/-
  TerminalCoverage — J4-415: THE TERMINAL-RESIDUE COVERAGE CHECKS + THREADINGS.

  Closing move for the Duhamel-core threading arc: the two TERMINAL residues carried VERBATIM by
  `DuhamelCoreThreaded.truncatedDuhamelCore_threaded` (J4-414) — `hdiff` (the a.e.-pointwise
  `∂_w`-under-`∫₀ᵗ∫` first-derivative `HasDerivAt` family) and `hPd2conv` (the atomic frozen→full
  second-partial interchange limit) — are checked against the BANKED per-`u` census suppliers and,
  where the shapes match VERBATIM, THREADED so their own honest lower-level carries become the
  residue.

  ══════════════════════════════════════════════════════════════════════════════════════════════════
  ## THE J4-415 DONT-UNDERCREDIT VERDICTS.

  •  (V1) `hPd2conv` vs `PerUCensusTuple.hPd2conv_perU_fired` (J4-407).  The threaded core's `hPd2conv`
     binder is, VERBATIM (same witness `vanVleckGatedWitness g gi hChr hK S a b`, same gate, same Levi
     source `leviSeries (heatOp g gi W)`, same window `U`, same `pd∘pd` frozen→full second-partial
     `Tendsto` at the concrete witness pair, binder order `∀ u ∈ U, ∀ i`), the CONCLUSION of the fired
     per-`u` census tuple `hPd2conv_perU_fired`.  **VERDICT: COVERED.**  Threading it (`hPd2conv_threaded`)
     trades the opaque `hPd2conv` binder for the fired version's own honest census carries
     {`nb`/`hnb_open`/`hnb0` · `hUpos` · `hProv` linewise provider · `fderivBulk`/`gderiv`/`C₀`/`C₁`/`C₂`
     · `hGint` integrability · `hbulkderiv`/`hsliver`/`hcont` shared analytic sliver carries ·
     `hfrozen_pd1` frozen germ-link}.

  •  (V2) `hdiff` vs `W2Finish.w2_hdiff` (J4-397).  The threaded core's `hdiff` binder is, VERBATIM
     (with `F := leviSeries (heatOp g gi W)`; same first-derivative kernel `witnessFieldDeriv … i`,
     same second-derivative kernel `witnessFieldDeriv2 … i`, same base `0`, same window/strip
     `Set.uIoc 0 (u − εₘ)`, same real-line nbhd `snb`, same a.e.-`s`, `∀ w ∈ snb` `HasDerivAt`
     shape), the CONCLUSION of `w2_hdiff`.  **VERDICT: COVERED.**  Threading it (`hdiff_threaded`)
     trades the opaque `hdiff` binder for `w2_hdiff`'s single honest carry `hInnerData` — the bundled
     `z`-level differentiation-under-∫ inputs (nbhd `znb`, `z`-measurabilities, base `z`-integrability,
     integrable `z`-dominator, `z`-level pointwise `HasDerivAt` family).

  Both TERMINAL residues are therefore DISCHARGEABLE into strictly lower-level, satisfiable, pointwise
  analytic census DATA — NOT for free: each threading trades one opaque binder for concrete carries.

  ══════════════════════════════════════════════════════════════════════════════════════════════════
  ⚠  HONESTY FIREWALL.  THIS FILE IS **NOT** `a₁ = R/6` and proves NOTHING new about `R/6`.  It is a
  binder-coverage / re-plumbing brick: it re-exports two banked suppliers in the EXACT threaded-core
  binder shapes, and (stretch) absorbs both into a `_v2` Duhamel-core carrier.  The capstone
  `a1_R6_from_data_v5` remains the maximally-unconditional **CONDITIONAL** a₁ two-jet; replacing the
  two terminal binders by their census carries closes NOTHING deeper.  No `sorry` (header prose
  excepted), no new axioms, no `:= True`, no vacuous / unsatisfiable hypothesis, no hypothesis equal
  to (or trivially yielding) the conclusion, no existing file edited, nothing committed, nothing wired
  into `QIQTH.lean` / `AxiomAudit`.  ⚠ NOT `a₁ = R/6`.
-/
import Mathlib
...
namespace QIQTH.TerminalCoverage
...
/-! ###############################################################################
    ### (V1) `hPd2conv_threaded` — the atomic frozen→full second-partial `Tendsto`, census-discharged.
    ############################################################################### -/

/-- **★★★ (V1) `hPd2conv_threaded`.**  THE J4-415 V1 LEAN WITNESS.  The threaded core's terminal
    `hPd2conv` binder — the atomic per-`u` frozen→full SECOND-partial interchange limit
      `∀ u ∈ U, ∀ i, Tendsto (fun m ↦ ∂ᵢ∂ᵢ(heatConvFrozen W F u (u−εₘ) · 0))
                            atTop (𝓝 (∂ᵢ∂ᵢ(heatConv W F u · 0)))`,
    with `W := vanVleckGatedWitness g gi hChr hK S a b`, `F := leviSeries (heatOp g gi W)` — produced
    VERBATIM from the fired per-`u` census tuple `PerUCensusTuple.hPd2conv_perU_fired` (J4-407).  The
    census carries {`nb`/`hnb_open`/`hnb0` · `hUpos` · `hProv` · `fderivBulk`/`gderiv`/`C₀`/`C₁`/`C₂` ·
    `hGint` · `hbulkderiv`/`hsliver`/`hcont` · `hfrozen_pd1`} REPLACE the opaque `hPd2conv` binder with
    concrete, satisfiable, pointwise analytic DATA; NONE is the conclusion.  ⚠ NOT `a₁ = R/6`. -/
theorem hPd2conv_threaded (g gi : Point n → Fin n → Fin n → ℝ)
...
/-! ###############################################################################
    ### (V2) `hdiff_threaded` — the a.e.-`s`, `∀ w ∈ snb` first-derivative `HasDerivAt` family, discharged.
    ############################################################################### -/

/-- **★★★ (V2) `hdiff_threaded`.**  THE J4-415 V2 LEAN WITNESS.  The threaded core's terminal `hdiff`
    binder — the a.e.-`s`, `∀ w ∈ snb` inner-slice first-derivative `HasDerivAt` family
      `HasDerivAt (fun w ↦ ∫ z, witnessFieldDeriv … i (u−s) (update 0 i w) z · F s z 0)
                  (∫ z, witnessFieldDeriv2 … i (u−s) (update 0 i w) z · F s z 0) w`,
    with `F := leviSeries (heatOp g gi W)` — produced VERBATIM from `W2Finish.w2_hdiff` (J4-397).  The
    single honest carry `hInnerData` (the bundled `z`-level differentiation-under-∫ inputs) REPLACES
    the opaque `hdiff` binder; it is not the conclusion.  ⚠ NOT `a₁ = R/6`. -/
theorem hdiff_threaded (g gi : Point n → Fin n → Fin n → ℝ)
    (hChr : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
    {K : Set (Point n)} (hK : IsCompact K) (S : Point n → Set (Point n)) (a b : ℝ)
    (U : Set ℝ) (snb : Set ℝ)
          ...
          HasDerivAt (fun w => ∫ z, witnessFieldDeriv g gi hChr hK S a b i (u - s)
            ...
            * leviSeries (heatOp g gi (vanVleckGatedWitness g gi hChr hK S a b)) s z 0)
            (∫ z, witnessFieldDeriv2 g gi hChr hK S a b i (u - s)
              (Function.update (0 : Point n) i w) z
            * leviSeries (heatOp g gi (vanVleckGatedWitness g gi hChr hK S a b)) s z 0) w :=
  QIQTH.W2Finish.w2_hdiff g gi hChr hK S a b U snb hInnerData

/-! ###############################################################################
    ### (V3) `truncatedDuhamelCore_threaded_v2` — the Core carrier with BOTH terminal residues absorbed.
    ############################################################################### -/

/-- **★★★★ (V3) `truncatedDuhamelCore_threaded_v2`.**  The endgame-gate Duhamel Core carrier
    `TruncatedDuhamelCore g gi (vanVleckGatedWitness g gi hChr hK S a b) t`, assembled from
    `DuhamelCoreThreaded.truncatedDuhamelCore_threaded` (J4-414) with its TWO terminal residues REMOVED
    and DISCHARGED internally via the banked per-`u` census:
      • `hdiff`    ← `hdiff_threaded` (V2, ← `W2Finish.w2_hdiff`), carry = `hInnerData`;
      • `hPd2conv` ← `hPd2conv_threaded` (V1, ← `PerUCensusTuple.hPd2conv_perU_fired`), carries =
        {`nbP`/`hnbP_open`/`hnbP0` · `hProvP` · `fderivBulk`/`gderiv`/`C₀`/`C₁`/`C₂` · `hGintP` ·
        `hbulkderiv`/`hsliver`/`hcont` · `hfrozen_pd1`}.
    All OTHER binders are carried VERBATIM.  The residue is therefore {the moment wall `hslot`/`hGpow`
    · the satisfiable census · the carried data}; the two terminal `hdiff`/`hPd2conv` bundles are gone.
    Every hypothesis is satisfiable and non-vacuous; NONE is the conclusion.  ⚠ NOT `a₁ = R/6`. -/
theorem truncatedDuhamelCore_threaded_v2 (g gi : Point n → Fin n → Fin n → ℝ)
    (hChr : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
    {K : Set (Point n)} (hK : IsCompact K) (S : Point n → Set (Point n)) (a b : ℝ)
    ...
    -- ── the WALL-A interchange census (as in J4-414), ∀ concrete data:
  ...
  -- (V2) discharge the terminal `hdiff` residue from the inner-`z` census.
  have hdiff := QIQTH.TerminalCoverage.hdiff_threaded g gi hChr hK S a b U snb hInnerData
  -- (V1) discharge the terminal `hPd2conv` residue from the per-`u` census.
  have hPd2conv := QIQTH.TerminalCoverage.hPd2conv_threaded g gi hChr hK S a b U hUpos
    nbP hnbP_open hnbP0 hProvP fderivBulk gderiv C₀ C₁ C₂ hGintP hbulkderiv hsliver hcont hfrozen_pd1
  -- re-export the collapsed Core carrier via the J4-414 threaded core.
  exact QIQTH.DuhamelCoreThreaded.truncatedDuhamelCore_threaded g gi hChr hK S a b
    ...
    D0 D1 hD0 hD1 hbnd E₀ E₁ C_L aT hE₀ hE₁ hC_L haT hUlb hEdom hFdom hFzero hIlo hIhi hEcomb
    ...
    Lc Bcomp Q Sconst hLc hBcomp hQ hSconst hslot hcap hEndpoint hPd2conv Cpair hCpair hGpow

end QIQTH.TerminalCoverage

/-! ## Axiom checks — every public theorem is `std-3` (propext, Classical.choice, Quot.sound). -/
section AxiomChecks
open QIQTH.TerminalCoverage
#print axioms hPd2conv_threaded
#print axioms hdiff_threaded
#print axioms truncatedDuhamelCore_threaded_v2
end AxiomChecks
