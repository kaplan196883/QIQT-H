...
theorem hasDerivAt_inner_boostUnitary_imaginary_pos
  ...
  -- `hF0_int` discharged from `hf2`: ⟪conj f, f⟫ = ‖f‖², integrable by the L² property (was a labelled input).
  have hF0_int : Integrable (fun θ => (starRingEnd ℂ) (f θ) * f θ) (volume : Measure ℝ) := by
    have hconj : (fun θ => (starRingEnd ℂ) (f θ) * f θ)
        = (fun θ => ((‖f θ‖ ^ 2 : ℝ) : ℂ)) := by
      funext θ; rw [RCLike.conj_mul]; norm_cast
    rw [hconj]
    exact ((memLp_two_iff_integrable_sq_norm hf2.aestronglyMeasurable).mp hf2).ofReal
...
theorem freeField_oneParticle_hFlux {m : ℝ} (hm : 0 < m)
  ...
  have hbc := hasDerivAt_inner_boostUnitary_imaginary_pos f f' hf2 hf_int hfd hf'_meas B hB
...
theorem freeField_component_hFlux {m : ℝ} (hm : 0 < m)
  ...
  have hHil := freeField_oneParticle_hFlux hm f f' hf2 hf_int hfd hf'_meas B hB
    hbar Tkk hTkk
