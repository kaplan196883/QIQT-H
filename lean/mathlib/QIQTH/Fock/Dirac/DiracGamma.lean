...
/-- **The spin generator acts only in its plane: `[σ_ab, γ_c] = 0` for `c ⟂ a, c ⟂ b`.**  A gamma
...
complement invariant (the planar nature of a Lorentz transformation).  Together with `diracSigma_comm_
gamma_left` (which rotates the plane) this fully characterizes the spinor action of `σ_ab` on the gamma
4-vector. -/
theorem diracSigma_comm_gamma_ortho {a b c : M} (hab : Q.IsOrtho a b)
    (hac : Q.IsOrtho a c) (hbc : Q.IsOrtho b c) :
    diracSigma Q a b * diracGamma Q c = diracGamma Q c * diracSigma Q a b := by
  have hcomm : (diracGamma Q a * diracGamma Q b) * diracGamma Q c
      = diracGamma Q c * (diracGamma Q a * diracGamma Q b) := by
    rw [show (diracGamma Q a * diracGamma Q b) * diracGamma Q c
          = diracGamma Q a * (diracGamma Q b * diracGamma Q c) from by noncomm_ring,
        diracGamma_swap_ortho Q hbc,
        show diracGamma Q a * -(diracGamma Q c * diracGamma Q b)
          = -((diracGamma Q a * diracGamma Q c) * diracGamma Q b) from by noncomm_ring,
        diracGamma_swap_ortho Q hac,
        show -(-(diracGamma Q c * diracGamma Q a) * diracGamma Q b)
          = diracGamma Q c * (diracGamma Q a * diracGamma Q b) from by noncomm_ring]
  rw [diracSigma_ortho Q hab, add_mul, mul_add, hcomm]
