/-
  HDConvLiveHmassoneDischarged — feed the BANKED abstract-`g` `hmassone` discharge (J4-896,
  `HmassoneFromGateAnnulusSplit.hmassone_from_gate_annulus_split`) INTO the LIVE order-1 `hDConv`
  (`HDConvLiveGateWired.hDConv_live_gate_wired`), ELIMINATING the analytic approximate-identity limit
  `hmassone` from the live `hDConv` antecedent surface.
  ...
  banked results:
    ...
    • `HDConvLiveGateWired.hDConv_live_gate_wired` (J4-897): the LIVE order-1 capstone's `hDConv`
      antecedent (diagonal time-`DifferentiableAt` of the moving-source heat convolution at the concrete
      van-Vleck gated witness, in the capstone's OWN raw `gatedKernel` form), reduced to the ~130-binder
      shared frozen/moving census — one member of which is the OPAQUE analytic limit
          `hmassone : Tendsto (fun m => ∫ z, Wit (epsSeq m) 0 z) atTop (𝓝 1)`
      (the mass-normalization / approximate-identity boundary trace as `ε → 0`);

    • `HmassoneFromGateAnnulusSplit.hmassone_from_gate_annulus_split` (J4-896): the abstract-`g` discharge
      of EXACTLY that `hmassone` proposition, down to the SATISFIABLE pre-`ρ` gate-activation triple
      `{rS, hKball, hSact}` + the base metric/gauge carriers `{hgCD, hgiCD, hgpos, hgdet0, h0Kmem}` + the
      census's OWN witness-slice measurability `hWmeas` (= `hWslice`) and zeroth wide domination `hWDom`
      (= `hDom`).

  This file COMPOSES them: it restates `hDConv_live_gate_wired` with the `hmassone` binder REMOVED and
  DERIVED internally from `hmassone_from_gate_annulus_split`, reusing the census's OWN `hWmeas`/`hWDom`
  and adding only the (independently satisfiable, capstone-geometry-supplied) carriers
  `{hgCD, hgiCD, hgpos, h0Kmem, hgdet0, ha, hab, rS, hrS, hKball, hSact}`.  Net effect: the OPAQUE
  approximate-identity analytic LIMIT `hmassone` is GONE from the live `hDConv` surface, traded for
  elementary satisfiable geometry + gate-activation inputs (all supplied by the capstone's own
  geometry).  This is the same discipline the campaign applied to isolate `hmassone` in the first place;
  landing it INTO the live `hDConv` is the missing composition step.
  ...
  ## WHAT THIS FILE LANDS.
    • `hDConv_live_gate_hmassone_discharged` — the LIVE order-1 capstone's `hDConv` antecedent
      proposition (raw `gatedKernel` form), now a THEOREM whose antecedent surface NO LONGER carries the
      analytic approximate-identity limit `hmassone`.  Every other census binder is reproduced verbatim
      from `hDConv_live_gate_wired`; `hmassone` is replaced by the satisfiable geometry/gate carriers of
      the banked J4-896 discharge, from which it is reconstructed internally.
  ...
  dominations, interchange bundles, sliver carries) is NOT attempted here.  a₁ = R/6 remains CONDITIONAL
  on `{hDuhamel, hDConv, hCConv}`; this brick only NORMALIZES the `hDConv` dependency by eliminating one
  analytic-limit binder in favour of satisfiable geometry inputs.
-/
...
open QIQTH.A1R6CoreAtGate QIQTH.ExpMap QIQTH.ResidueBound
open scoped Interval Topology BigOperators

namespace QIQTH.HDConvLiveHmassoneDischarged
...
/-- **★★★ `hDConv_live_gate_hmassone_discharged`.**  The LIVE order-1 capstone's `hDConv` antecedent —
    the diagonal time-`DifferentiableAt` of the moving-source heat convolution at the concrete van-Vleck
    gated witness, stated in the capstone's OWN raw `gatedKernel` / `globalCutoffParametrixWitnessN`
    form — with the analytic approximate-identity LIMIT `hmassone` ELIMINATED from the antecedent
    surface.  Every other binder is verbatim from `HDConvLiveGateWired.hDConv_live_gate_wired`;
    `hmassone` is DERIVED internally from `HmassoneFromGateAnnulusSplit.hmassone_from_gate_annulus_split`
    fed with the census's own `hWmeas`/`hWDom` + the (satisfiable, capstone-geometry-supplied) carriers
...
theorem hDConv_live_gate_hmassone_discharged (g gi : Point n → Fin n → Fin n → ℝ)
  ...
  exact QIQTH.HDConvLiveGateWired.hDConv_live_gate_wired g gi hChr hK S a b F hFeq t T hT U hUopen htU
...
end QIQTH.HDConvLiveHmassoneDischarged

section AxiomChecks
open QIQTH.HDConvLiveHmassoneDischarged
#print axioms hDConv_live_gate_hmassone_discharged
end AxiomChecks
