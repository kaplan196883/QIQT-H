/-
  UniformFlowOctupleSupply — Plan v6 Task I (C³ climb, brick 2): the OCTUPLE-flow SUPPLY, the
  base-velocity Fréchet derivative of the octupled-flow endpoint — one order up from
  `QuadrupleFlowSupply.uniformFlow_quadrupleEndpoint_baseVelocity_hasFDerivAt`.

  ══════════════════════════════════════════════════════════════════════════════════════════════
  ⚠  HONEST FIREWALL.  NOT `a₁ = R/6`.  The mechanical "supply" half of the FOURTH velocity-jet climb
  ...
  ── CONTEXT.  Each `genericDoubled` doubling of the geodesic field climbs one velocity-derivative order:
      `geodesicField` → D¹,  `doubledField` → D²,  `quadrupledField` → D³ (banked).
  The fourth jet (⟹ C³) needs the base-velocity Fréchet derivative of the OCTUPLED-flow endpoint,
  built as a genuine confined `octupledField = genericDoubled (genericDoubled (doubledField g gi))`
  integral-curve family — mirroring the quadruple supply one order up.

  ── WHAT LANDS (DERIVED; NO `sorry`, NO new axioms, NO `expRho`).
    • `uniformFlow_octupleEndpoint_baseVelocity_hasFDerivAt` — for `q ∈ K`, `‖v‖ < ρ_K`, seeds `a b c`,
        there are a velocity-Jacobi family `Jf` (seed `(0,b)`), a `doubledField`-linearized family `Uf`
        (seed `((0,a),(0,0))`), and a `quadrupledField`-linearized family `Tf` (seed
        `(((0,c),(0,0)),((0,0),(0,0)))`) such that the OCTUPLED-flow endpoint
          `δ ↦ ((((tube(v+δ) 1, Jf δ 1), Uf δ 1), Tf δ 1))`
        is Fréchet-differentiable at `0`.  A GENUINE confined octuple integral-curve family: the
        quadruple base curve `((tube, Jf), Uf)` is the banked confined quadruple supply; `Tf` is a
        genuine `quadrupledField`-linearized field along it (Grönwall-confined via
        `quadrupledField_fderiv_bddOn_compact`); the pair is an `octupledField` integral curve
        (`genericDoubled_prod_hasDerivAt`); fed to `autonomousFlow_endpoint_hasFDerivAt_window_exists`
        with `Φ := octupledField`, all 8-fold field regularity discharged from `UniformFlowOctupleField`.
    • `uniformFlow_octupleEndpoint_component_hasFDerivAt` — the projected deep scalar-slot component
        `δ ↦ (Tf δ 1).2.2.1`, Fréchet-differentiable at `0`.

  ── HONEST FIREWALL (binding) — what the FOURTH JET still needs.
    The base-velocity Fréchet derivative of the octuple endpoint lands here fully.  The per-seed FOURTH
    jet needs the value-IDENTIFICATION linking the octuple deep component `(Tf δ 1).2.2.1` to the third
    jet at `v+δ` (the `Z1`-analogue one order up), a separate brick.  CARRIED.  NO `expRho`.
...
/-- Reducible abbreviations for the tower of phase spaces (State = `Point n × Point n`).  These keep
    the deeply-nested product types short for the elaborator (instance synthesis / defeq go through the
    reducible abbrevs) while staying definitionally equal to the explicit products the
    `doubledField`/`genericDoubled` API is stated over. -/
abbrev St2 (n : ℕ) : Type := (Point n × Point n) × (Point n × Point n)
abbrev St4 (n : ℕ) : Type := St2 n × St2 n
abbrev St8 (n : ℕ) : Type := St4 n × St4 n

/-- **The octuple-flow supply.**  For `q ∈ K`, `‖v‖ < ρ_K`, and seeds `a b c`, there are a velocity-
    Jacobi family `Jf`, a `doubledField`-linearized family `Uf`, and a `quadrupledField`-linearized
    family `Tf` such that the OCTUPLED-flow endpoint
      `δ ↦ ((((tube(v+δ) 1, Jf δ 1), Uf δ 1), Tf δ 1))`
    is Fréchet-differentiable at `0`.  GENUINE confined octuple integral-curve supply; NO `expRho`. -/
theorem uniformFlow_octupleEndpoint_baseVelocity_hasFDerivAt (g gi : Point n → Fin n → Fin n → ℝ)
    ...
    ∃ Tf : Point n → ℝ → St4 n,
      ...
      ∃ L : Point n →L[ℝ] St8 n,
        HasFDerivAt
          (fun δ => (((((uniformFlowTube g gi hC hK q (v + δ) 1, Jf δ 1), Uf δ 1), Tf δ 1)) : St8 n)) L 0 := by
  classical
  ...
  -- The full octuple confinement set.
  set S : Set (St8 n) :=
    Squad ×ˢ Metric.closedBall (0 : St4 n) Tbound with hSdef
  have hScompact : IsCompact S := hSquadcompact.prod (isCompact_closedBall _ _)
  ...
  -- Per-δ: velocity Jacobi `Jc`, doubled-linearized `Uc`, quadrupled-linearized `Tc`, confined.
  have key : ∀ δ : Point n,
  ...
  have hspec : ∀ δ : Point n, ‖v + δ‖ ≤ ρ →
    ...
    fun δ => Classical.choose_spec (Classical.choose_spec (Classical.choose_spec (key δ)))
  -- The base octuple curve `W0 = (((tube v, Jf 0), Uf 0), Tf 0)`, padded continuous.
  ...
  obtain ⟨_, _, _, hJf0pad, _, _, _, hUf0pad, _, _, _, hTf0pad⟩ := hspec 0 hadm0
  set W0 : ℝ → St8 n :=
    fun τ => (((uniformFlowTube g gi hC hK q (v + 0) τ, Jf 0 τ), Uf 0 τ), Tf 0 τ) with hW0def
  have hW0pad : ContinuousOn W0 (Set.Icc (-(1/2) : ℝ) (3/2)) :=
    (((hP0cont.prodMk hJf0pad).prodMk hUf0pad).prodMk hTf0pad)
  -- The `Φ̃̃`-linearized field `Vf δ` along the fixed base octuple curve `W0`.
  ...
  -- The base-velocity seed CLM `δ ↦ ((((0,δ),(0,0)),0₄),0₄)` at the octuple level.
  ...
  set seedCLM : Point n →L[ℝ] St8 n :=
    (ContinuousLinearMap.inl ℝ (St4 n) (St4 n)).comp seedQuad with hseedCLMdef
  have hseed_eq : ∀ δ : Point n,
    ...
    intro δ
    simp [hseedCLMdef, hseedQuaddef, hseedInnerdef, ContinuousLinearMap.comp_apply,
  ...
  -- The per-δ `Φ̃̃`-linearized field along `W0`, seed `seedCLM δ`.
  have varkey : ∀ δ : Point n,
      ∃ Vc : ℝ → St8 n,
        Vc 0 = seedCLM δ ∧
        (∀ τ ∈ Set.Icc (0 : ℝ) 1,
          HasDerivAt Vc
            (fderiv ℝ (genericDoubled (genericDoubled (doubledField g gi))) (W0 τ) (Vc τ)) τ) := by
    intro δ
    obtain ⟨Vc, hVc0, hVcode, _⟩ := linODE_exists_narrowpad_continuousOn Aoct hAoct (seedCLM δ)
    refine ⟨Vc, hVc0, fun τ hτ => ?_⟩
    have := hVcode τ hτ; rwa [hAoctdef] at this
  set Vf : Point n → ℝ → St8 n := fun δ => Classical.choose (varkey δ) with hVfdef
  -- The perturbed octuple family `W δ = (((tube (v+δ), Jf δ), Uf δ), Tf δ)`.
  set W : Point n → ℝ → St8 n :=
    fun δ t => (((uniformFlowTube g gi hC hK q (v + δ) t, Jf δ t), Uf δ t), Tf δ t) with hWdef
  have ht1 : (1 : ℝ) ∈ Set.Icc (0 : ℝ) 1 := Set.right_mem_Icc.mpr zero_le_one
  -- Engine regularity inputs (from `UniformFlowOctupleField`).
  have hΦcd : ContDiff ℝ (⊤ : WithTop ℕ∞) (genericDoubled (genericDoubled (doubledField g gi))) :=
    contDiff_octupledField g gi hC
  have hdiff : ∀ x ∈ S, DifferentiableAt ℝ (genericDoubled (genericDoubled (doubledField g gi))) x :=
  ...
  have hdiff2 : ∀ x ∈ S, DifferentiableAt ℝ
      (fderiv ℝ (genericDoubled (genericDoubled (doubledField g gi)))) x :=
    fun x _ => (hΦcd'.differentiable (by simp)).differentiableAt
  ...
  have hLip : LipschitzOnWith K₀ (genericDoubled (genericDoubled (doubledField g gi))) S :=
  ...
  have hWode : ∀ δ : Point n, ‖δ‖ ≤ σ → ∀ τ ∈ Set.Icc (0 : ℝ) 1,
      HasDerivAt (W δ)
        (genericDoubled (genericDoubled (doubledField g gi)) (W δ τ)) τ := by
    intro δ hδ τ hτ
    have hadm : ‖v + δ‖ ≤ ρ := hle δ hδ
    obtain ⟨_, hJfode, _, _, _, hUfode, _, _, _, hTfode, _, _⟩ := hspec δ hadm
    ...
    exact genericDoubled_prod_hasDerivAt (genericDoubled (doubledField g gi)) hQode (hTfode τ hτ)
  have hVode : ∀ δ : Point n, ∀ τ ∈ Set.Icc (0 : ℝ) 1,
      HasDerivAt (Vf δ)
        (fderiv ℝ (genericDoubled (genericDoubled (doubledField g gi))) (W0 τ) (Vf δ τ)) τ :=
  ...
  have hmem : ∀ δ : Point n, ‖δ‖ ≤ σ → ∀ τ ∈ Set.Icc (0 : ℝ) 1, W δ τ ∈ S := by
    intro δ hδ τ hτ
    have hadm : ‖v + δ‖ ≤ ρ := hle δ hδ
    obtain ⟨_, _, hJfbnd, _, _, _, hUfbnd, _, _, _, hTfbnd, _⟩ := hspec δ hadm
  ...
  have hKb : ∀ τ ∈ Set.Icc (0 : ℝ) 1,
      ‖fderiv ℝ (genericDoubled (genericDoubled (doubledField g gi))) (W0 τ)‖ ≤ Kf := by
    intro τ hτ
  ...
  have hIC : ∀ δ : Point n, ‖δ‖ ≤ σ → W δ 0 - W 0 0 = seedCLM δ := by
    intro δ hδ
    have hadm : ‖v + δ‖ ≤ ρ := hle δ hδ
    obtain ⟨hJfδ0, _, _, _, hUfδ0, _, _, _, hTfδ0, _, _, _⟩ := hspec δ hadm
    obtain ⟨hJf00, _, _, _, hUf00, _, _, _, hTf00, _, _, _⟩ := hspec 0 hadm0
    ...
    simp only [hWdef]
    rw [h1, h2, hJfδ0, hJf00, hUfδ0, hUf00, hTfδ0, hTf00, hUseeddef, hTseeddef, hseed_eq δ]
    simp only [Prod.mk_sub_mk, sub_self, add_zero, add_sub_cancel_left, Prod.mk_zero_zero]
  -- Feed the abstract first-jet engine with `Φ := octupledField`.
  obtain ⟨L, _hLeq, hFD⟩ :=
  ...
  refine ⟨Jf, Uf, Tf, ?_, L, ?_⟩
  · intro δ hδ
    obtain ⟨hJfδ0, hJfode, _, _, hUfδ0, hUfode, _, _, hTfδ0, hTfode, _, _⟩ := hspec δ (hle δ hδ)
    exact ⟨hJfδ0, hJfode, hUfδ0, hUfode, hTfδ0, hTfode⟩
  · have hfe : (fun δ => W δ 1)
        = (fun δ => (((((uniformFlowTube g gi hC hK q (v + δ) 1, Jf δ 1), Uf δ 1), Tf δ 1)) : St8 n)) := by
      funext δ; rw [hWdef]
    rw [hfe] at hFD
    exact hFD

/-- **The projected deep scalar-slot component.**  Projecting `.2` (the `Tf` factor) then `.2.2.1` of
    the octuple-flow endpoint derivative, the map `δ ↦ (Tf δ 1).2.2.1` is Fréchet-differentiable at
    `0`.  This is the object the fourth-jet value-identification (CARRIED) rewrites to the third jet.
    NO `expRho`. -/
theorem uniformFlow_octupleEndpoint_component_hasFDerivAt (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
    {K : Set (Point n)} (hK : IsCompact K) (q : Point n) (hq : q ∈ K) (v : Point n)
    (hv : ‖v‖ < uniformFlowRadius g gi hC hK) (a b c : Point n) :
    ∃ Jf : Point n → ℝ → Point n × Point n,
    ...
    ∃ Tf : Point n → ℝ → St4 n,
      ...
      ∃ L₄ : Point n →L[ℝ] Point n,
        HasFDerivAt (fun δ => (Tf δ 1).2.2.1) L₄ 0 := by
  obtain ⟨Jf, Uf, Tf, hprops, L, hFD⟩ :=
    uniformFlow_octupleEndpoint_baseVelocity_hasFDerivAt g gi hC hK q hq v hv a b c
  refine ⟨Jf, Uf, Tf, hprops, ?_⟩
  ...
  have hfe : (⇑proj ∘ fun δ =>
      (((((uniformFlowTube g gi hC hK q (v + δ) 1, Jf δ 1), Uf δ 1), Tf δ 1)) : St8 n))
      = (fun δ => (Tf δ 1).2.2.1) := by
    funext δ
    simp [hprojdef]
  rw [hfe] at hc
  exact hc
