...
# The Williamson normal form — W1–W2: the decomposition structure, symplectic algebra, carried Youla
...
**Honest scope.**  W1–W2 are scaffolding + one carried hypothesis.  The load-bearing reduction
(`williamson_of_youla`) is the *later* increment W3; here Williamson is *not yet* proved, and even
when it is it will be proved **conditional on Youla**, not unconditionally.  Nothing here unlocks
the area-law `S ∝ A` scaling (the entropy machinery is area/volume-blind).  Axiom-free.
-/
