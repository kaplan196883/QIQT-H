...
/-! ### (C) — continuity of the second derivative `v ↦ fderiv (fderiv exp_p) v`

The final step of Rung 2.  `fderiv exp_p v = π∘(Φ_v 1)∘ι` and `fderiv (fderiv exp_p) v = expJetD2 …`
(both from `expMap_fderiv_hasFDerivAt`); `expJetD2 … v Φ_v k h = π(Q^{hk}_v(1))`.  So continuity of
the second derivative reduces to the **`v`-two-point Lipschitz bound of `Q^{hk}_v(1)`**:
`‖Q^{hk}_v(1) − Q^{hk}_w(1)‖ ≤ C·‖v−w‖·‖h‖·‖k‖`.

Route (a parameter-Grönwall, mirroring `expFund_two_pt_diff_Icc`): the difference
`D(t) = Q^{hk}_v(t) − Q^{hk}_w(t)` solves `D(0)=0`, `D'(t) = DF(Y_v t)(D t) + ρ(t)` with residual
`ρ(t) = [DF(Y_v t) − DF(Y_w t)](Q^{hk}_w t) + (Θ^{hk}_v(t) − Θ^{hk}_w(t))`, and
`‖ρ(t)‖ ≤ C·‖v−w‖·‖h‖·‖k‖` on `[0,1]`; then `gronwall_vec_residual` gives the endpoint bound. -/

/-- **All-`t` value bound for the second-variation solution `Q^{hk}`.**  The `Set.Icc`-uniform mirror
    of `expJet2Fund_value_bound` (which concludes only at `t = 1`): under the same Jacobi/`D²F`/`Φ`
    bounds, `‖Q t‖ ≤ (Kstar₂·Cphi²·e^{Kstar})·‖h‖·‖k‖` for every `t ∈ [0,1]`.  Same Grönwall, using
    the `Set.Icc` residual form `gronwall_vec_residual_Icc`.  Supplies the `‖Q^{hk}_w t‖` control the
    (C) parameter-residual bound consumes on the whole interval. -/
theorem expJet2Fund_value_bound_Icc (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y)) (p v : Point n)
    (Φ : ℝ → ((Point n × Point n) →L[ℝ] (Point n × Point n))) (h k : Point n)
    (Kstar Kstar₂ Cphi : ℝ)
    (hKstar0 : 0 ≤ Kstar) (hKstar₂0 : 0 ≤ Kstar₂) (hCphi0 : 0 ≤ Cphi)
    ...
    (hderiv : ∀ t ∈ Set.Icc (0 : ℝ) 1, HasDerivWithinAt Q
      ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)) (Q t)
        + expJet2Rhs g gi hC p v Φ h k t) (Set.Icc (0 : ℝ) 1) t) :
    ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖Q t‖ ≤ (Kstar₂ * Cphi ^ 2 * Real.exp Kstar) * ‖h‖ * ‖k‖ := by
  have hρ0 : (0 : ℝ) ≤ Kstar₂ * (Cphi * ‖h‖) * (Cphi * ‖k‖) :=
    mul_nonneg (mul_nonneg hKstar₂0 (mul_nonneg hCphi0 (norm_nonneg _)))
      (mul_nonneg hCphi0 (norm_nonneg _))
  have hΘbd : ∀ t ∈ Set.Icc (0 : ℝ) 1,
    ...
    fun t ht =>
      expJet2Rhs_norm_le g gi hC p v Φ h k Kstar₂ Cphi hKstar₂0 hCphi0 hKstar₂ hCphi t ht
  have hgron := gronwall_vec_residual_Icc Q (fun t => expJet2Rhs g gi hC p v Φ h k t)
    (fun t => fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t))
    Kstar (Kstar₂ * (Cphi * ‖h‖) * (Cphi * ‖k‖)) hKstar0 hρ0 hQ0 hderiv hKstar hΘbd
  intro t ht
  calc ‖Q t‖ ≤ (Kstar₂ * (Cphi * ‖h‖) * (Cphi * ‖k‖)) * Real.exp Kstar := hgron t ht
    _ = (Kstar₂ * Cphi ^ 2 * Real.exp Kstar) * ‖h‖ * ‖k‖ := by ring

/-- **Trilinear-difference telescope estimate.**  For two continuous bilinear maps `A, A'` and
    argument pairs `(a,b), (a',b')`,
    `‖A a b − A' a' b'‖ ≤ ‖A−A'‖‖a‖‖b‖ + ‖A'‖‖a−a'‖‖b‖ + ‖A'‖‖a'‖‖b−b'‖`.
    The standard three-term telescoping `A a b − A' a' b' = (A−A')ab + A'(a−a')b + A'a'(b−b')`,
    each term bounded by the bilinear operator-norm inequality.  Used to bound the two-point
    difference `Θ^{hk}_v − Θ^{hk}_w = D²F(Y_v)(Φv·)(Φv·) − D²F(Y_w)(Φw·)(Φw·)`. -/
theorem clm_bilinear_diff_norm_le {E F G : Type*}
    [NormedAddCommGroup E] [NormedSpace ℝ E]
    [NormedAddCommGroup F] [NormedSpace ℝ F]
    [NormedAddCommGroup G] [NormedSpace ℝ G]
    (A A' : E →L[ℝ] F →L[ℝ] G) (a a' : E) (b b' : F) :
    ‖A a b - A' a' b'‖
      ≤ ‖A - A'‖ * ‖a‖ * ‖b‖ + ‖A'‖ * ‖a - a'‖ * ‖b‖ + ‖A'‖ * ‖a'‖ * ‖b - b'‖ := by
  have hid : A a b - A' a' b'
      = ((A - A') a) b + (A' (a - a')) b + A' a' (b - b') := by
    simp only [ContinuousLinearMap.sub_apply, map_sub]
    abel
  have ht1 : ‖((A - A') a) b‖ ≤ ‖A - A'‖ * ‖a‖ * ‖b‖ :=
    ((A - A') a).le_opNorm b |>.trans
      (mul_le_mul_of_nonneg_right ((A - A').le_opNorm a) (norm_nonneg _))
  have ht2 : ‖(A' (a - a')) b‖ ≤ ‖A'‖ * ‖a - a'‖ * ‖b‖ :=
    (A' (a - a')).le_opNorm b |>.trans
      (mul_le_mul_of_nonneg_right (A'.le_opNorm (a - a')) (norm_nonneg _))
  have ht3 : ‖A' a' (b - b')‖ ≤ ‖A'‖ * ‖a'‖ * ‖b - b'‖ :=
    (A' a').le_opNorm (b - b') |>.trans
      (mul_le_mul_of_nonneg_right (A'.le_opNorm a') (norm_nonneg _))
  rw [hid]
  refine (norm_add_le _ _).trans ?_
  refine (add_le_add (norm_add_le _ _) le_rfl).trans ?_
  exact add_le_add (add_le_add ht1 ht2) ht3

/-- **The (C) parameter-difference ODE.**  For two second-variation solutions `Qv` (at `v`) and `Qw`
    (at `w`) of the Jet₂ IVP `Q' = DF(Y_·)(Q) + Θ^{hk}_·`, the difference `D = Qv − Qw` solves
    `D'(t) = DF(Y_v t)(D t) + ρ(t)` with residual
    `ρ(t) = [DF(Y_v t) − DF(Y_w t)](Qw t) + (Θ^{hk}_v(t) − Θ^{hk}_w(t))`, `D(0) = 0`.
    Pure linearity rearrangement of `Qv' − Qw'`. -/
theorem expJet2_v_residual_hasDerivWithinAt (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
    (p v w : Point n)
    (Φv Φw : ℝ → ((Point n × Point n) →L[ℝ] (Point n × Point n)))
    ...
    (t : ℝ) (ht : t ∈ Set.Icc (0 : ℝ) 1) :
    HasDerivWithinAt (fun s => Qv s - Qw s)
      ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)) (Qv t - Qw t)
        + ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
             - fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)) (Qw t)
           + (expJet2Rhs g gi hC p v Φv h k t - expJet2Rhs g gi hC p w Φw h k t)))
      (Set.Icc (0 : ℝ) 1) t := by
  have hcomb := (hQvd t ht).sub (hQwd t ht)
  have heq :
      (fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)) (Qv t - Qw t)
        + ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
             - fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)) (Qw t)
           + (expJet2Rhs g gi hC p v Φv h k t - expJet2Rhs g gi hC p w Φw h k t))
      = ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)) (Qv t)
          + expJet2Rhs g gi hC p v Φv h k t)
        - ((fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)) (Qw t)
          + expJet2Rhs g gi hC p w Φw h k t) := by
    simp only [map_sub, ContinuousLinearMap.sub_apply]
    abel
  rw [heq]
  exact hcomb

/-- **The (C) residual `ρ(t)` two-point bound.**  Given the eight uniform `[0,1]` control bounds
    `B1..B8 ≥ 0` — `B1 ≥ ‖DF(Y_v)−DF(Y_w)‖`, `B2 ≥ ‖Qw‖`, `B3 ≥ ‖D²F(Y_v)−D²F(Y_w)‖`,
    `B4 ≥ ‖D²F(Y_w)‖`, `B5 ≥ ‖Φv(ιh)‖,‖Φw(ιh)‖`, `B6 ≥ ‖Φv(ιk)‖`, `B7 ≥ ‖(Φv−Φw)(ιh)‖`,
    `B8 ≥ ‖(Φv−Φw)(ιk)‖` — the residual
    `ρ(t) = [DF(Y_v)−DF(Y_w)](Qw) + (Θ^{hk}_v − Θ^{hk}_w)` obeys
    `‖ρ(t)‖ ≤ B1·B2 + (B3·B5·B6 + B4·B7·B6 + B4·B5·B8)`.  Term (a): the bilinear opNorm; term (b):
    `clm_bilinear_diff_norm_le` on `D²F(Y_v)(Φv·)(Φv·) − D²F(Y_w)(Φw·)(Φw·)`. -/
theorem expJet2_v_residual_norm_le (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
    (p v w : Point n)
    (Φv Φw : ℝ → ((Point n × Point n) →L[ℝ] (Point n × Point n)))
    (Qw : ℝ → (Point n × Point n)) (h k : Point n)
    (B1 B2 B3 B4 B5 B6 B7 B8 : ℝ)
    (hB1 : 0 ≤ B1) (_hB2 : 0 ≤ B2) (hB3 : 0 ≤ B3) (hB4 : 0 ≤ B4)
    (hB5 : 0 ≤ B5) (_hB6 : 0 ≤ B6) (hB7 : 0 ≤ B7) (_hB8 : 0 ≤ B8)
    (hDFdiff : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
          - fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)‖ ≤ B1)
    (hQwbd : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qw t‖ ≤ B2)
    (hD2diff : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖fderiv ℝ (fderiv ℝ (geodesicField g gi)) (expTube g gi hC p v t)
          - fderiv ℝ (fderiv ℝ (geodesicField g gi)) (expTube g gi hC p w t)‖ ≤ B3)
    (hAw : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖fderiv ℝ (fderiv ℝ (geodesicField g gi)) (expTube g gi hC p w t)‖ ≤ B4)
    (hav : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Φv t (expJetIota h)‖ ≤ B5)
    (hbv : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Φv t (expJetIota k)‖ ≤ B6)
    (haw : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Φw t (expJetIota h)‖ ≤ B5)
    (hadiff : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖Φv t (expJetIota h) - Φw t (expJetIota h)‖ ≤ B7)
    (hbdiff : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖Φv t (expJetIota k) - Φw t (expJetIota k)‖ ≤ B8)
    (t : ℝ) (ht : t ∈ Set.Icc (0 : ℝ) 1) :
    ‖(fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
          - fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)) (Qw t)
        + (expJet2Rhs g gi hC p v Φv h k t - expJet2Rhs g gi hC p w Φw h k t)‖
      ≤ B1 * B2 + (B3 * B5 * B6 + B4 * B7 * B6 + B4 * B5 * B8) := by
  -- term (a): the linear opNorm bound.
  set DFd := fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
      - fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t) with hDFd
  have ha : ‖DFd (Qw t)‖ ≤ B1 * B2 :=
    (DFd.le_opNorm (Qw t)).trans
      (mul_le_mul (hDFdiff t ht) (hQwbd t ht) (norm_nonneg _) hB1)
  -- term (b): the trilinear telescope on `Θ_v − Θ_w`.
  have hb : ‖expJet2Rhs g gi hC p v Φv h k t - expJet2Rhs g gi hC p w Φw h k t‖
      ≤ B3 * B5 * B6 + B4 * B7 * B6 + B4 * B5 * B8 := by
    rw [expJet2Rhs_apply, expJet2Rhs_apply]
    set Av := fderiv ℝ (fderiv ℝ (geodesicField g gi)) (expTube g gi hC p v t) with hAv
    set Aw := fderiv ℝ (fderiv ℝ (geodesicField g gi)) (expTube g gi hC p w t) with hAwe
    refine (clm_bilinear_diff_norm_le Av Aw (Φv t (expJetIota h)) (Φw t (expJetIota h))
      (Φv t (expJetIota k)) (Φw t (expJetIota k))).trans ?_
    refine add_le_add (add_le_add ?_ ?_) ?_
    · -- `‖Av − Aw‖·‖Φv ιh‖·‖Φv ιk‖ ≤ B3·B5·B6`
      exact mul_le_mul (mul_le_mul (hD2diff t ht) (hav t ht) (norm_nonneg _) hB3)
        (hbv t ht) (norm_nonneg _) (mul_nonneg hB3 hB5)
    · -- `‖Aw‖·‖Φv ιh − Φw ιh‖·‖Φv ιk‖ ≤ B4·B7·B6`
      exact mul_le_mul (mul_le_mul (hAw t ht) (hadiff t ht) (norm_nonneg _) hB4)
        (hbv t ht) (norm_nonneg _) (mul_nonneg hB4 hB7)
    · -- `‖Aw‖·‖Φw ιh‖·‖Φv ιk − Φw ιk‖ ≤ B4·B5·B8`
      exact mul_le_mul (mul_le_mul (hAw t ht) (haw t ht) (norm_nonneg _) hB4)
        (hbdiff t ht) (norm_nonneg _) (mul_nonneg hB4 hB5)
  exact (norm_add_le _ _).trans (add_le_add ha hb)

/-- **The assembled (C) two-point Lipschitz constant** for `v ↦ Q^{hk}_v(1)`.  Collecting the four
    residual sub-terms (see `expJet2_v_residual_norm_le`) and the outer Grönwall factor `e^{Kstar}`,
    the coefficient of `‖v−w‖·‖h‖·‖k‖` is `e^{Kf}·e^{3 Kstar}·(Ld2f + 3·Ldf·Kstar₂·e^{Kstar})`. -/
noncomputable def expJet2VtpConst (Kf Ldf Ld2f Kstar Kstar₂ : ℝ) : ℝ :=
  Real.exp Kf * (Real.exp Kstar) ^ 3 * (Ld2f + 3 * Ldf * Kstar₂ * Real.exp Kstar)

theorem expJet2VtpConst_nonneg (Kf Ldf Ld2f Kstar Kstar₂ : ℝ)
    (hLdf : 0 ≤ Ldf) (hLd2f : 0 ≤ Ld2f) (hKstar₂ : 0 ≤ Kstar₂) :
    0 ≤ expJet2VtpConst Kf Ldf Ld2f Kstar Kstar₂ := by
  unfold expJet2VtpConst
  have : 0 ≤ Ld2f + 3 * Ldf * Kstar₂ * Real.exp Kstar := by positivity
  positivity

set_option maxHeartbeats 3200000 in
/-- **The (C) core — the `v`-two-point Lipschitz bound of the second-variation value `Q^{hk}_v(1)`.**
    ...
    `‖Qv 1 − Qw 1‖ ≤ expJet2VtpConst·‖v−w‖·‖h‖·‖k‖`.  The analytic heart of (C): the difference
    `D = Qv − Qw` solves `D' = DF(Y_v)(D) + ρ` (`expJet2_v_residual_hasDerivWithinAt`), the residual is
    `‖ρ‖ ≤ B1B2 + …` (`expJet2_v_residual_norm_le`) with the eight uniform bounds supplied by the tube
    two-point separation (`geodesic_twopoint_gronwall`), the `DF`/`D²F` Lipschitz constants, the `Φ`
    two-point Lipschitz (`expFund_two_pt_diff_Icc`), the `Φ`-norm bound (`expJetFund_norm_le_exp`) and
    the `Q` value bound (`expJet2Fund_value_bound_Icc`); then `gronwall_vec_residual`. -/
theorem expJet2Val_two_pt_diff (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
  ...
  -- the difference Grönwall.
  have hgron := gronwall_vec_residual (fun s => Qv s - Qw s)
    (fun t => (fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t)
        - fderiv ℝ (geodesicField g gi) (expTube g gi hC p w t)) (Qw t)
      + (expJet2Rhs g gi hC p v Φv h k t - expJet2Rhs g gi hC p w Φw h k t))
    (fun t => fderiv ℝ (geodesicField g gi) (expTube g gi hC p v t))
  ...
  -- read off the endpoint and factor the constant.
  have hfinal : ‖Qv 1 - Qw 1‖ ≤ ρval * Real.exp Kstar := hgron
  refine hfinal.trans (le_of_eq ?_)
  rw [hρvaldef, hB1def, hB2def, hB3def, hB5def, hB6def, hB7def, hB8def, expJet2VtpConst]
  ring

set_option maxHeartbeats 1600000 in
/-- **The `expJetD2` operator-norm two-point bound.**  The `v`-two-point value bound
    (`expJet2Val_two_pt_diff`), projected by `π` (`‖π‖ ≤ 1`) and lifted through the two continuous-
    linear-map layers by `ContinuousLinearMap.opNorm_le_bound` (twice), gives
    `‖expJetD2_v − expJetD2_w‖ ≤ expJet2VtpConst·‖v−w‖`.  The Lipschitz datum for the second
    derivative `v ↦ fderiv (fderiv exp_p) v`. -/
theorem expJetD2_two_pt_diff (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y))
    (p v w : Point n) (hv : ‖v‖ ≤ expRho g gi hC p) (hw : ‖w‖ ≤ expRho g gi hC p)
    (Kf Ldf Ld2f : NNReal) (Kstar Kstar₂ : ℝ) (hKstar0 : 0 ≤ Kstar) (hKstar₂0 : 0 ≤ Kstar₂)
    (hLipF : LipschitzOnWith Kf (geodesicField g gi)
    ...
    ‖expJetD2 g gi hC p v Φv hv hΦvcont - expJetD2 g gi hC p w Φw hw hΦwcont‖
      ≤ expJet2VtpConst (Kf : ℝ) (Ldf : ℝ) (Ld2f : ℝ) Kstar Kstar₂ * ‖v - w‖ := by
  set C : ℝ := expJet2VtpConst (Kf : ℝ) (Ldf : ℝ) (Ld2f : ℝ) Kstar Kstar₂ with hCdef
  have hC0 : 0 ≤ C := expJet2VtpConst_nonneg _ _ _ _ _ (Ldf.2) (Ld2f.2) hKstar₂0
  -- the per-`(h,k)` value two-point bound, via the chosen `Q` witnesses.
  have hval : ∀ h k : Point n,
      ‖expJetPi (expJet2Val g gi hC p v Φv hv hΦvcont h k)
          - expJetPi (expJet2Val g gi hC p w Φw hw hΦwcont h k)‖
        ≤ C * ‖v - w‖ * ‖h‖ * ‖k‖ := by
    intro h k
    obtain ⟨hQv0, -, -, hQvderiv⟩ := (expJet2Fund g gi hC p v Φv hv hΦvcont h k).choose_spec
    obtain ⟨hQw0, -, -, hQwderiv⟩ := (expJet2Fund g gi hC p w Φw hw hΦwcont h k).choose_spec
    have hb := expJet2Val_two_pt_diff g gi hC p v w hv hw Kf Ldf Ld2f Kstar Kstar₂
      hKstar0 hKstar₂0 hLipF hLipDF hLipD2F hKstar hKstar₂ Φv Φw hΦv0 hΦw0 hΦvd hΦwd
      (expJet2Fund g gi hC p v Φv hv hΦvcont h k).choose
      (expJet2Fund g gi hC p w Φw hw hΦwcont h k).choose h k hQv0 hQw0 hQvderiv hQwderiv
    have hπ : expJetPi (expJet2Val g gi hC p v Φv hv hΦvcont h k)
          - expJetPi (expJet2Val g gi hC p w Φw hw hΦwcont h k)
        = expJetPi ((expJet2Fund g gi hC p v Φv hv hΦvcont h k).choose 1
            - (expJet2Fund g gi hC p w Φw hw hΦwcont h k).choose 1) := by
      rw [map_sub]; rfl
    rw [hπ]
    refine ((expJetPi (n := n)).le_opNorm _).trans ?_
    refine (mul_le_mul expJetPi_opNorm_le hb (norm_nonneg _) (by norm_num)).trans ?_
    exact le_of_eq (by ring)
  -- lift to the operator norm through the two CLM layers.
  ...
  rw [ContinuousLinearMap.sub_apply, ContinuousLinearMap.sub_apply, expJetD2_apply,
    expJetD2_apply]
  calc ‖expJetPi (expJet2Val g gi hC p v Φv hv hΦvcont h k)
          - expJetPi (expJet2Val g gi hC p w Φw hw hΦwcont h k)‖
      ≤ C * ‖v - w‖ * ‖h‖ * ‖k‖ := hval h k
    _ = C * ‖v - w‖ * ‖k‖ * ‖h‖ := by ring

set_option maxHeartbeats 1600000 in
/-- **(D) — the Rung-2 capstone: `exp_p` is `ContDiff²` near `0`, UNCONDITIONALLY.**
    The second derivative `v ↦ fderiv (fderiv exp_p) v` is Lipschitz on the exp-ball
    (`expJetD2_two_pt_diff` at once-obtained geometric constants), hence continuous; the first
    derivative is differentiable (`expMap_fderiv_hasFDerivAt`); so `v ↦ fderiv exp_p v` is
    `ContDiff¹` (`contDiffOn_succ_of_fderivWithin`, `fderivWithin = fderiv` on the open ball), which
    the proven reduction `expMap_contDiffOn_two_of_fderiv_contDiffOn_one` upgrades to `ContDiff²`.
    No side hypothesis: the crux left open by the earlier reduction is now discharged. -/
theorem expMap_contDiffOn_two (g gi : Point n → Fin n → Fin n → ℝ)
    (hC : ∀ a b c, ContDiff ℝ (⊤ : WithTop ℕ∞) (fun y => christoffel g gi a b c y)) (p : Point n) :
    ContDiffOn ℝ 2 (expMap g gi hC p) (Metric.ball (0 : Point n) (expRho g gi hC p)) := by
  refine expMap_contDiffOn_two_of_fderiv_contDiffOn_one g gi hC p ?_
  set s : Set (Point n) := Metric.ball (0 : Point n) (expRho g gi hC p) with hsdef
  ...
  obtain ⟨Kstar₂, hKstar₂0, hKstar₂⟩ := expJet_fderiv2_tube_bddAbove_unif g gi hC p
  set Cc : ℝ := expJet2VtpConst (Kf : ℝ) (Ldf : ℝ) (Ld2f : ℝ) Kstar Kstar₂ with hCcdef
  have hCc0 : 0 ≤ Cc := expJet2VtpConst_nonneg _ _ _ _ _ Ldf.2 Ld2f.2 hKstar₂0
  have hmemlt : ∀ v ∈ s, ‖v‖ < expRho g gi hC p := by
    intro v hv; rw [hsdef, Metric.mem_ball, dist_zero_right] at hv; exact hv
  -- the first derivative is differentiable on the ball.
  have hdiffF : DifferentiableOn ℝ (fun u => fderiv ℝ (expMap g gi hC p) u) s := by
    intro v hv
    ...
    exact (expMap_fderiv_hasFDerivAt g gi hC p v Φv (hmemlt v hv) hΦv0 hΦvcont hΦvd
      hFDv.fderiv).differentiableAt.differentiableWithinAt
  -- the second derivative is Lipschitz (⟹ continuous) on the ball.
  have hcont : ContinuousOn
      (fun v => fderiv ℝ (fun u => fderiv ℝ (expMap g gi hC p) u) v) s := by
    have hlip : LipschitzOnWith Cc.toNNReal
      ...
      have hev := (expMap_fderiv_hasFDerivAt g gi hC p v Φv (hmemlt v hv) hΦv0 hΦvcont hΦvd
        hFDv.fderiv).fderiv
      have hew := (expMap_fderiv_hasFDerivAt g gi hC p w Φw (hmemlt w hw) hΦw0 hΦwcont hΦwd
        hFDw.fderiv).fderiv
      rw [hev, hew]
      exact expJetD2_two_pt_diff g gi hC p v w (hmemlt v hv).le (hmemlt w hw).le
        Kf Ldf Ld2f Kstar Kstar₂ hKstar0 hKstar₂0 hLipF hLipDF hLipD2F hKstar hKstar₂
        Φv Φw hΦv0 hΦw0 hΦvd hΦwd hΦvcont hΦwcont
    exact hlip.continuousOn
  -- assemble `ContDiff¹` of the first derivative.
  have hopen : IsOpen s := by rw [hsdef]; exact Metric.isOpen_ball
  have hcont_fw : ContinuousOn
      (fun y => fderivWithin ℝ (fun u => fderiv ℝ (expMap g gi hC p) u) s y) s :=
    hcont.congr (fun y hy => fderivWithin_of_isOpen hopen hy)
  have hres := contDiffOn_succ_of_fderivWithin (n := 0) hdiffF (by simp)
