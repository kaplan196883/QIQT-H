...
/-- **The flow (Heisenberg) equation:** `d/dt Δ_{σ|ρ}^{it} = Δ_{σ|ρ}^{it} · (i·K)` for all `t` — the
    modular flow solves the one-parameter-group ODE generated by `i·K` (`K = log Δ`).  The Stone
    generator `hasDerivAt_relModFlow` is the `t = 0` case (`Δ^{i·0} = 1`). -/
theorem hasDerivAt_relModFlow' (hσ : σ.PosDef) (hρ : ρ.PosDef) (t₀ : ℝ) :
    HasDerivAt (fun t : ℝ => relModFlow hσ hρ t)
      (relModFlow hσ hρ t₀ * (Complex.I • relModGen hσ hρ)) t₀ := by
  have hfun : ∀ t : ℝ, relModFlow hσ hρ t
      = NormedSpace.exp (t • (Complex.I • relModGen hσ hρ)) := by
    intro t; rw [relModFlow]; congr 1; rw [← smul_assoc, Complex.real_smul, mul_comm]
