/-
  A1R6FromLabelled — J4-340: v3-export brick 5, THE FINAL FACADE.  `a1_R6_from_labelled` assembles the
  a₁ two-jet CONCLUSION at the literal constant-radius gate from {the base geometry/gauge raw inputs +
  the satisfiable census piles + the FOUR labelled inputs (`hGauss`/`hraw`/`hD2Hexpand`/`hPd2conv`)} via
  the banked v3 chain (bricks 1–4 + the four producers).  The campaign's final composition.

  ══════════════════════════════════════════════════════════════════════════════════════════════════
  ⚠⚠⚠ HONESTY FIREWALL — THIS IS **NOT** UNCONDITIONAL `a₁ = R/6`. ⚠⚠⚠
  `a1_R6_from_labelled` is a CONDITIONAL implication.  Its conclusion (the a₁ two-jet with the genuine
  Ricci scalar `∑ i, ricci g gi i i 0` in the `t`-coefficient) holds ONLY given its full antecedent
  census, which STILL contains the FOUR genuinely-labelled inputs, each the subject of a separate
  open derivation campaign, none discharged here:
    • `hGauss`     — the Riemann-normal-coordinate gauge `∑ j g x i j · x j =ᶠ x i` near `0` (feeds `htr`,
                     hence the `-(2/3)·Ric` source coefficient);
    • `hraw`       — the width-1 residual `GlobalGatedRawBound` (feeds the `Da`-limit residual envelope);
    • `hD2Hexpand` — the derivative-layer Leibniz–Gaussian identity carried inside every `dataAmp`
                     `AmplitudeDerivativeData` bundle (feeds the `√ε` sliver amplitudes);
    • `hPd2conv`   — the atomic frozen→full second-`x`-derivative interchange limit (feeds `hLapFull`).
  Everything else is banked-shaped SATISFIABLE census; NONE of the hypotheses is the conclusion; there
  is no `sorry`, no `:= True`, no new axiom.  This theorem is the CONDITIONAL MILESTONE — "the a₁ two-jet
  FROM the four labelled inputs + the satisfiable data" — and is NOT a proof of `a₁ = R/6`.

  ══════════════════════════════════════════════════════════════════════════════════════════════════
  ## THE BANKED CHAIN (one heavy application per intermediate).
  Internally (typed `have`s):
    • `htr`   := `A1R6SlotAdapters.htr_adapter` from {gauge/smoothness + `hGauss`}   (brick 2);
    • `hDa`   := `GlobalRawBoundFacade.hDaLimLU_from_labelled` at `S := constGate g gi hChr hK c`
                 (J4-337) — the whole ~60-binder `Da`-limit labelled census, incl. `hraw`/`hPd2conv`
                 and the `dataAmp` amplitude bundle (whose hard field is `hD2Hexpand`);
    • `core`  := `HDuhamelExportRethread.truncatedDuhamelCore_AT_GATE_FULL` at the same gate (J4-311) —
                 its ~90-binder Duhamel/`Da`-concrete census;
    • `hbdry` := `HDerivConvComposition.hbdryLU_CONCRETE` (J4-310) from the frozen+moving lists;
    • `slots` := `A1R6SlotAdapters.a1_R6_slots_AT_GATE` (brick 4) from `core`, `hDa`, `hbdry`, the F2/
                 `hDConv` census, and the `hCConv` sliver census;
    • `exact  A1R6CoreAtGate.wide_a1_R6_core_AT_CONSTRADIUS` (brick 1) at `Ric := fun cc d => ricci
                 g gi cc d 0`, fed `htr`, the package facts, `hS1`, and `slots.{hDuhamel,hDConv,hCConv}`.

  ## STRUCTURE DECISION (Sol consult #11).  The ~60 + ~90 binder piles are too heterogeneous for fast
  structures, so the facade is FLAT with comment-delimited groups (kernel safety over readability; brick
  5 readability matters least).  Several regularity facts recur across producers at the SAME gate/source
  and are SHARED as single binders — noted where they occur — so the surviving census is not
  double-counted:
    • the W2 differentiation-under-∫ family (`V…hdiff`) is shared between `hDaLimLU_from_labelled` and
      `truncatedDuhamelCore_AT_GATE_FULL`;
    • the F2 / `hDConv` regularity family (`A₀…hCross`) is shared between `…_FULL` and `a1_R6_slots_AT_GATE`;
    • the frozen/moving envelope list (`ρ…hUsub`) is shared between `…_FULL` and `hbdryLU_CONCRETE`;
    • the residual-window floor (`aa`/`haa`/`hau`) doubles as the `…_FULL` floor (`aT`/`haT`/`hUlb`);
    • `hUT` doubles as `hDaLimLU_from_labelled`'s `hUTle`.

  ## SATISFIABILITY CERTIFICATE for the package facts (`hpkgBound`/`hmemS0`/`hopenS0`/`hS1`).  These are
  carried as EXPLICIT hypotheses at fixed `(a,b,c,C,δ₀)` rather than obtained from an existential (so the
  census piles all stay at the ONE shared syntactic gate and the composition is `∃`-free).  Their JOINT
  satisfiability is certified by `ConstRadiusGateExport.constRadius_package_and_S1`, which produces
  exactly these four fields for the literal gate — with `hS1` guaranteed under the honest smallness
  `c < δ₀` (the chart-vs-jet reach condition; `δ₀` is the package's own reach radius).  The `hpkgBound`
  shape matches because `vanVleckGatedWitness g gi hChr hK (constGate g gi hChr hK c) a b` unfolds
  (defeq) to `gatedKernel K (fun z => uniformFlowExp … z '' ball 0 c) (globalCutoffParametrixWitnessN 1 …)`,
  the package's own kernel.
-/
import Mathlib
...
namespace QIQTH.A1R6FromLabelled

variable {n : ℕ}

set_option maxHeartbeats 4000000

/-- **★★★★★ J4-340 — `a1_R6_from_labelled` — THE FINAL FACADE.**  The a₁ two-jet CONCLUSION at the
    literal constant-radius gate `constGate g gi hChr hK c` (with the genuine Ricci scalar
    `∑ i, ricci g gi i i 0` in the `O(t)` coefficient), derived from the base geometry/gauge inputs, the
    satisfiable banked census piles, and the FOUR labelled inputs `hGauss`/`hraw`/`hD2Hexpand` (carried
    inside `dataAmp`)/`hPd2conv`.  Assembled via the banked v3 chain: `htr_adapter` (brick 2) →
    `hDaLimLU_from_labelled` (J4-337) → `truncatedDuhamelCore_AT_GATE_FULL` (J4-311) → `hbdryLU_CONCRETE`
    (J4-310) → `a1_R6_slots_AT_GATE` (brick 4) → `wide_a1_R6_core_AT_CONSTRADIUS` (brick 1).  ⚠ CONDITIONAL
    — NOT a proof of `a₁ = R/6`; see the file header for the four surviving labelled inputs and the
    package satisfiability certificate. -/
theorem a1_R6_from_labelled
    ...
    (P : ℝ) (hP : 0 ≤ P)
    (hraw : GlobalGatedRawBound g gi (vanVleckGatedWitness g gi hChr hK (constGate g gi hChr hK c) a b) P)  -- ★ LABELLED
  ...
  have hDa :
      DaLimLUGoal g gi (vanVleckGatedWitness g gi hChr hK (constGate g gi hChr hK c) a b)
        (leviSeries (heatOp g gi (vanVleckGatedWitness g gi hChr hK (constGate g gi hChr hK c) a b))) U :=
    hDaLimLU_from_labelled g gi hChr hK (constGate g gi hChr hK c) a b T U hUopen hn
...
end QIQTH.A1R6FromLabelled

/-! ## Axiom check — the final facade is `std-3` (propext, Classical.choice, Quot.sound). -/
section AxiomChecks
open QIQTH.A1R6FromLabelled
#print axioms a1_R6_from_labelled
end AxiomChecks
