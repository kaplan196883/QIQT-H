...
/-!
# Jet₄ quadratic remainder bound, explicit directional factors — rung J4-5c

EXPLICIT-DIRECTIONAL-FACTOR variant of `expJet4_remainder_quadratic_bound`
(`ExpJet4Remainder.lean`).  Identical five-block cancellation, but the directional norms
`‖h‖·‖k‖·‖l‖` are threaded EXPLICITLY through every bound, so the conclusion reads
`≤ C₀·‖h‖·‖k‖·‖l‖·‖m‖²`.  This mirrors how `expJet3_remainder_quadratic_bound'`
(`ExpMapContDiff3.lean:3144`) relates to the plain `expJet3_remainder_quadratic_bound`
(`:1605`): each carried INPUT hypothesis gains the directional norms of the directions it
differentiates, and the witness constant `C₀` is `h,k,l`-independent so downstream
`opNorm_le_bound` can extract the operator norm.

The `O(…‖m‖²)` conclusion is DERIVED by the real cancellation (a five-block telescope +
`abel` + per-term estimates).  Only strictly-lower-order INPUTS are carried; the conclusion
is never carried.
-/
...
/-- **Jet₄ quadratic remainder bound, explicit directional factors** (mirror of
    `expJet3_remainder_quadratic_bound'`, one Fréchet order up).  DERIVES the
    `O(‖h‖·‖k‖·‖l‖·‖m‖²)` bound on the exact `ρ(t)` fed to `expJet4_residual_bound`.
    Carried INPUTS gain the directional norms of the directions they differentiate; the
    witness constant is `h,k,l`-independent. -/
theorem expJet4_remainder_quadratic_bound' (g gi : Point n → Fin n → Fin n → ℝ)
    ...
    -- first→second variation residuals (direction m), explicit directional factors
    (hFPh : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖Φ' t (expJetIota h) - Φ t (expJetIota h) - Qhm t‖ ≤ Cd * ‖h‖ * ‖m‖ ^ 2)
    (hFPk : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖Φ' t (expJetIota k) - Φ t (expJetIota k) - Qkm t‖ ≤ Cd * ‖k‖ * ‖m‖ ^ 2)
    (hFPl : ∀ t ∈ Set.Icc (0 : ℝ) 1,
      ‖Φ' t (expJetIota l) - Φ t (expJetIota l) - Qlm t‖ ≤ Cd * ‖l‖ * ‖m‖ ^ 2)
    -- second→third variation residuals, explicit directional factors
    (hFQkl : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwkl t - Qvkl t - Qklm t‖ ≤ Cd * ‖k‖ * ‖l‖ * ‖m‖ ^ 2)
    (hFQhl : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwhl t - Qvhl t - Qhlm t‖ ≤ Cd * ‖h‖ * ‖l‖ * ‖m‖ ^ 2)
    (hFQhk : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwhk t - Qvhk t - Qhkm t‖ ≤ Cd * ‖h‖ * ‖k‖ * ‖m‖ ^ 2)
    -- solution Lipschitz bounds, explicit directional factors
    (hQlip : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qw t - Qv t‖ ≤ Ce * ‖h‖ * ‖k‖ * ‖l‖ * ‖m‖)
    (hQlipkl : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwkl t - Qvkl t‖ ≤ Ce * ‖k‖ * ‖l‖ * ‖m‖)
    (hQliphl : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwhl t - Qvhl t‖ ≤ Ce * ‖h‖ * ‖l‖ * ‖m‖)
    (hQliphk : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwhk t - Qvhk t‖ ≤ Ce * ‖h‖ * ‖k‖ * ‖m‖)
    -- value bounds on the abstract second-variation curves, explicit directional factors
    (hVkl : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qkl t‖ ≤ Cq2 * ‖k‖ * ‖l‖)
    (hVhl : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qhl t‖ ≤ Cq2 * ‖h‖ * ‖l‖)
    (hVhk : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qhk t‖ ≤ Cq2 * ‖h‖ * ‖k‖)
    (hVwkl : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwkl t‖ ≤ Cq2 * ‖k‖ * ‖l‖)
    (hVwhl : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwhl t‖ ≤ Cq2 * ‖h‖ * ‖l‖)
    (hVwhk : ∀ t ∈ Set.Icc (0 : ℝ) 1, ‖Qwhk t‖ ≤ Cq2 * ‖h‖ * ‖k‖)
    :
  ...
  -- ── clean (h,k,l-independent) block constants and finish ──
  set M3 : ℝ := (Kstar3 * eKs * eKs * eKs
