...
> **Formal verification.** The framework's deductive core — Theorems 3, 6, 7, Lemma 1, Donald's identity, no-signaling, the Bell/CHSH inequality with a rigorous singlet construction reaching $2\sqrt{2}$, seven structural audits (independence and sharp reference-weight reformulation of (H2); decoherence-vs-concentration; modular-local entropy vs CPW renormalized entropy; branch-summed cost vs Shannon; FQ-finite admissibility gives trivial dynamics; compression-locality leakage; $\mu$-selection is load-bearing for Born and support preservation $\neq$ Born equivariance), and three sub-theorems for the Canonical IC Measure Principle (typicality Mackey-Gleason, operational data insufficient, FQ-equivariance uniqueness) — is machine-verified in Lean 4 with Mathlib. A subsequent A1+A2+A4+A6 strengthening pass adds: locality of Alice's marginal proved as a theorem about equivariant measures (`MarginalLocality`); Goldstein-Struyve sub-lemma 1c upgraded from axiom to proved theorem under explicit permutation symmetry (`step1c_collapse_of_perm_symmetric`); Chebyshev frequency concentration on finite Bernoulli trials (`BornConcentration`); and a unified independence/minimality package showing each remaining Born sub-axiom is provably necessary by concrete finite countermodel (`BornMinimalityTable`). The `FQEquivarianceUniqueness` module was consolidated to eliminate 12 redundant abstract axioms; the headline theorem `canonical_ic_measure_principle` now depends on exactly two project-specific axioms. See `lean/mathlib/QIQTH/` in the project repository, and `lean/mathlib/QIQTH/AxiomAudit.lean` for the complete axiom audit.
