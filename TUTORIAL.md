...
> **Formal verification.** The framework's deductive core — Theorems 3, 6, 7, Lemma 1, Donald's identity, no-signaling, the Bell/CHSH inequality with a rigorous singlet construction reaching $2\sqrt{2}$, seven structural audits (independence and sharp reference-weight reformulation of (H2); decoherence-vs-concentration; modular-local entropy vs CPW renormalized entropy; branch-summed cost vs Shannon; FQ-finite admissibility gives trivial dynamics; compression-locality leakage; $\mu$-selection is load-bearing for Born and support preservation $\neq$ Born equivariance), and three sub-theorems for the Canonical IC Measure Principle (typicality Mackey-Gleason, operational data insufficient, FQ-equivariance uniqueness) — is machine-verified in Lean 4 with Mathlib. A subsequent A1+A2+A4+A6 strengthening pass adds: the marginal-locality step proved as a theorem holding for *any* measure (`MarginalLocality.pushforward_marginal_local`), so locality is no longer an independent Born-selection sub-axiom — modulo a named Hilbert→set-level bridge axiom into which the locality content is relocated, not eliminated; the permutation-symmetry collapse component of Goldstein-Struyve sub-lemma 1c proved under its necessary hypothesis (`step1c_collapse_of_perm_symmetric`), with the *full* Schur classification still a single named interface axiom; single-trial Chebyshev frequency concentration and the variance-addition (independence) lemma on finite Bernoulli trials (`BornConcentration`), with the $N$-trial scaling still resting on the LLN interface axiom; and a unified independence/minimality package showing each remaining Born sub-axiom is necessary by concrete finite countermodel (`BornMinimalityTable`). **Update 2026-06:** the entire development has since been driven to **budget 0 — axiom-free**. All interface axioms (the 33 continuum / operator-algebra ones, and the Born sub-axioms above) have been retired via typeclass-and-instance discharges; `lake build QIQTH.AxiomAudit` now reports raw axiom count **0**, no `sorry`, standard three only (`propext`, `Classical.choice`, `Quot.sound`), enforced by `scripts/axiom_budget_check.sh`. And **λ's law is now machine-checked** — kinematics (the Takesaki criterion fixing the pointer algebra), persistence (the decoherence map commutes with the modular flow for all $t$), the Type-independent Born rule, and the single-outcome selection event — at the finite, the one-particle continuum ($\Delta^{it}$), *and* the second-quantized free-field ($\Gamma(\Delta^{it})$) levels. See `lean/mathlib/QIQTH/` and `lean/mathlib/QIQTH/AxiomAudit.lean` for the complete audit.
...
Here $\chi_R(\omega)$ measures how complicated the regional state $\omega_R$ is compared to the sector reference state $\sigma_R$ (typically the vacuum), evaluated *intrinsically* on the regional algebra. It is finite, basis-independent, and well-defined even when the algebra is Type III (no density-matrix factorization needed).
...
So when we say "QIQT-H's constraint is like a gauge constraint," we mean only this: both restrict a larger *formal* space to a smaller set of *physically instantiable* contents. **That structural pattern is the entire analogy, and even there it is imperfect.** The mathematical machinery is completely different — and, importantly, the analogy does *not* extend to the dynamics. In gauge theory one also restricts the admissible Hamiltonians to gauge-preserving ones; QIQT-H does **not** restrict the Hamiltonians at all. The global Schrödinger / Heisenberg law remains exactly unitary and unconstrained; what is restricted is only which regional *content* a run actually realizes — a single record, supplied by the actuality selector $\lambda$, not by a capacity overflow (the "$\ge 2$ records exceed capacity" reading is a retired category error; capacity is the finite record *stage*, not the single-outcome rule).
...
An effectively noninterfering component of the wavefunction produced by decoherence. In QIQT-H a branch is *not* a separate world: decoherence makes branches stop interfering, but that does not make one of them *actual*. **Correction (2026):** an earlier version said a $\ge 2$-record content *exceeds the regional capacity* $Q_R$ and so is not instantiable — that is a **category error** (the holographic bound counts independent degrees of freedom, not a sum of redundant records, and ordinary record entropy is capped far below the bound). Capacity is the finite record *stage* (how many distinguishable records exist, $\le e^{Q_R}$), not a single-outcome rule. The resident regional content is single-record because the actuality selector $\lambda$ — a non-dynamical, atemporal fact about which macroscopic realization the run is — marks exactly one record actual; the unrealized branches remain in $\Phi$ but have no per-run physical referent (they are not "real elsewhere").
