...
**Bottom line:** the variational principle clarifies the *form* the answer should take, but delegates the actual selection to five sub-problems.
...
**Open:** observer-worldline dependence (covariance ≠ uniqueness); the restricted-monotonicity setting that makes $C(R)$ well-defined; principled choice of holographic surface prescription. Without these the framework has relocated foliation dependence rather than eliminated it.
...
**Difficulty.** Gisin's specific mechanism does not directly apply. But postselection-style signaling under global admissibility is a *real* threat with a concrete counter-example. The framework's formulation must be regionally local.
