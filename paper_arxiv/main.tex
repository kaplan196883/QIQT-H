...
A subsequent thread, postdating the snapshot quantified below, extends the same discipline to
emergent gravity. It formalizes, axiom-free and conditionally, Jacobson's equation-of-state route to
the Einstein field equations: from the framework's holographic capacity bound together with
relative-entropy (Klein) positivity it derives the field equations $a\,T = G + \Lambda g$, with a
genuine Einstein tensor and constant $\Lambda$, modulo exactly three explicitly labelled physics
inputs (a wedge KMS property, matter conservation, and standard structural regularity). The
differential area law $\delta S = \eta\,\delta A$, the modular-flow$=$boost identification, and the
Raychaudhuri focusing are derived rather than assumed; the labelled inputs are kept as hypotheses,
never as axioms. We report it only as evidence that the loop reaches new domains: like every result
here it certifies conditional mathematics, not the physics, and it is not a derivation of general
relativity from first principles.
