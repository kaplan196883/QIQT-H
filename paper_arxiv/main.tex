...
An Audited Human-AI Loop that Machine-Checks the Einstein Field Equations from a Finite-Information Bound}
...
at zero. Running this loop, we obtained a machine-checked, \emph{axiom-free} Lean derivation of the
...
same development carries an axiom-free
no-collapse measurement core; in total 192 modules, roughly 2{,}010 theorems, 0 project-specific
axioms, no \texttt{sorry}. We are explicit about scope: verification certifies that the derivation
is correct and rests on no hidden axiom, with its physics inputs labelled as explicit hypotheses;
it does not by itself establish those cited inputs or the framework's physical postulates. The
contribution is a transferable, auditable architecture for AI-assisted formalization, demonstrated
...
Einstein field equations; Jacobson equation of state; general relativity.
\end{abstract}
...
\item \textbf{Headline result: the Einstein field equations, machine-checked and axiom-free, from a
  ...
  Bekenstein-type capacity bound: a Jacobson-style equation of state in which the thermodynamic
  (area-law) input is \emph{derived} from the framework rather than assumed, with only
  ...
  about 2{,}010 theorems, 0 project axioms, no \texttt{sorry} (\S4).
...
directive for every headline theorem (830 of them in the case study), and we classify each dependency
...
hypothesis it closes is genuinely removable, but one it leaves standing is only \emph{not closed by
...
with $G$ the genuine Einstein tensor built from the metric. It is axiom-free and \texttt{sorry}-free,
...
(Klein's inequality) together \emph{derive} the differential first law $\delta\langle K\rangle=\eta\,
\delta A$ (\texttt{differential\_area\_law}); no hypothesis asserts the area law or the conclusion. Two
...
Lorentzian metric, and the constancy of $\Lambda$. Table~\ref{tab:grchain} sorts the chain into what
QIQT-H derives, what is cited, and what is machine-checked geometry.

\begin{table}[ht]
\centering
\caption{The QIQT-H + Bekenstein $\Rightarrow$ Einstein-field-equations chain, by status.}
\label{tab:grchain}
...
\S3.5, which anyone can re-run. The ledger reports that \texttt{qiqt\_bekenstein\_gives\_gr} takes 14
...
library could already supply. Consistent with \S3.5, this is a lower bound on minimality rather than a
...
The artifact is quantified rather than asserted. As of 2026-06-21 the development comprises 192 modules
and approximately 2{,}010 theorems and lemmas, with 830 \texttt{\#print axioms} directives in a
...
definiteness conjecture, or the Born principle are physically correct; and the headline derivation of
the Einstein field equations is explicitly conditional on its two cited inputs (the Bisognano-Wichmann
wedge-modular flux and Raychaudhuri focusing) and standard structural assumptions, which the
formalization records as hypotheses but does not establish. We state this not as a closing caveat but
as a structural feature of what machine verification can and cannot do for foundational science.
...
\bibitem{r43} P.~Kap\l{}a\'nski. \emph{QIQT-H formalization repository and master write-up.} \url{https://github.com/kaplan196883/QIQT-H}, commit 5727bcd, 2026.
...
\bibitem{r46} T.~Jacobson. \emph{Thermodynamics of Spacetime: The Einstein Equation of State.} Phys. Rev. Lett. 75, 1260, 1995. arXiv:gr-qc/9504004.
\bibitem{r47} J.~J.~Bisognano and E.~H.~Wichmann. \emph{On the duality condition for a Hermitian scalar field.} J. Math. Phys. 16, 985, 1975.
\bibitem{r48} W.~G.~Unruh. \emph{Notes on black-hole evaporation.} Phys. Rev. D 14, 870, 1976.
\bibitem{r49} A.~Raychaudhuri. \emph{Relativistic cosmology. I.} Phys. Rev. 98, 1123, 1955.
