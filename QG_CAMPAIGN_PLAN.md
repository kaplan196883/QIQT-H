...
- [x] **GATE 3 — the state-level LV bound. ✅ EXECUTED 2026-07-02** (theorem-shaped; GPT-5.5-pro-verified; `QIQTH/QG/StateLevelLVGate.lean`, all std-3 or axiom-free): **the surviving state-level reading makes NO low-energy LV prediction.** (A) the constraint set is FRAME-FREE (`admissible_smul_iff`, `constraintSet_invariant`; hinge `Sren_cov_of_traceCovariant` — trace transport carried, deletable via the RvD tower); (B) `stateLevel_noDeltaC2` — no dynamical modification ⟹ Δc² = 0 identically; (C) the ONLY reopening channels, certified: non-invariant prepared states (state breaking, not law-level), background-selecting saturation (`conditioned_invariant_iff_orbit_constant`), a NON-EQUIVARIANT enforcement mechanism (`equivariant_enforcement_preserves_invariance` — **the dynamical-realization gap is the sole remaining LV door**), biased selector λ (covariant-probability already held). Finite non-vacuous instance `permutationCapacity`. Literature-consistent (Bousso/Casini/QNEC: entropy bounds are not regulators).
