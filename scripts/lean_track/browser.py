...
def render_browser(decls, notation=None, roles=None, ref="main"):
    ...
    out.append("\n## Declarations\n")
    # declarations: theorems first (the interesting nodes), then defs
    ...
    for d in order:
        ...
        if d["kind"] in ("thm", "axiom", "opaque"):
            ...
            out.append("$$")
            out.append("")
        # (definitions are navigation nodes: their signature is on the source link)
        uses = [u for u in d.get("uses", []) if u in present]
        if uses:
            out.append("**uses** " + " ".join(link(u) for u in sorted(set(uses))))
        ub = usedby.get(name, [])
        if ub:
            out.append("**used by** " + " ".join(link(u) for u in sorted(set(ub))))
        out.append("")
    return "\n".join(out)
