...
def run(N=16, p=0.7):
  ...
  HONEST caveats / the one escape worth trying: (a) LZ is a proxy for K; (b) this is the HARD cutoff
  — a SOFT version (weight histories by 2^{-βK} alongside Born, a complexity 'temperature') need not
  collapse Born and is the only formulation that might thread the needle; (c) the biting regime is
  horizons (QG), where a Born deviation is at least conceptually where one COULD live. Net: the
  clean hard-cutoff law is killed; the soft-penalty variant is the only survivor to test next.""")
