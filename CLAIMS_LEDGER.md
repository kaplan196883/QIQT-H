...
| A3 | "**~299 files / ~3,300 theorems**; the `#print axioms` audit carries **2,263 directives over 259 modules**." | abstract; §11.4 banner; `formalization` (scale line) | §0 (~2,263 directives; 299 files) — counted directly | fact | ✓ (numbers refreshed 2026-07-01: +3 files/+3 modules/+50 pins from ModularEnergyBound/OperationalCapacity/MaxEntropyCapacity) |
...
| D8 | "The granularity capacity **maps onto the holographic dictionary**: with the induced `G=1/(N Λ_s²)`, the AdS/CFT boundary Cardy microstate count of a BTZ horizon *equals* QIQT-H's bulk capacity exponent `(A/4)N Λ_s²` (AdS radius cancels). A machine-checked **correspondence** — the two holographic bookkeepings agree under the shared `G` — **NOT** an import of a boundary CFT, the Cardy formula, bulk reconstruction, or AdS/CFT's cross-check; QIQT-H's capacity stays postulated." | §1.1a (Track C); `idea`/`theory`/`formalization`/`open-problems`/`ladder` | §0/§3 `HolographicBridge` (`btz_cardy_eq_qiqth_capacity`, `centralCharge_in_primitives`) | **[AF]** (bridge/correspondence, not a QIQT-H physics claim) | ✓ |
...
- **Audit scale (299 files / ~3,300 thms / 2,263 directives / 259 modules / budget 0)** — now identical in the
