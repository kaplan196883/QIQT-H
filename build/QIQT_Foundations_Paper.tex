...
What was previously deferred to \emph{four opaque interface axioms} in \texttt{LorentzSelection.lean} — the existence of the finite record algebras with the holographic bound (linchpin hypotheses 1–3), the boundary-reconstruction isomorphism (2), the Born/decoherence-functional measure with its Kolmogorov consistency and $\sigma$-additive extension (6), and the no-signaling marginal lemma (9) — has now been \textbf{discharged from the axiom budget}: those four \texttt{axiom \_ : Prop} placeholders (which asserted nothing and were used by no theorem) are \textbf{retired} and replaced by an explicit \texttt{RecordedHistoryNet} structure that \emph{writes the content down} as type-checked data and propositions (the holographic finiteness/cardinality bound, the boundary-reconstruction natural isomorphism commuting with restriction, the decoherence-functional probability weights, and the projective/no-signaling marginal). The covariance result is now a \emph{conditional theorem over this structure} (\texttt{covariant\_selection\_of\_net}, \texttt{covariant\_selection\_exists}), and no-signaling is itself a \emph{theorem} (\texttt{net\_no\_signaling}: the marginal onto a sub-diamond $K$ is the same regardless of which larger diamond $D\supseteq K$ it is computed from). The whole \texttt{LorentzSelection} module now adds \textbf{zero project axioms} (every headline theorem depends on the standard Lean axioms only, per \texttt{AxiomAudit.lean}; the project-wide raw axiom count drops $44\to 40$). This is the corpus's standard \emph{interface-as-hypothesis, not axiom} discipline (cf. \texttt{pushforward\_marginal\_local} and the now-axiom-free Gleason module): the deep Type III$_1$ / Tomita–Takesaki / Haagerup-$L^p$ analysis is \textbf{not} solved — current Mathlib does not reach it, and the physics literature cites rather than reproves it — but it is no longer four assumed axioms; it is the single, precise, written-down existence question \emph{"does a \texttt{RecordedHistoryNet} exist for a realistic relativistic QFT?"}. The honest status: \emph{equivariant naturality + a recorded-history net $\Rightarrow$ a covariant single-outcome selection map} is machine-checked and axiom-free; the finite Type-I modular flow and the free-field finite-mode record structure (count, decoherence factorization, covariant boost action) are machine-checked; and the sole remaining gap is the continuum \emph{existence} of the net — a finite skeleton proved and the continuum analysis named as one explicit conjecture, not several axioms.
