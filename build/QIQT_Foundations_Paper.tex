...
\emph{Lean status (\texttt{GleasonSelector.lean}).} The architecture is formalized in the finite-dimensional record model, in the project's standing discipline: the deep finite-dim Busch / POVM-Gleason representation step is the \emph{single named interface axiom} \texttt{effect\_gleason\_representation} (the additive→linear→trace-duality→ray-certainty chain, not in current Mathlib), and from it the consequences that matter are \emph{proved} — \texttt{history\_measure\_is\_born} / \texttt{history\_measure\_total} (the record measure is forced to the Born / decoherence-functional weights and is normalized) and \texttt{no\_signaling\_marginal} (requirement (e)/no-signaling, honest form: one bilinear correlation gives spacelike-marginal independence for \emph{all} of Bob's settings, depending on \textbf{no} project axiom). The Born functional's own linearity/normalization (\texttt{born\_add}, \texttt{born\_smul}, \texttt{born\_one}) is proved outright.
