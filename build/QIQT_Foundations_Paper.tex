...
\subsection{$\lambda$'s machine-checked law — the complete picture}

As of 2026-06-16, the actuality selector $\lambda$ has gone from a bare postulate to a \emph{law whose every checkable piece is machine-checked} (Lean 4 / Mathlib, axiom-free, raw axiom count 0). The earlier idea that $\lambda$ needs the gravitational crossed product to live on a Type II algebra is \textbf{retired as a category error} (Type II factors also have no minimal projections, so the records $\lambda$ selects come from a chosen abelian \emph{pointer} subalgebra $\mathfrak{A}$ that already lives inside the Type III$_1$ local algebra); $\lambda$ is \textbf{Type-III-native}, riding the standard form and the modular automorphism. The verified law has four layers, each established at the \textbf{finite}, the \textbf{one-particle continuum} (the Rieffel–Van Daele bounded modular flow $\Delta^{it}$), \emph{and} the \textbf{second-quantized free-field} (the unitary group $\Gamma(\Delta^{it})$) levels:

\begin{enumerate}
  \item \textbf{Which pointer algebra (kinematics).} The pointer algebra $\mathfrak{A}$ carrying $\lambda$'s records is fixed by \textbf{Takesaki's criterion}: an $\omega$-preserving conditional expectation onto $\mathfrak{A}$ exists iff $\sigma_t^\omega(\mathfrak{A}) = \mathfrak{A}$, which in the spectral/finite picture is $[\rho, P_\alpha] = 0$ — \emph{exact decoherence}. The decoherence map $E(x) = \sum_\alpha P_\alpha x P_\alpha$ is then that $\omega$-preserving conditional expectation (\texttt{LambdaPointer}, \texttt{ContinuumLambda.modAutOp\_fixes\_iff\_commute}, \texttt{ContinuumLambdaField}).
  \item \textbf{Born weights (Type-independent).} The weight of a record is the algebraic state value $\omega(P_\alpha) = \lVert P_\alpha \Phi\rVert^2$ — via the natural cone / vacuum state, \emph{no trace required}. Over a resolution of the pointer family these are a genuine probability (\texttt{bornWeights\_sum}, \texttt{NaturalConeBorn}, and on the genuine Fock vacuum state \texttt{FieldBorn}: a POVM's vacuum weights sum to one, the Weyl-bit record giving $(1\pm e^{-\lVert u\rVert^2/2})/2$).
  \item \textbf{Persistence (the static $\to$ dynamical bridge).} The decoherence map \textbf{commutes with the modular flow} $\sigma_t$ for \emph{every} $t$ (\texttt{dephase\_sigmaDiag\_commute}, \texttt{dephaseOp\_specProj\_commute}, \texttt{dephaseFock\_modAutFock\_commute}): a records-definite state stays records-definite, coherence between pointer sectors never regenerates, and the selected record is a fixed point of the dynamics with constant Born weights. This is the first genuinely \emph{dynamical} result — the selection does not un-select under evolution.
  \item \textbf{The selection event (one record, not zero or two).} An explicit inverse-CDF constructor takes an "actuality seed" $s \in [0,1)$ to \textbf{exactly one} record (\texttt{selects\_exists\_unique}, \texttt{continuum\_selects\_exists\_unique}, \texttt{field\_selects\_exists\_unique} — totality \emph{and} uniqueness), and the uniform seed measure of a record equals its Born weight (\texttt{volume\_selects}), so the selection \emph{realizes} Born as an across-run frequency. The selector is order-dependent, not equivariant — exactly as the \textbf{no-covariant-selector} result requires (a covariant \emph{measure} but no covariant \emph{selector}, the $S^2$ obstruction), while the measure is covariant; the construction is contextuality-safe (no global value-map) with state-independent no-signaling.
\end{enumerate}

\textbf{Born reduction and its irreducible core.} Born is reduced (axiom-free) to a \emph{state-supervenience} premise, and that premise is \textbf{split exactly}: its weak half (naturality / permutation-equivariance) is machine-checked to be \emph{blind to the exponent} — it holds for every reprocessing $f$, so it cannot force Born (\texttt{weight\_naturality}; the $w^2$ witness is natural yet non-Born, \texttt{weak\_underdetermines\_born}) — while its strong half (refinement-additivity) is what discriminates the square and linearizes into equiprobability (\texttt{refinementAdditive\_nsmul}). A \textbf{no-go} (\texttt{NoBornFromNothing}) proves \emph{some} such premise is unavoidable.
...
\begin{qiqtquote}
\textbf{Note (2026-06-15/16):} the per-row "named interface axiom" / project-axiom labels in this matrix are a \emph{historical snapshot} of the strengthening pass. The development has since been driven to \textbf{budget 0 — entirely axiom-free} (every theorem depends only on Lean's standard \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}), and λ's law (kinematics, persistence, Born, selection event) is machine-checked at the finite, one-particle continuum, and second-quantized free-field levels (§11.2b). Read the axiom-count cells as "as of the strengthening pass", superseded by the axiom-free milestone.
\end{qiqtquote}
