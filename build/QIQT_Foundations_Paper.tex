...
\textbf{Formal verification.} The deductive core of this framework, Theorems 3, 6, 7, Lemma 1, Donald's identity, the no-signaling chain, the Bell/CHSH inequality (with rigorous singlet construction reaching $2\sqrt{2}$), seven structural audits (independence of (H2) from (H1) plus its sharp reference-weight reformulation; unitary decoherence does not imply concentration; the modular-local entropy bound is distinct from the CPW renormalized entropy; branch-summed cost is not a holographic consequence; literal-finite (FQ) admissibility gives trivial dynamics; compression-locality leakage and the local-projection constraint on (FQ); $\mu$-selection is load-bearing for Born statistics; support preservation $\neq$ Born equivariance), and three sub-theorems for the Canonical IC Measure Principle (typicality Mackey-Gleason; operational data insufficient; FQ-equivariance uniqueness), is machine-verified in Lean 4 with Mathlib. A subsequent \textbf{A1+A2+A4+A6 strengthening pass} further: (i) proves the \emph{finite marginal-locality step} of the Canonical IC Measure Principle as a theorem holding for \textbf{any} IC measure, with no equivariance assumption (\texttt{Marginal\allowbreak{}Locality.\allowbreak{}pushforward\_\allowbreak{}marginal\_\allowbreak{}local}), so locality is no longer an \emph{independent} Born-selection sub-axiom, though the reduction remains conditional on a named, explicit Hilbert→set-level locality bridge axiom (\texttt{set\_\allowbreak{}level\_\allowbreak{}locality\_\allowbreak{}from\_\allowbreak{}unitary\_\allowbreak{}dilation}); (ii) proves the permutation-symmetry collapse component of Goldstein-Struyve sub-lemma 1c under its necessary hypothesis (\texttt{step1c\_\allowbreak{}collapse\_\allowbreak{}of\_\allowbreak{}perm\_\allowbreak{}symmetric}) together with the foundational matrix-conjugation identities $P_\sigma \cdot E_{ij} \cdot P_\sigma^* = E_{\sigma(i),\sigma(j)}$ and $D(z)\cdot E_{ij}\cdot D(z)^* = (z_i\,\overline{z_j})\,E_{ij}$, while the \emph{full} Goldstein-Struyve Schur classification remains a single named interface axiom; (iii) proves a single-trial Chebyshev frequency-concentration bound and the variance-addition (independence) lemma for Bernoulli trials (\texttt{Born\allowbreak{}Concentration.\allowbreak{}lean}), with the $N$-fold product-measure scaling still resting on the LLN interface axiom; and (iv) packages the three remaining Born sub-axioms with their independence countermodels in a unified minimality table (\texttt{Born\allowbreak{}Minimality\allowbreak{}Table.\allowbreak{}lean}). Consolidation: the \texttt{FQEquivariance\allowbreak{}Uniqueness} module was rewritten to route through the concrete \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Fin\allowbreak{}Dim} proof (eliminating 12 redundant abstract axioms), and the five Goldstein-Struyve Step-1 sub-axioms (three of which were \emph{literally false as stated} and all of which were unused) were deleted, reducing the project axiom total from 57 to 40 and leaving \texttt{canonical\_\allowbreak{}ic\_\allowbreak{}measure\_\allowbreak{}principle} dependent on exactly two project-specific axioms (Schur classification + tensor multiplicativity). The theorems introduced by the strengthening pass themselves depend only on the standard Lean axioms \texttt{propext}, \texttt{Classical.\allowbreak{}choice}, \texttt{Quot.\allowbreak{}sound}; the \emph{end-to-end} Born pipeline still rests on the named interface axioms (Mackey-Gleason / Radon-Nikodym, Schur classification, tensor multiplicativity, the LLN scaling, and the locality bridge), which the complete axiom audit in \texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}Axiom\allowbreak{}Audit.\allowbreak{}lean} enumerates explicitly. A subsequent \textbf{capacity-exclusion pass} added three further modules, all \emph{axiom-free} (every theorem depends only on the standard Lean axioms \texttt{propext}, \texttt{Classical.\allowbreak{}choice}, \texttt{Quot.\allowbreak{}sound}), that mechanize the single-record \emph{mechanism} (the number-bound form of the Macroscopic Definiteness Conjecture, §7.6) as a conditional theorem: \texttt{Core\allowbreak{}No\allowbreak{}Collapse.\allowbreak{}lean} (finite-capacity exclusion + the actuality selector $\lambda$ $\Rightarrow$ exactly one actual record; a genuine projection-valued measure with Born normalization $\sum_k\|E_k\psi\|^2=1$ and collapse recovered as Bayesian conditionalization), \texttt{Capacity\allowbreak{}Model.\allowbreak{}lean} (the capacity threshold \emph{derived} from orthonormality of pointer states, $\sum_j \dim_j \le D$, so at most one record can occupy more than half the register), \texttt{SBSBridge.\allowbreak{}lean} (an objective record broadcast to $R$ environment fragments in a Spectrum Broadcast Structure forces storage $\ge R\log n$, with robustness to \emph{approximate} decoherence and an amplification result turning weak per-collision distinguishability $\gamma<1$ into exponentially reliable records), and \texttt{Collisional\allowbreak{}Gamma.\allowbreak{}lean} (the per-collision $\gamma<1$ itself \emph{derived} from the toy Hamiltonian $H_{\rm int}=g\,\sigma_z^S\otimes\sigma_x^E$: the branch records overlap by $\cos 2\theta$ via a verified one-parameter unitary group, so $\gamma=|\cos2\theta|<1$, amplifying to $\gamma^L\to0$). This pass also completed a bounded-Borel functional calculus $\int f\,dE$ on the spectral side (\texttt{Spectral/\allowbreak{}PVM.\allowbreak{}lean}), and brings the project axiom total to \textbf{37}. The capacity-exclusion result is \emph{conditional}: with the toy-Hamiltonian discharge of the scattering premise above, its remaining open physical input is the \emph{field-theoretic} origin of that premise (uniform $\gamma<1$ and the QND/no-recoherence structure from a realistic Hamiltonian); everything downstream of the premise is machine-checked. See \texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}} in the project repository. \textbf{Status (current, 2026-06-15): the project is now axiom-free.} The budget check reports zero \texttt{axiom} declarations — every theorem depends only on Lean's standard \texttt{propext}, \texttt{Classical.\allowbreak{}choice}, \texttt{Quot.\allowbreak{}sound}; the 57 → 40 → 37 reduction above continued to \textbf{budget 0} as the remaining continuum interface axioms were retired (typeclass/instance pattern), with the continuum Type III$_1$ realization now a \emph{cited frontier} rather than an axiom. A further axiom-free pass mechanizes the corrected $(\Phi,\lambda)$ law: \texttt{Redundancy\allowbreak{}Compressible} (redundant records are compressible — the combinatorial core of the H2 category error: $R$ copies carry $H(X)$, not $R\,H(X)$); \texttt{Weyl\allowbreak{}Bit\allowbreak{}Consistency} / \texttt{Weyl\allowbreak{}Bit\allowbreak{}Strong\allowbreak{}Decoherence} / \texttt{SBSSuppression} (decoherent-histories consistency of the free-field record measure — weak \texttt{Re D = 0} everywhere, full \texttt{D = 0} for orthogonal records, redundancy-driven suppression); \texttt{Born\allowbreak{}Equiprobable} / \texttt{Envariance\allowbreak{}Justification} / \texttt{State\allowbreak{}Supervenience} (Born-from-typicality reduced to a single state-supervenience premise, with envariance \emph{proved} and the amplitude→count bridge from orthonormality); and \texttt{Covariant\allowbreak{}Gluing} / \texttt{Contextuality\allowbreak{}Safe} / \texttt{Fock.\allowbreak{}Weyl\allowbreak{}Bit\allowbreak{}Bell} (Open Problem 3b: a covariant measure exists but no covariant \emph{selector}; the record measure is contextuality-safe with state-independent no-signaling). All are axiom-free; the continuum Type III$_1$ / interacting case remains the cited frontier.
\end{abstract}
...
\begin{itemize}
  \item \textbf{Continuous, conserved amplitude.} The weights $|c_k|^2$ are continuous and are \emph{conserved} under unitary evolution; decoherence never drives them to $0$ or $1$ (formalized as \texttt{No\allowbreak{}Concentration}; §9). The wave function itself has no binary structure and no probability.
...
\begin{enumerate}
  ...
  \item The standard textbook formal wave function, a superposition $\sum_i c_i |s_i\rangle|A_i\rangle|E_i\rangle$, is an idealized statistical description of an ensemble of per-run wave functions across many runs. The per-run wave function evolves unitarily from specific microscopic initial conditions; decoherence renders the macroscopic record components non-interfering; and the actuality selector $\lambda$ leaves the per-run regional content single-record. (An earlier version attributed the single record to a \emph{capacity overflow} — that a $\ge 2$-record content "exceeds $Q_R$"; this is \textbf{retired as a category error}, see §11.2a. $Q_R$ is the finite record \emph{stage}, bounding how many distinguishable records exist, not a single-outcome rule; the single outcome is $\lambda$'s.) The run's microscopic initial conditions index which record.
...
\begin{itemize}
  ...
  \item \textbf{Deterministic-generator ('t Hooft Cellular Automaton) reading — considered and not adopted.} A distinct way to give $\lambda$ a "law" would make it a \emph{finite-information deterministic generator} of the actual history (a small seed + rule, in the spirit of 't Hooft's deterministic quantum mechanics) rather than a typicality-sampled brute fact. We flag this only to set it aside, with the honest verdict (developed in the supplementary explorations): it is empirically equivalent to standard QM; it must pay Bell's price (nonlocality or superdeterminism, since a finite \emph{local} generator is CHSH-bounded at $2 < 2\sqrt2$); and it is untestable in practice — a Born-typical history is algorithmically \emph{incompressible} (Martin–Löf random), so a compressible generator would deviate from Born only at its repetition scale, which for the realized cosmic entropy ($\sim 10^{104}$ bits) is the \textbf{Poincaré recurrence time} $\sim 2^{10^{104}}$. The program therefore keeps $\lambda$ as a \emph{brute, Born-typical (incompressible)} actuality fact, not a pseudo-random generator; the generator picture is recorded as a mapped-but-not-adopted alternative.
\end{itemize}
...
\textbf{Formalization note, the number-bound is a machine-checked conditional theorem (the strong form is not).} The cost-counting argument above has a \emph{continuum} part (the renormalized-entropy scaling on the Type II regional algebra, which remains the open Macroscopic Definiteness Conjecture, Open Problem 3) and a \emph{finite-dimensional core} that we have now mechanized. In a tractable finite model the argument is no longer a conjecture but an axiom-free Lean theorem (modules \texttt{Core\allowbreak{}No\allowbreak{}Collapse.\allowbreak{}lean}, \texttt{Capacity\allowbreak{}Model.\allowbreak{}lean}, \texttt{SBSBridge.\allowbreak{}lean}; standard Lean axioms only). Concretely: realizing the constituent records as an objective Quantum-Darwinist record, a Spectrum Broadcast Structure (Korbicz, arXiv:2007.04276) in which the $n$-outcome pointer is redundantly broadcast to $R$ environment fragments, forces the broadcast support to have Hilbert dimension $\ge n^{R}$ (the fragment Hilbert spaces tensor, so dimensions multiply), hence an \emph{information} storage cost $\ge R\log n$ (\texttt{SBSBridge.\allowbreak{}redundancy\_\allowbreak{}le\_\allowbreak{}log\allowbreak{}Storage}). Under a finite additive storage capacity $Q_{\max}$, a "macroscopic" record (large redundancy $R$) then has cost exceeding $Q_{\max}/2$, so a region cannot carry two of them: at most one such record is active (\texttt{Core\allowbreak{}No\allowbreak{}Collapse.\allowbreak{}coactual\_\allowbreak{}subsingleton}, \texttt{SBSBridge.\allowbreak{}sbs\_\allowbreak{}single\_\allowbreak{}outcome}); the selector $\lambda$ supplies which one. The capacity threshold is \emph{derived} from orthonormality of the pointer states ($\sum_j\dim_j\le D$, \texttt{Capacity\allowbreak{}Model.\allowbreak{}capacity\_\allowbreak{}total}), not stipulated, and the chain is robust to \emph{approximate} decoherence, a near-orthonormal fragment family (pairwise overlap $<1/(n-1)$) is still linearly independent, so the dimension bound survives imperfect distinguishability (\texttt{fragment\_\allowbreak{}finrank\_\allowbreak{}ge\_\allowbreak{}approx}), with an amplification lemma showing that weak per-collision distinguishability $\gamma<1$ compounds to exponentially reliable records (block overlap $\le\gamma^{L}\to 0$). This mechanizes the \emph{number-bound} (at most one macroscopic record) form of the conjecture; it does \textbf{not} establish the strong superposition-exclusion form (the redundant cat state $\alpha|0\rangle^{\otimes N}+\beta|1\rangle^{\otimes N}$ remains the obstruction, §7.6, Open Problem 3). It is moreover a \emph{conditional} theorem whose one physical input is the factorized, branch-dependent scattering premise ($\gamma<1$); \textbf{for the standard collisional/QND monitoring model that premise is itself now a machine-checked theorem.} Taking the textbook interaction $H_{\rm int} = g\,\sigma_z^S \otimes \sigma_x^E$ (Zurek), the branch-conditioned environment records overlap by $\langle E_+|E_-\rangle = \cos^2\theta - \sin^2\theta = \cos 2\theta$, so the per-collision distinguishability $\gamma = |\cos 2\theta| < 1$ for generic coupling $\theta = g\tau$ is \emph{derived}, not assumed (module \texttt{Collisional\allowbreak{}Gamma.\allowbreak{}lean}: \texttt{branch\_\allowbreak{}overlap}, \texttt{gamma\_\allowbreak{}lt\_\allowbreak{}one}). The propagator $U_s(\theta) = \exp(-i\theta\,sA)$ is a verified one-parameter unitary group ($A = \sigma_x$ a self-adjoint involution; \texttt{collision\allowbreak{}U\_\allowbreak{}group}, \texttt{hamiltonian\_\allowbreak{}is\allowbreak{}Symmetric}), the pointer basis is einselected because $[H_{\rm int},\sigma_z^S]=0$, and over $L$ independent collisions the overlap factorizes to $\gamma^L \to 0$ (\texttt{collisional\_\allowbreak{}overlap\_\allowbreak{}tendsto\_\allowbreak{}zero}), supplying the previously-assumed amplification input. The residual open physics is the \emph{field-theoretic} version, deriving uniform $\gamma<1$, and the QND/no-recoherence structure itself, from a realistic system–environment Hamiltonian rather than positing the collisional form, a sharply isolated form of the Strasberg–Winter "branch selection problem" (arXiv:2601.19703). Everything downstream of the premise is machine-checked.
...
even though at the ontic level $P(A\mid a,b,\lambda)\neq P(A\mid a,\lambda)$. Operational no-signaling is established for the framework's instruments (Theorem 7, §7.7; Lean \texttt{Theorem7.\allowbreak{}Setup.\allowbreak{}no\_\allowbreak{}signaling}). That the \emph{required} cancellation is not fine-tuned, i.e. that the same $\mu$ delivering Born weights also enforces no-signaling without an experiment-by-experiment conspiracy, is a genuine open burden (the Wood–Spekkens fine-tuning problem), recorded as part of the open Born-typicality program (§7.4, §11.4) and in \texttt{PROGRAM\_\allowbreak{}STATUS.\allowbreak{}md}.
...
The conditional shape of Theorem 5 is now formalized in \texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}Born\allowbreak{}Typicality.\allowbreak{}lean} (theorem \texttt{qiqth\_\allowbreak{}born\_\allowbreak{}typicality\_\allowbreak{}conditional}). The deterministic core (mean per-run frequency equals the outcome marginal) is rigorously proved; the LLN step is taken as a standard probability black box at the interface layer.
...
\textbf{Audit remark, universal realizability (the negative companion).} A machine-verified audit (\texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}No\allowbreak{}Born\allowbreak{}From\allowbreak{}Nothing.\allowbreak{}lean}) establishes that for \textbf{any} target outcome distribution $p$ (not just $|c_k|^2$) and any surjective outcome map from the IC space, there exists a measure $\mu_p$ whose outcome-marginal equals $p$. Construction: choose a section of the outcome map and place mass $p_k$ on $s(k)$. Hence the framework's structural axioms (FQ, microcausality, Donald, holographic bound) do \emph{not} select Born over any other distribution. Hypothesis (i) of Theorem 5 is therefore \emph{genuinely independent} of the other QIQT-H postulates, it must be added as a separate principle, not derived.

\textbf{Audit remark, support preservation $\neq$ Born equivariance.} A second machine-verified audit (\texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}Equivariance\allowbreak{}Gap.\allowbreak{}lean}) establishes that the framework's claim that the exactly-unitary dynamics carries instantiable regional contents to instantiable regional contents (the closure property of §7.6, preservation of the \emph{support} of the admissible set) is strictly weaker than the measure-preservation hypothesis (iii) of Theorem 5. Concrete counterexample: a bijection on a 2-point space preserves the support trivially yet shuffles a non-uniform measure. Hypothesis (iii) is therefore a \emph{genuine additional commitment} beyond the framework's current postulates.
...
\textbf{Sharpening of (H2), reference-weight form.} A machine-verified audit (see \texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}H1\allowbreak{}H2\allowbreak{}Audit.\allowbreak{}lean} in the project repository) confirms that (H2) is genuinely independent of (H1), Donald's identity, Klein positivity, and DPI: a classical KL countermodel with $\sigma=(1/2,1/2)$ and $\rho=\delta_0$ satisfies all the structural hypotheses but violates (H2) with $I_0=1$. The same audit also yields the following sharp structural reformulation of (H2). For a perfect-record state on event $E_{\rm record}$,
...
\subsection{Where the finite budget bites: a kinematic map, and why $\lambda$ stays inert}
...
\begin{itemize}
  \item \textbf{Ordinary matter sits far below the line.} A trapped-ion qubit, a cubic centimetre of gas, the Earth, the Sun, the cosmic microwave background all lie $30$–$60$ orders of magnitude below their holographic budgets. Even the entire observable universe, whose entropy is dominated by supermassive black holes ($\sim 10^{104}$ bits), runs at $\sim 10^{-18}$ of its holographic capacity ($\sim 10^{122}$ bits). For every such system the budget is slack, decoherence sets the operative resolution, and the framework predicts exactly standard quantum mechanics (consistent with §9.1).
  \item \textbf{Only horizons saturate.} The systems that reach $S_{\text{demand}} = Q_R$ are precisely black holes and cosmological horizons, the maximal-entropy gravitational objects, which lie on the boundary $S = A/4\ell_P^2$ by construction. This is the regime of §9.3.
\end{itemize}

Two honest qualifications fix what this does and does not show. First, the bound is \textbf{kinematic}: it caps how much record information a region can carry, not any rate or force; a finite budget is not a finite dynamics. Second — and decisively — \textbf{the saturation regime is governed by standard black-hole thermodynamics (Bekenstein–Hawking), not by $\lambda$.} Modelling the finite budget in concrete continuous systems (a particle in a box; a harmonic oscillator in coherent, squeezed, thermal, and Fock states) confirms this in detail: the budget caps phase-space resolution exactly as semiclassical state-counting already does, with no $\lambda$-specific correction anywhere, and the apparent "truncation = Bekenstein–Hawking" reading is a re-labelling of the holographic bound, not an independent effect. Because $\lambda$ is non-dynamical and Born-transparent, it cannot produce a resolution cap, a cooling, or a deviation; on the horizon line the physics is the gravitational entropy bound that every approach shares. The framework therefore remains operationally equivalent to Everett across the entire chart: slack everywhere observable, and standard quantum gravity where it saturates.

\subsection{Emergent gravity: the Einstein equations as a machine-checked equation of state}

If the holographic capacity bound is taken seriously as a thermodynamic constraint on regional information, the natural question is whether it reproduces gravitational dynamics in the manner of Jacobson's (1995) "Einstein equation of state." It does, as a \textbf{conditional} result that we have formalized end to end in Lean 4 / Mathlib — axiom-free in the same sense as the rest of the development (\texttt{\#print axioms} returns only \texttt{propext}, \texttt{Classical.\allowbreak{}choice}, \texttt{Quot.\allowbreak{}sound}).

The headline is a single machine-checked theorem (\texttt{qiqt\_\allowbreak{}gr\_\allowbreak{}from\_\allowbreak{}wedge\_\allowbreak{}kms}): the \textbf{Einstein field equations} $a\,T_{\mu\nu} = G_{\mu\nu} + \Lambda\,g_{\mu\nu}$, with a genuine Einstein tensor and a \emph{constant} $\Lambda$, follow from QIQT-H's holographic \textbf{capacity bound} $S \le \eta A$ together with \textbf{Klein positivity} $S(\rho\Vert\sigma)\ge 0$, modulo \textbf{exactly three clearly-labelled physics inputs}: the wedge \textbf{KMS property}, \textbf{matter conservation} $\nabla^\mu T_{\mu\nu}=0$, and standard \textbf{structural regularity} (a Lorentzian congruence, Raychaudhuri kinematics, and smoothness).

What is genuinely \emph{derived} in the chain — no hypothesis assuming the conclusion:

\begin{itemize}
  \item the \textbf{differential area law} $\delta S = \eta\,\delta A$, obtained from the capacity \emph{bound} together with saturation at the reference state and Klein positivity (the bound, its saturation, and positivity are theorems here; no hypothesis asserts the equality $S = \eta A$);
  \item the \textbf{boost-charge} content of the wedge KMS input: the wedge modular flow is the geometric Lorentz boost (one-particle Bisognano–Wichmann), and the boost-energy derivative is purely imaginary, $i\cdot(\text{real})$, derived from the explicit boost generator together with unitarity;
  \item the \textbf{focusing} content of the structural input: at a stationary horizon the Raychaudhuri equation collapses to pure Ricci focusing, $\mathrm{d}\theta/\mathrm{d}\lambda = -R_{kk}$;
  \item the entire \textbf{differential geometry}: the Bianchi identities, $\nabla^\mu G_{\mu\nu}=0$, the null-cone $\Rightarrow$ tensor step, and the constancy of $\Lambda$.
\end{itemize}

\textbf{Honest scope.} This is a verified \emph{formalization} of a \emph{conditional} chain, not a derivation of general relativity from nothing, and not a new physical prediction. The three labelled inputs are kept as explicit hypotheses and never as axioms. The wedge KMS property bottoms out in cited algebraic-QFT facts (KMS-uniqueness, the strip property, standardness — the Bisognano–Wichmann package), which we cite rather than formalize: these are standard, true theorems, but a Lean proof would require operator-algebra infrastructure (unbounded Tomita–Takesaki theory, Hardy-strip methods) not yet present in Mathlib — a substantial separate undertaking, not an impossibility in principle. The boost charge's final identification requires the free-field stress tensor (one scalar identity, not yet constructed); and matter conservation $\nabla^\mu T_{\mu\nu}=0$ is a physical postulate. What QIQT-H supplies as \emph{theorems} is the inequality side of the area law; what turns it into general relativity is Jacobson's argument, here machine-checked. The contribution is to have \emph{drained} the analytic and geometric content out of two of the three inputs into theorems (modular flow $=$ boost; Raychaudhuri focusing), sharpening each labelled input to its irreducible kernel, and to have certified the whole chain at the level of the proof-assistant kernel. It is consistent with, and complementary to, the Wald reading of §8.3: there the gravitational theory fixes the regional trace and hence $Q_R$; here the capacity bound, read as a Clausius relation, returns the field equations. The development is documented in the formalization companion; a statement-level index is public.
...
\textbf{Machine-checked progress on $\lambda$'s law (2026-06-15).} Since the first draft, the actuality selector's \emph{kinematic} law has been made precise and machine-checked (axiom-free, Lean 4 / Mathlib, finite / Type I shadow; \texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}Lambda\allowbreak{}Pointer.\allowbreak{}lean}). Two findings. First, $\lambda$ is \textbf{Type-III-native}: an operator-algebra red-team retired the idea that the Type II crossed product is needed to give $\lambda$ a \emph{home}. Type II factors also lack minimal projections, so the records $\lambda$ selects come from a chosen abelian \emph{pointer} subalgebra $\mathfrak A$ that already exists inside the Type III$_1$ factor, and the Born weights are the algebraic state value $\omega(P_\alpha)=\lVert P_\alpha\Phi\rVert^2$ (via the natural cone, no trace required). The Type II framework remains the rigorous home for the (FQ) \emph{capacity / entropy} bound, its proper use, but it is not load-bearing for \emph{selection}. Second, \emph{which} pointer algebra is consistent is fixed by \textbf{Takesaki's criterion}: an $\omega$-preserving conditional expectation onto $\mathfrak A$ exists iff $\sigma_t^\omega(\mathfrak A)=\mathfrak A$, which in finite dimensions is $[\rho,P_\alpha]=0$, exact decoherence (\texttt{mod\allowbreak{}Aut\_\allowbreak{}fixes\_\allowbreak{}iff\_\allowbreak{}commute}); the decoherence map $E(x)=\sum_\alpha P_\alpha x P_\alpha$ is then that $\omega$-preserving conditional expectation (\texttt{dephase\_\allowbreak{}preserves\_\allowbreak{}state}); the Born weights are a genuine probability (\texttt{born\allowbreak{}Weights\_\allowbreak{}sum}); and, the first \emph{dynamical} result, that selection \textbf{persists for all time} under the real-time modular flow $\sigma_t(x)=\rho^{it}x\rho^{-it}$ (\texttt{dephase\_\allowbreak{}sigma\allowbreak{}Diag\_\allowbreak{}commute}; unconditional in the einselected basis, \texttt{dephase\_\allowbreak{}sigma\allowbreak{}Diag\_\allowbreak{}commute\_\allowbreak{}diagonal}), so coherence between pointer sectors never regenerates and the selected record is a fixed point of the dynamics with constant Born weights. What remains open for $\lambda$: a constructor for the \emph{selection event} (why exactly one record becomes actual), the across-run \emph{frequency} derivation (Open Problem 1, bounded by the Born no-go), and the continuum Type III$_1$ realization, now targeting the \textbf{standard form / natural cone} with the modular $\sigma_t^\omega$ via Connes cocycles rather than the retired crossed-product route.

\textbf{Honest characterization of math status:} the framework provides the \emph{scaffolding} (Type II algebras from CPW/Witten) and the \emph{axiom} ((FQ) literal reading). It establishes \emph{qualitative} consequences, and now an axiom-free, machine-checked \emph{selection-and-consistency schema} for $\lambda$ — the kinematic criterion, the modular-invariance, the Born-weight bookkeeping, and the inverse-CDF selection representation — at the finite, one-particle continuum, and second-quantized free-field levels (§11.2b), conditional on a primitive actuality datum and a Born premise. What it does \emph{not} establish are the \emph{physical} dynamical claims (irreversibility under the actual Hamiltonian; the origin of the seed), nor theorems for the two cited research walls (the Haagerup natural-cone existence, the interacting case). This is the status of a research program with a sharply-localized residual — and a verified consistency scaffold around an admitted primitive — not a completed mathematical theory.
...
\subsection{$\lambda$'s machine-checked *selection schema* — the verified scaffold and its honest reach}

What is machine-checked around $\lambda$ (Lean 4 / Mathlib, axiom-free, raw axiom count 0) is best stated precisely, because the word "law" would overclaim. It is a \textbf{selection-and-consistency schema conditional on a primitive actuality datum}: \emph{given} a chosen decohered record context, \emph{given} the Born weights, \emph{given} a refinement-additive typicality premise, and \emph{given} an actual seed, the formalism selects exactly one record and verifies a body of modular-invariance, normalization, covariance, and contextuality facts. "Axiom-free in Lean" means no \emph{Lean} axioms beyond the standard foundations; it does \textbf{not} mean no physical postulates — those sit in the hypotheses (the chosen record algebra, exact decoherence, the Born weights, the uniform seed measure, refinement-additivity). With that framing, the verified content has four parts, each established at the \textbf{finite}, the \textbf{one-particle continuum} (the Rieffel–Van Daele bounded modular flow $\Delta^{it}$), \emph{and} the \textbf{second-quantized free-field} (the unitary group $\Gamma(\Delta^{it})$) levels. (The earlier idea that $\lambda$ needs the gravitational crossed product to live on a Type II algebra is \textbf{retired as a category error}: Type II factors also have no minimal projections, so the records $\lambda$ selects come from a chosen abelian coarse-graining $\mathfrak{A}=\operatorname{span}\{P_\alpha\}$ associated with — not an atom of — the Type III$_1$ local algebra. $\lambda$ rides the standard form and the modular automorphism, \emph{not} a forced trace; "Type-III-native" should be read in that sense, not as "$\lambda$ is an atom of the factor.")

\begin{enumerate}
  \item \textbf{Which record context (kinematics).} The condition for a record context to carry an $\omega$-preserving coarse-graining is \textbf{Takesaki's criterion}: an $\omega$-preserving conditional expectation exists iff the subalgebra is invariant under the modular automorphism $\sigma_t^\omega$, which in the spectral/finite picture is $[\rho, P_\alpha] = 0$ — \emph{exact decoherence}. The dephasing map $E(x) = \sum_\alpha P_\alpha x P_\alpha$ is then the $\omega$-preserving conditional expectation onto the \textbf{block-diagonal algebra} $\{P_\alpha\}'$ (generally nonabelian; the \emph{classical record labels} are the atoms of the abelian sub-coarse-graining $\operatorname{span}\{P_\alpha\}$ within it). Two honest caveats: (i) here $\rho/\omega$ is a \emph{faithful normal} (hence mixed/reduced) state — for a \emph{pure} global $\Phi$, $[P_\alpha,|\Phi\rangle\langle\Phi|]=0$ would force all but one weight to vanish; the cyclic-separating-vector ($\lVert P_\alpha\Phi\rVert^2$) reading is the AQFT/Reeh–Schlieder one; (ii) exact $[\rho,P_\alpha]=0$ is an idealization (a superselection-like exactness), and an approximate-robustness version is future work (\texttt{Lambda\allowbreak{}Pointer}, \texttt{Continuum\allowbreak{}Lambda.\allowbreak{}mod\allowbreak{}Aut\allowbreak{}Op\_\allowbreak{}fixes\_\allowbreak{}iff\_\allowbreak{}commute}, \texttt{Continuum\allowbreak{}Lambda\allowbreak{}Field}).
  \item \textbf{Born weights.} The weight of a record is the algebraic value of the (faithful normal) state on its effect, $\omega(P_\alpha)$ — via the natural cone / vacuum state, \emph{no trace required} — and over a resolution these are a genuine probability (\texttt{born\allowbreak{}Weights\_\allowbreak{}sum}, \texttt{Natural\allowbreak{}Cone\allowbreak{}Born}; on the genuine Fock vacuum state \texttt{Field\allowbreak{}Born}: a POVM's vacuum weights sum to one, the Weyl-bit record effect $(1\pm \operatorname{Re}W(u))/2$ giving $(1\pm e^{-\lVert u\rVert^2/2})/2$).
  \item \textbf{Modular invariance (not physical dynamics).} The dephasing map \textbf{commutes with the modular flow} $\sigma_t$ for \emph{every} $t$ (\texttt{dephase\_\allowbreak{}sigma\allowbreak{}Diag\_\allowbreak{}commute}, \texttt{dephase\allowbreak{}Op\_\allowbreak{}spec\allowbreak{}Proj\_\allowbreak{}commute}, \texttt{dephase\allowbreak{}Fock\_\allowbreak{}mod\allowbreak{}Aut\allowbreak{}Fock\_\allowbreak{}commute}). This is a genuine consistency fact — no \emph{modular} recoherence inside the chosen invariant record algebra — but it is essentially a functional-calculus consequence of building the records from operators invariant under the same generator, and the \textbf{modular flow is not the physical Hamiltonian evolution} (the two coincide only in special KMS / Bisognano–Wichmann cases). So it is \emph{modular-invariance of an exactly-decohered record algebra}, \textbf{not} a proof that actual records do not recohere under the physical dynamics; that physical-irreversibility statement is not established.
  \item \textbf{The selection event (a sampling representation).} An explicit \textbf{inverse-CDF} constructor takes an "actuality seed" $s \in [0,1)$ to exactly one record (\texttt{selects\_\allowbreak{}exists\_\allowbreak{}unique}, \texttt{continuum\_\allowbreak{}selects\_\allowbreak{}exists\_\allowbreak{}unique}, \texttt{field\_\allowbreak{}selects\_\allowbreak{}exists\_\allowbreak{}unique} — totality \emph{and} uniqueness of a standard sampling map), and the \textbf{single-shot} Lebesgue measure of the seeds selecting a record equals its Born weight (\texttt{volume\_\allowbreak{}selects}). This is a clean representation of "one outcome, Born-weighted"; it adds no actualization \emph{mechanism} (the seed's origin and typicality are postulated), and a single-shot measure equality is \emph{not yet} an across-run frequency — that requires a product/history measure and a law of large numbers (a separate finite-LLN module). The selector is order-dependent, not equivariant — exactly as the \textbf{no-covariant-selector} result requires (a covariant \emph{measure} but no covariant \emph{point-selector}, an $S^2$-type obstruction); the construction is contextuality-safe (no global value-map) with state-independent no-signaling (which is operationally weaker than Bell local causality — the selector remains globally contextual/nonlocal).
\end{enumerate}

\textbf{Born reduction and its irreducible core.} Born is reduced (axiom-free) to a \emph{state-supervenience} premise, and that premise is \textbf{split exactly} — a Gleason/Busch-style assumption audit: its weak half (naturality / permutation-equivariance) is machine-checked to be \emph{blind to the exponent} — it holds for every reprocessing $f$, so it cannot force Born (\texttt{weight\_\allowbreak{}naturality}; the $w^2$ witness is natural yet non-Born, \texttt{weak\_\allowbreak{}underdetermines\_\allowbreak{}born}) — while its strong half (refinement-additivity) is what discriminates the square and linearizes into equiprobability (\texttt{refinement\allowbreak{}Additive\_\allowbreak{}nsmul}). A \textbf{no-go} (\texttt{No\allowbreak{}Born\allowbreak{}From\allowbreak{}Nothing}) proves \emph{some} such premise is unavoidable. This is honest assumption-auditing, not "Born from nothing."

\textbf{Conservativity, and what this does and does not buy.} Because $\lambda$ has no back-reaction and the Born weights are assumed, the scheme is \textbf{operationally equivalent to standard (Everettian/orthodox) quantum mechanics}: it makes no prediction beyond standard Born statistics, and $\lambda$ is unobservable. Its content is interpretive — a single actual record by stipulation, with no collapse dynamics and no second substance — placing it in the \textbf{modal / Everett-plus-actuality-tag} family rather than as a new dynamical law. The irreducible residuals are exactly two: the \textbf{actuality datum itself} (the selection-event constructor reduces the whole selection to \emph{which seed is actual}, and that seed \emph{is} $\lambda$ — a primitive a non-dynamical single-world ontology must take as given; note the seed is a real number carrying unbounded information, consistent only because $\lambda$ is not a physical record counted by $Q_R$), and the \textbf{strong Born premise} (proven unavoidable). Beyond the free-field sector the remaining mathematics is two cited research walls — the \textbf{Haagerup natural-cone existence} in Mathlib (we state the Born rule on vector states; the canonical state$\mapsto$vector identification is cited) and the \textbf{interacting (non-free) case} — and the natural next \emph{content}-adding targets are a \textbf{global decoherent-history} selector (one coherent actual world, not one atom of one finite resolution) and an explicit \textbf{across-run frequency} theorem.

\textbf{What remains, sharply localized.} Two residuals, both honest. (i) The actuality datum itself: the selection-event constructor reduces the whole selection to \emph{which seed $s$ is actual} — and the seed \emph{is} $\lambda$, the one primitive a non-dynamical single-world ontology must take as given; its \emph{dynamical origin} is not derived (and arguably cannot be, $\lambda$ being non-dynamical by construction). (ii) The continuum walls beyond the free-field sector: the \textbf{Haagerup natural-cone existence} in Mathlib (we state the Born rule on vector states; the canonical state$\mapsto$vector identification is cited) and the \textbf{interacting (non-free) case}. Both are cited, research-grade boundaries, not gaps in the account. In short: $\lambda$'s law is precise and verified where it can be; what is left is either \emph{provably irreducible} (the seed; the strong Born premise) or \emph{genuine multi-year mathematics} (natural-cone existence, interacting QFT).

\subsection{The *metaselector*: what fixes the record framework (a machine-checked no-go trilogy + einselection)}

§11.2b part 1 takes the record context $\mathfrak{A}=\operatorname{span}\{P_\alpha\}$ as \emph{given}. What fixes it — the "\textbf{metaselector}," the choice of abelian record framework — is itself a question, now resolved (Lean 4 / Mathlib, axiom-free, raw axiom count 0) as a \textbf{no-go trilogy plus a positive selector}, sharpening the division of labour of §11.3.

\textbf{Three things cannot select the framework.} (i) \emph{Capacity} — a holographic cardinality bound does not pick a realm (\texttt{Realm\allowbreak{}Selection.\allowbreak{}capacity\_\allowbreak{}underdetermines\_\allowbreak{}realm}: two distinct orthogonal $2$-record realms in $\mathbb{C}^2$ are equally capacity-maximal). (ii) \emph{Symmetry / typicality} — the unitary group acts \textbf{transitively} on frameworks (orthonormal bases), so \emph{any} unitarily-invariant typicality score is \textbf{constant} and selects none (\texttt{Symmetry\allowbreak{}No\allowbreak{}Go.\allowbreak{}unitary\_\allowbreak{}invariant\_\allowbreak{}score\_\allowbreak{}constant}, the continuous form of (i); \texttt{exists\_\allowbreak{}unitary\_\allowbreak{}map} exhibits the single orbit; the abstract principle \texttt{invariant\_\allowbreak{}of\_\allowbreak{}pretransitive\_\allowbreak{}constant} needs no Lean axioms at all). (iii) \emph{The state alone} — a single projection $P=|\Phi\rangle\langle\Phi|$ generates only the trivial framework $\{0,P,1-P,1\}$ (\texttt{State\allowbreak{}Alone\allowbreak{}No\allowbreak{}Go.\allowbreak{}state\_\allowbreak{}records\_\allowbreak{}trivial}: any idempotent $a\,P+b(1-P)$ has $a,b\in\{0,1\}$ — Bub–Clifton). So neither capacity, nor symmetry, nor $\Phi$ fixes the records.

\textbf{Einselection does.} The positive selector is Zurek's commutativity criterion, machine-checked: a record observable commuting with the monitored observable $A$ commutes with the interaction $A\otimes B$ (\texttt{Metaselector\allowbreak{}Selection.\allowbreak{}pointer\_\allowbreak{}commutes}, i.e. $[P,A]=0 \Rightarrow [P\otimes 1, A\otimes B]=0$), and an $A$-eigenstate stays a \emph{product} under the coupling (\texttt{pointer\_\allowbreak{}invariant}: $a\otimes e \mapsto a\otimes(\alpha\, Be)$ — decoherence-free, no entanglement generated). The record framework is therefore the spectral algebra of the \textbf{interaction Hamiltonian's} monitored observable: the metaselector is einselection (predictability sieve + Darwinist redundancy, the latter the \texttt{SBSSuppression} spectrum-broadcast structure), \emph{not} symmetry or capacity. This is conditional on a system–environment factorization, which remains a genuine residual (the Dowker–Kent set-selection / Heisenberg-cut problem; einselection is the empirically-correct but \emph{conditional} selector — a partial, not complete, answer).

\textbf{The record/area contract.} The bookkeeping around $\lambda$ is made category-error-proof in a labeled scaffold (\texttt{Record\allowbreak{}Contract}), separating four inputs — \emph{factorization} (cut) $\to$ \emph{einselection} (which framework) $\to$ \emph{area capacity} (how big) $\to$ \emph{$\lambda$} (which atom) — and threading them with proved bridges: coarse-graining is a capacity-non-increasing pushforward (\texttt{coarsen\_\allowbreak{}card\_\allowbreak{}le}), the record information obeys $H(R)\le\log|R|$ (Jensen on $-x\log x$, \texttt{shannon\_\allowbreak{}le\_\allowbreak{}log\_\allowbreak{}card}), and the holographic bound enters as a \textbf{one-sided capacity boundary condition} in \emph{two distinct versions kept separate} — the \textbf{entropy} form $H(R)\le S_{\text{area}}$ (Bousso, on the decohered record) and the strictly stronger \textbf{capacity} form $\log|R|\le S_{\text{area}}$ (a $\dim\mathcal{H}_{\text{rec}}\le e^{S_{\text{area}}}$ postulate, \emph{not} implied by Bousso, since a low-entropy law can have huge support). The contract rests on Born-\emph{from-projectors}: for a finite orthogonal PVM and normalized $\Phi$, $\mu(r)=\|P_r\Phi\|^2=\langle\Phi,P_r\Phi\rangle$ is a genuine record law (\texttt{Born\allowbreak{}Proj\allowbreak{}Bridge.\allowbreak{}born\allowbreak{}Record\allowbreak{}Law}), and the selector's event statistics \emph{are} this Born measure (\texttt{Record\allowbreak{}Contract.\allowbreak{}event\allowbreak{}Prob}). The one finite-budget effect that is a theorem: a $D$-dimensional record space cannot hold $M>D$ mutually orthogonal records (\texttt{finite\_\allowbreak{}budget\_\allowbreak{}forces\_\allowbreak{}overlap}) — demanding more forces a residual-interference floor (astronomically small for realistic horizon entropies). All postulates are carried as Lean \emph{hypotheses}, never axioms, so the budget stays $0$.

\textbf{Honest reach.} This is \emph{verified scaffold}, not new physics: it answers "what fixes the framework" (einselection) and fences off the shortcuts (capacity/symmetry/state), but $\lambda$ stays inert and the Born weights are assumed, so the scheme remains \textbf{operationally Everett} with zero empirical content (\texttt{everett\_\allowbreak{}equivalence}). In particular the tempting "select the most typical framework by symmetry, \emph{with physical impact}" route is \textbf{provably closed} (no-go (ii)): a non-dynamical Born-sampled $\lambda$ cannot have observable impact without \emph{becoming} an operational deviation from standard QM, whose only weakly-constrained home is the cosmological/holographic regime (an early-universe non-equilibrium deviation, e.g. a low-$\ell$ CMB $\xi_0\ne1$) — out of scope here, and not yet a theory. The pedagogical one/two/three-qubit "universes" used to \emph{illustrate} this layer (\texttt{Tiny\allowbreak{}Universe}, \texttt{Two\allowbreak{}Bit\allowbreak{}Universe}, \texttt{Three\allowbreak{}Qubit\allowbreak{}Universe}) are single-basis toys, explicitly caveated (their "entanglement" is single-basis correlation, not a context-crossing witness). Five GPT-5.5-pro red-team passes kept the framing honest; every claim was checked against the Lean before adoption.

\subsection{The central thesis, in one paragraph}

\begin{qiqtquote}
The Bekenstein-Bousso holographic bound, taken literally as a Lorentz-invariant physical information limit on bounded regions of spacetime, bounds the number of mutually \emph{distinguishable} macroscopic records a region can carry ($\# \le e^{Q_R}$). We are explicit that this is a \textbf{capacity (cardinality) bound, not a superposition-exclusion rule}: by linearity of the exactly-unitary dynamics it does \emph{not}, on its own, forbid a macroscopic superposition of records, the redundant zero-entropy cat state $\alpha|0\rangle^{\otimes N}+\beta|1\rangle^{\otimes N}$ is the obstruction (§7.6). Whether finite capacity plus einselection can be strengthened into a genuine macroscopic \emph{superselection} rule consistent with unitarity is the open Macroscopic Definiteness Conjecture (Open Problem 3); its strong "forbids superposition" form is currently \emph{implausible}, and the framework's single-outcome content does \textbf{not} depend on its resolution. The Schrödinger / Heisenberg evolution law of the underlying field algebra is left \textbf{exactly unitary and entirely standard} (including ordinary measurement interactions); no amplitude is trimmed and no Hamiltonian is restricted. \textbf{The per-run ontology is the pair $(\Phi, \lambda)$:} an exactly-unitarily-evolving global wavefunction $\Phi(t)$, together with a fixed \emph{non-dynamical, atemporal actuality selector} $\lambda$, a single global fact about which macroscopic realization the run is, fixed for the whole 4-dimensional history (not past-localized "initial data," to avoid any Conway–Kochen / setting-correlation reading; see §6.9). For each region $R$ the actual regional content is the derived beable $C_R(t) = A_R[\Phi(t), \lambda]$, the single $\lambda$-selected decoherent record sector of $\Phi(t)$; the unselected sectors remain in $\Phi(t)$ but are not regionally instantiated. The mechanism has a clean \textbf{division of labour}: decoherence removes interference and einselects stable, robust, effectively Boolean records (conserving the weights $|c_k|^2$; proved \emph{not} to select an outcome, \texttt{No\allowbreak{}Concentration.\allowbreak{}lean}), this gives robustness, not uniqueness; the capacity bound makes the set of distinguishable records \emph{finite}, cardinality, not selection; and \textbf{$\lambda$ supplies which single record is actual}, this is where outcome-uniqueness genuinely comes from. No collapse dynamics, no Bohmian particle-position ontology or guidance law, no Everettian parallel \emph{actual} branches (the unrealized records are not a parallel reality, they remain in $\Phi$ but are not regionally instantiated), and \textbf{no modification of the dynamics} are added; the only actuality rule is the fixed $\lambda / A_R$ reading rule, not a stochastic or modal dynamics. The framework is \textbf{ψ-monist in the weak (dynamical) sense}: $\Phi$ is the only thing that dynamically evolves and the only ontology subject to a law of motion, but the complete description of a run is the pair $(\Phi, \lambda)$, with $\lambda$ a non-dynamical actuality fact (a broad-sense beable, structurally analogous to the configuration in Bohmian mechanics or the actual history in modal interpretations, but with no guidance law and no back-reaction on $\Phi$). There are \textbf{no fundamental single-run probabilities}, no per-run stochastic or collapse chance is postulated; the Born weights $|c_k|^2$ are to be recovered, \emph{if at all}, as an emergent relative frequency across the $\lambda$-indexed ensemble of runs, conditional on a typicality measure $\mu$ over actuality selectors that is \textbf{currently unconstructed / interface-level} (the Born-typicality program, §7.4, §11.4, the framework's central open problem, together with the requirement that the same $\mu$ enforce operational no-signaling without fine-tuning, §6.9). The CPW Type II crossed-product algebra framework provides the rigorous mathematical infrastructure; the holographic capacity bound is the framework's new physical input on top. The single-record-per-region claim is therefore an \emph{architecture} in which decoherence + holography furnish a finite stage of robust classical records and $\lambda$ makes one actual, \textbf{a decomposition of the measurement problem into a robustness part, a finiteness part, and two clearly-stated primitives ($\lambda$ actuality, $\mu$ typicality), not a completed derivation of single outcomes from the bound alone.}
\end{qiqtquote}

\subsection{Open problems}

The Lean audit work (see \texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}} and the formal-verification footnote in the abstract) has sharpened the boundary between what the framework derives from FQ + AQFT + holography and what remains as load-bearing additional commitments. The open problems are reorganized here to reflect that sharpened boundary, with the Born / canonical-measure problem promoted to first position as the most central remaining commitment.

\textbf{Open Problem 1, Canonical IC Measure Principle (the central Born/typicality problem).}

This problem decomposes naturally into six layers, each requiring different kinds of work. The decomposition is based on a closing audit (\texttt{lean/\allowbreak{}mathlib/\allowbreak{}QIQTH/\allowbreak{}Axiom\allowbreak{}Audit.\allowbreak{}lean} + GPT-5.5-pro consultation) that distinguishes what is mathematically derivable from what is genuinely open physics from what is irreducible empirical calibration.
...
Machine-verified audits flag the difficulty: \texttt{No\allowbreak{}Born\allowbreak{}From\allowbreak{}Nothing.\allowbreak{}lean} (any distribution realizable; the structural axioms do not pick out Born), \texttt{Equivariance\allowbreak{}Gap.\allowbreak{}lean} (support preservation $\neq$ Born equivariance), \texttt{Operational\allowbreak{}No\allowbreak{}Go.\allowbreak{}lean} (operational frequency data alone insufficient). The independence package \texttt{Born\allowbreak{}Minimality\allowbreak{}Table.\allowbreak{}lean} re-exports these three countermodels and shows that the locality premise is \emph{not} an independent Born-selection sub-axiom, its marginal-level content is a theorem holding for any measure (\texttt{Marginal\allowbreak{}Locality.\allowbreak{}pushforward\_\allowbreak{}marginal\_\allowbreak{}local}), modulo a named Hilbert→set-level bridge axiom (§11.4.5). Relative to the current finite formal decomposition, the Canonical IC Measure Principle's irreducible Born-selection content is three named sub-axioms (canonical measure, equivariance, operational sufficiency), each provably necessary by concrete finite countermodel, alongside the two acknowledged operator-algebra interface axioms (Schur classification, tensor multiplicativity) that the Born representation itself rests on.
...
The conditional Born typicality theorem (\texttt{Born\allowbreak{}Typicality.\allowbreak{}lean}) requires three additional physical inputs:

\begin{itemize}
  \item \textbf{Equivariance theorem.} If $\rho_t = U_t \rho_0 U_t^*$ under FQ-restricted unitary dynamics, then $\mu_{\rho_t}(p) = \mu_{\rho_0}(U_t^* p U_t)$. For QFT-style Hamiltonians, this requires careful treatment of domain / self-adjointness issues. The \texttt{Equivariance\allowbreak{}Gap.\allowbreak{}lean} audit establishes that support preservation (the framework's current claim) is strictly weaker than measure preservation.
  \item \textbf{Projection locality.} Need precise assumptions on the local algebraic structure: tensor product factorisation (or its absence in Type III), commuting local algebras (microcausality), split property, finite corners. The \texttt{Compression\allowbreak{}Locality.\allowbreak{}lean} audit isolates the compression-locality leakage identity and identifies when the FQ projection preserves microcausality at the restricted level. \emph{Post A1 strengthening} (\texttt{Marginal\allowbreak{}Locality.\allowbreak{}pushforward\_\allowbreak{}marginal\_\allowbreak{}local}): given set-level locality of Bob's dynamics ($r\circ T = r$), the Alice-marginal is invariant for \textbf{any} IC measure: with no equivariance assumption, so locality of the marginal is a theorem, not a separate Born-selection postulate. This reduction is, however, \emph{conditional} on a named Hilbert→set-level locality bridge axiom (\texttt{set\_\allowbreak{}level\_\allowbreak{}locality\_\allowbreak{}from\_\allowbreak{}unitary\_\allowbreak{}dilation}, a standard Heisenberg↔Schrödinger correspondence): locality has been relocated into that explicit bridge, not eliminated. Subject to that caveat, locality is no longer counted among the independent Born-selection sub-axioms of the Canonical IC Measure Principle.
  \item \textbf{Empirical frequency bridge.} If μ is interpreted as typicality (rather than primitive probability), one needs a DGZ-style step from typical initial configurations to observed frequencies. The \texttt{Born\allowbreak{}Typicality.\allowbreak{}lean} module formalizes this conditionally via standard finite LLN.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Frequency concentration (PROVED, single-trial + independence step).} The Lean module \texttt{Born\allowbreak{}Concentration.\allowbreak{}lean} proves Chebyshev's tail bound on finite probability spaces ($\Pr(|X-\mu| \ge \varepsilon) \le \mathrm{Var}/\varepsilon^2$), computes the Bernoulli variance ($p(1-p)$), proves the variance-addition (independence) lemma \texttt{variance\_\allowbreak{}add\_\allowbreak{}of\_\allowbreak{}product} (variance of a sum of independent variables equals the sum of variances), and combines these to give the single-trial concentration inequality and the two-trial Bernoulli variance $2p(1-p)$. \emph{What is proved:} the single-trial Chebyshev bound and the reusable variance-addition lemma the $N$-fold induction would iterate. \emph{What remains axiomatic:} the full $N$-trial product-measure scaling $\Pr(|\mathrm{freq}_N - p| \ge \varepsilon) \le p(1-p)/(N\varepsilon^2)$ rests on the LLN interface axiom in \texttt{Born\allowbreak{}Typicality.\allowbreak{}lean}. Accordingly the framework's frequency statement ("Born \emph{frequencies}", not merely "Born \emph{means}") is established up to that single named scaling axiom: for which the per-trial variance and the independence step are now both in hand.
...
Per GPT-5.5-pro's sixth audit, the honest Born claim is not "Born is derived from holography alone", \texttt{No\allowbreak{}Born\allowbreak{}From\allowbreak{}Nothing} refutes that, but rather:
...
The independence package \texttt{Born\allowbreak{}Minimality\allowbreak{}Table.\allowbreak{}lean} makes this explicit. Relative to the current finite formal decomposition, the Canonical IC Measure Principle has three irreducible Born-selection sub-axioms, each with a concrete finite countermodel witnessing its necessity (with the locality premise reducible, modulo the named bridge axiom of §11.4.5):
...
\hline
P1 & Canonical IC measure (some structurally-distinguished $\mu$ is selected) & \texttt{No\allowbreak{}Born\allowbreak{}From\allowbreak{}Nothing.\allowbreak{}any\_\allowbreak{}anti\_\allowbreak{}born\_\allowbreak{}realizable} & $\Gamma$ arbitrary finite \\
\hline
P2 & Measure equivariance under FQ-restricted dynamics & \texttt{Equivariance\allowbreak{}Gap.\allowbreak{}support\_\allowbreak{}preservation\_\allowbreak{}does\_\allowbreak{}not\_\allowbreak{}imply\_\allowbreak{}…} & $\mathrm{Fin}\,2$ swap \\
\hline
P3 & Operational sufficiency (click-statistics determine IC marginal) & \texttt{Operational\allowbreak{}No\allowbreak{}Go.\allowbreak{}operational\_\allowbreak{}data\_\allowbreak{}insufficient} & $\mathrm{Fin}\,3 \to \mathrm{Fin}\,2$ \\
\hline
P4 & Locality of Alice's marginal under Bob's local dynamics & \textbf{REDUCIBLE}, proved as a theorem holding for \emph{any} measure (\texttt{Marginal\allowbreak{}Locality.\allowbreak{}pushforward\_\allowbreak{}marginal\_\allowbreak{}local}), conditional on the named Hilbert→set-level bridge axiom (§11.4.5) & (not an independent Born-selection axiom) \\
\hline
\end{tabularx}
\end{center}
...
\hline
11.4.6 & Born \textbf{frequency} concentration (single-trial Chebyshev) & \textbf{PROVED} in \texttt{Born\allowbreak{}Concentration.\allowbreak{}born\_\allowbreak{}chebyshev\_\allowbreak{}single\_\allowbreak{}trial}; $N$-trial scaling at LLN axiom interface \\
\hline
11.4.7 & Independence/minimality of P1, P2, P3 by finite countermodel & \textbf{PROVED} in \texttt{Born\allowbreak{}Minimality\allowbreak{}Table.\allowbreak{}lean} \\
\hline
11.4.7 & Reducibility of locality (P4): marginal-locality step proved for \emph{any} measure & \textbf{PROVED} in \texttt{Marginal\allowbreak{}Locality.\allowbreak{}pushforward\_\allowbreak{}marginal\_\allowbreak{}local}; reduction conditional on the named Hilbert→set bridge axiom \\
\hline
\end{tabularx}
\end{center}
...
\emph{Formalization note (Lean), now partially carried out.} The linchpin's discrete skeleton is more amenable to machine verification than its analytic core, and the project's existing strategy, prove the order-theoretic / combinatorial scaffold, name the functional-analytic inputs as interface axioms (cf. \texttt{Axiom\allowbreak{}Audit.\allowbreak{}lean}), applies directly. Four Lean modules now realize this, all compiling and axiom-audited (full build $\approx$ 2994 jobs; standard axioms only on every headline theorem):

\begin{itemize}
  \item \textbf{\texttt{Lorentz\allowbreak{}Selection.\allowbreak{}lean}}: the poset/sheaf layer: the causal-diamond poset $\mathsf{Diam}$, the record-sector presheaf $X_\Phi$ with functorial restriction, the global section $\lambda \in \Gamma(X_\Phi)$, \texttt{bulk\_\allowbreak{}overlap\_\allowbreak{}agreement} (Correction A; \emph{no axioms}), and the \textbf{evaluation-gives-covariance theorem} $A_{gD}[U_g\Phi,g\lambda] = g\cdot A_D[\Phi,\lambda]$, proved from equivariant naturality, depending on standard axioms only and on \emph{none} of the (now-retired) AQFT axioms. The Poincaré action is an \texttt{Order\allowbreak{}Iso} of diamonds; this makes the pushed-forward section's consistency a \emph{proved theorem} (\texttt{act\allowbreak{}Section\_\allowbreak{}consistent}) rather than an axiom, the earlier transport-bookkeeping axiom was thereby eliminated. The four AQFT analytic inputs are no longer opaque axioms but an explicit \texttt{Recorded\allowbreak{}History\allowbreak{}Net} structure (holographic finiteness bound, boundary-reconstruction natural iso, decoherence-functional probability weights, projective/no-signaling marginal), over which the covariance result becomes the conditional theorems \texttt{covariant\_\allowbreak{}selection\_\allowbreak{}of\_\allowbreak{}net} / \texttt{covariant\_\allowbreak{}selection\_\allowbreak{}exists} and no-signaling becomes the theorem \texttt{net\_\allowbreak{}no\_\allowbreak{}signaling}, leaving the module \textbf{axiom-free}.
  \item \textbf{\texttt{Finite\allowbreak{}Modular\allowbreak{}Theory.\allowbreak{}lean}}: the finite-matrix modular engine: the inner conjugation automorphism, the KMS \emph{boundary} identity $\omega(xy)=\omega(y\,\sigma(x))$ from trace cyclicity, and, the genuine modular \emph{data}, the \textbf{real-time modular flow} $\sigma_t(x)=\rho^{it}x\rho^{-it}$ in the diagonal case, proved to be a real one-parameter group ($\sigma_s\circ\sigma_t=\sigma_{s+t}$). This is honestly the \emph{finite Type-I} modular skeleton, not the continuum Tomita–Takesaki data (GNS $\Delta$, $J$, $S=J\Delta^{1/2}$, the implementation theorem, KMS strip-analyticity remain future work).
  \item \textbf{\texttt{Free\allowbreak{}Field\allowbreak{}Record.\allowbreak{}lean}}: the free-field finite-mode instance: the holographic record count $\log_2 \#\mathrm{Atoms} = N \le Q_R$; the \textbf{decoherence factorization} $\langle E_\alpha|E_\beta\rangle = \prod_i \langle E_{\alpha_i}|E_{\beta_i}\rangle$ with the \emph{derived} bound $|\langle E_\alpha|E_\beta\rangle| \le q^{\,d_H(\alpha,\beta)} \to 0$ (Gaussian/Wick suppression, derived from the mode product rather than postulated); and the finite-mode Lorentz action on record sectors (a one-parameter group of bijections, capacity-covariant).
  \item \textbf{\texttt{Lorentz\allowbreak{}Selection\allowbreak{}Strong.\allowbreak{}lean}}: the \emph{strengthening} pass that answers the adversarial-review caveats (i)–(ii) above with genuine content (the review's items A–G), keeping the interface \emph{conditional} but making it \textbf{rigid} and \textbf{connected} rather than vacuous, and still axiom-free: \textbf{(A)} an \emph{externalized} \texttt{Geometry\allowbreak{}Spec} fixes the area-law budget $N$ and the boundary presheaf \emph{before} a net is chosen, so the holographic bound and reconstruction can no longer be satisfied by the cheap $N:=\#X_\Phi$, $P_b:=P$ choices (consuming theorems: \texttt{card\_\allowbreak{}le\_\allowbreak{}of\_\allowbreak{}le}, using holographic monotonicity, and \texttt{recon\allowbreak{}Section}, the screen-encoded selected history as a consistent boundary section); \textbf{(B)} a genuine Poincaré \emph{group} action \texttt{Group\allowbreak{}Action G P} with identity/composition laws, giving covariance for \emph{every} $g$ (\texttt{group\_\allowbreak{}evaluation\_\allowbreak{}covariance}) and law-consuming identities (\texttt{act\_\allowbreak{}one\_\allowbreak{}diam}, \texttt{act\_\allowbreak{}mul\_\allowbreak{}diam}), replacing the single order-automorphism; \textbf{(C)} \texttt{measure\_\allowbreak{}pushforward\_\allowbreak{}total}, a theorem that \emph{consumes} the measure-covariance hypothesis (a $g$-covariant weight has $g$-invariant total mass, by reindexing along the sector equivalence), the measure-level content the bare covariance theorem ignored; and \textbf{(G)} the \textbf{Born link} (the anti-vacuity lock): a \texttt{Born\allowbreak{}Data} pins the weights to Born values $\omega_\Phi(P_i^D)=\langle\psi_D|E_x^D|\psi_D\rangle$ of an actual unit state on a resolution of identity, whence \emph{normalization is a theorem} (\texttt{bornω\_\allowbreak{}sum\_\allowbreak{}one} / \texttt{bornωRe\_\allowbreak{}sum\_\allowbreak{}one}: $\sum_x\omega = \langle\psi|\,{\textstyle\sum_x}E_x\,|\psi\rangle = \langle\psi|\psi\rangle = 1$) derived from the already-discharged Born functional (\texttt{Gleason\allowbreak{}Selector.\allowbreak{}born\_\allowbreak{}add}/\texttt{born\_\allowbreak{}one}), tying the Lorentz strand to the axiom-free Gleason strand. A second pass (after a follow-up adversarial review) closes the three remaining gaps that review named: \textbf{(probabilities, not just affine normalization)} a \texttt{PVMData} adds the projection hypotheses $E_x^\dagger=E_x$, $E_x^2=E_x$ that \texttt{Born\allowbreak{}Data} lacked, whence \texttt{born\_\allowbreak{}pos\allowbreak{}Semidef\_\allowbreak{}nonneg} ($\langle\psi|E|\psi\rangle\ge 0$ for positive-semidefinite $E$, via \texttt{Matrix.\allowbreak{}Pos\allowbreak{}Semidef.\allowbreak{}dot\allowbreak{}Product\_\allowbreak{}mul\allowbreak{}Vec\_\allowbreak{}nonneg}) gives \texttt{pvm\_\allowbreak{}bornωRe\_\allowbreak{}nonneg} and \texttt{pvm\_\allowbreak{}bornω\_\allowbreak{}im\_\allowbreak{}zero}, so \texttt{pvm\_\allowbreak{}is\allowbreak{}Probability} certifies the weights are a genuine finite probability distribution (nonnegative \emph{and} summing to $1$), not the signed/complex "weights" the bare $\sum_x E_x=1$ would have permitted; \textbf{(a real representation)} the γ-cocycle laws \texttt{Is\allowbreak{}Rep\allowbreak{}One}/\texttt{Is\allowbreak{}Rep\allowbreak{}Mul} (with the fibre transport \texttt{fib\allowbreak{}Cast}) upgrade the family $\{\gamma_g\}$ from a per-element natural equivalence to a genuine action on the record fibres, with \texttt{γ\_\allowbreak{}cocycle\_\allowbreak{}apply} ($\gamma_{g_1 g_2}=\gamma_{g_2}\circ\gamma_{g_1}$ up to the diamond cast) as the consuming theorem; and \textbf{(per-cell covariance)} \texttt{measure\_\allowbreak{}pushforward\_\allowbreak{}cell} strengthens the total-mass statement to per-sector equality $\omega_{gD}(y)=\omega_D(\gamma_g^{-1}y)$. A third pass then \textbf{discharges the measure-covariance hypothesis itself} (\texttt{hcov}, previously the marquee \emph{assumption}): the unconditional theorem \texttt{born\_\allowbreak{}unitary\_\allowbreak{}invariant} ($\langle U\psi|UEU^\dagger|U\psi\rangle=\langle\psi|E|\psi\rangle$ for $U^\dagger U=1$, proved by the adjoint identities \texttt{star\_\allowbreak{}mul\allowbreak{}Vec}/\texttt{dot\allowbreak{}Product\_\allowbreak{}mul\allowbreak{}Vec}/\texttt{vec\allowbreak{}Mul\_\allowbreak{}vec\allowbreak{}Mul}) is the engine; a \texttt{Unitary\allowbreak{}Covariance} records the \emph{lower-level} equivariance, the state transforms $\psi_{gD}=U_{g,D}\psi_D$ and the effects conjugate $E_{gD}(\gamma_g x)=U_{g,D}E_x^D U_{g,D}^\dagger$, from which \texttt{ubornω\_\allowbreak{}covariant} \emph{derives} $\omega_{gD}(\gamma_g x)=\omega_D(x)$ as a \textbf{theorem}, and \texttt{ubornω\_\allowbreak{}pushforward\_\allowbreak{}cell} / \texttt{ubornω\_\allowbreak{}total\_\allowbreak{}invariant} then make the per-cell and total-mass covariances \textbf{unconditional on \texttt{hcov}}. This converts caveats (i)–(ii) from "the analytic fields are ignored / non-rigid" into "the fields are externally rigid, consumed by theorems, the weights are certified probabilities, and their Poincaré covariance is \emph{derived from unitarity} rather than assumed." A fourth pass then threads the γ-cocycle onto the \emph{selection itself}, \texttt{selection\_\allowbreak{}cocycle} proves $\gamma_{g_1 g_2}(\lambda_D)=\gamma_{g_2}\big(\text{selector}(g_1\!\cdot\!\lambda)(g_1 D)\big)$ (up to the diamond cast), i.e. multi-step transformed selections agree with the one-step transform, consuming both \texttt{Is\allowbreak{}Rep\allowbreak{}Mul} and \texttt{group\_\allowbreak{}evaluation\_\allowbreak{}covariance}, and fuses the two strands into a single object: \texttt{Uniform\allowbreak{}PVMData} carries the projection hypotheses \emph{and} the uniform-dimension Born data, and \texttt{upvm\_\allowbreak{}covariant\_\allowbreak{}probability} certifies the weights on each diamond are simultaneously (i) nonnegative, (ii) summing to $1$, and (iii) Poincaré-covariant $\omega_{gD}(\gamma_g x)=\omega_D(x)$, a genuine \textbf{covariant probability distribution} whose covariance is \emph{derived}. A coherence check is also discharged as a theorem: \texttt{proj\_\allowbreak{}conj\_\allowbreak{}unitary} (unitary conjugation preserves a Hermitian projection) gives \texttt{E\_\allowbreak{}cov\_\allowbreak{}preserves\_\allowbreak{}proj}, the boosted effects $E_{gD}(\gamma_g x)=U E_x^D U^\dagger$ remain a PVM, so \texttt{Unitary\allowbreak{}Covariance} over a \texttt{Uniform\allowbreak{}PVMData} is internally consistent (not over-determined); and \texttt{covariant\allowbreak{}Probability\_\allowbreak{}of\_\allowbreak{}unitary\allowbreak{}PVM} packages the three facts as a \texttt{Covariant\allowbreak{}Probability} bundle. A final pass discharges the two remaining reachable in-interface statements. \textbf{(Section-object group action)} \texttt{Lorentz\allowbreak{}Selection} now exposes a public spec \texttt{act\allowbreak{}Section\_\allowbreak{}val} (the \texttt{act\allowbreak{}Val} field is private) plus \texttt{Global\allowbreak{}Section.\allowbreak{}ext} and \texttt{Global\allowbreak{}Section.\allowbreak{}val\_\allowbreak{}cast}; on top of these \texttt{act\allowbreak{}Section\_\allowbreak{}one} ($1\cdot\lambda=\lambda$) and \texttt{act\allowbreak{}Section\_\allowbreak{}mul} ($(g_1 g_2)\cdot\lambda = g_2\cdot(g_1\cdot\lambda)$) are \emph{proved}, so \texttt{act\allowbreak{}Section} is a genuine \textbf{group action on $\Gamma(X)$}, of which the earlier selector-level \texttt{selection\_\allowbreak{}cocycle} was only the evaluation shadow. The transport casts are handled cleanly by \texttt{act\allowbreak{}Section\_\allowbreak{}val\_\allowbreak{}act'} (the cast-free evaluation $(g\cdot\lambda)(gD)=\gamma_g(\lambda_D)$), \texttt{val\_\allowbreak{}cast}, and \texttt{fib\allowbreak{}Cast\_\allowbreak{}symm\_\allowbreak{}cast\allowbreak{}Sector}, no raw \texttt{HEq}, the route the adversarial review outlined. \textbf{(Full PVM preservation)} \texttt{unitary\_\allowbreak{}preserves\_\allowbreak{}resolution} proves $U(\sum_x E_x)U^\dagger = U\,U^\dagger = 1$ (using $UU^\dagger=1$ derived from the finite-square $U^\dagger U=1$), so the unitary transport preserves the \emph{resolution of identity}, not just individual projections, the boosted effects are a \emph{full} PVM. What remains genuinely open is now only: the boundary-action/reconstruction compatibility, and, the principal gap, the continuum \emph{realization} (existence of such a net, with its unitary Poincaré transport, from a genuine relativistic QFT: the Type III$_1$ / Tomita–Takesaki problem). Every reachable assumption-to-theorem conversion in the finite conditional interface is complete: the interface is rigid, connected, a \emph{covariant probability distribution} with covariance derived from unitarity, coherent under transport (a full PVM preserved by the action), and carries a genuine group action on its global sections; its \emph{instantiation} from QFT is the open problem.
  \item \textbf{\texttt{Lorentz\allowbreak{}Witness.\allowbreak{}lean}}: a \textbf{concrete non-trivial model} of the strengthened interface, settling the \emph{vacuity} question the review raised (the structure also admits the trivial one-point net, so "a model exists" alone is empty). Over a single causal diamond with a two-element record fibre \texttt{Fin 2}, a two-dimensional Hilbert space, the rational unit state $\psi=(3/5,4/5)$, and the diagonal projective measurement, it builds a full \texttt{Uniform\allowbreak{}PVMData} + \texttt{Unitary\allowbreak{}Covariance} and hence (via \texttt{covariant\allowbreak{}Probability\_\allowbreak{}of\_\allowbreak{}unitary\allowbreak{}PVM}) a \texttt{Covariant\allowbreak{}Probability}, with \texttt{witness\_\allowbreak{}nondegenerate} proving the Born weights are $\omega(0)=9/25,\ \omega(1)=16/25 \in (0,1)$, a genuinely \emph{spread} two-outcome distribution rather than a point mass, and \texttt{witness\_\allowbreak{}fibre\_\allowbreak{}card} confirming the fibre has two elements (not one). So the conditional interface has real content: it is satisfiable by non-degenerate models, not only the trivial net (standard Lean axioms only). A second witness exercises the \emph{covariance} machinery on a genuine orbit: two causal diamonds with the indiscrete preorder (so every permutation is an order-isomorphism), the group $G=\mathrm{Perm}$ permuting them, and homogeneous PVM data, \texttt{witness2\_\allowbreak{}covariant\allowbreak{}Probability} gives a \texttt{Covariant\allowbreak{}Probability} over this non-trivial group, and \texttt{witness2\_\allowbreak{}action\_\allowbreak{}nontrivial} proves the swap moves diamond $d_0$ to $d_1$ (the action is genuinely non-trivial, unlike Witness A's trivial group). \emph{Honest scope:} these are finite, rational (no $\sqrt 2$), homogeneous models that prove the interface is non-vacuous and its covariance applies to a non-trivial orbit; the continuum QFT realization (Type III$_1$ / Tomita–Takesaki) remains the genuine open content, untouched.
\end{itemize}

What was previously deferred to \emph{four opaque interface axioms} in \texttt{Lorentz\allowbreak{}Selection.\allowbreak{}lean}, corresponding to the existence of the finite record algebras with the holographic bound (linchpin hypotheses 1–3), the boundary-reconstruction isomorphism (2), the Born/decoherence-functional measure with its Kolmogorov consistency and $\sigma$-additive extension (6), and the no-signaling marginal lemma (9), has been \textbf{removed from the axiom budget}: those four \texttt{axiom \_\allowbreak{} : Prop} placeholders (which asserted nothing and were used by no theorem) are \textbf{retired} and replaced by an explicit \texttt{Recorded\allowbreak{}History\allowbreak{}Net} structure that \emph{writes the content down} as type-checked data and propositions (a holographic finiteness/cardinality bound, a boundary-reconstruction natural isomorphism commuting with restriction, decoherence-functional probability weights, and the projective/no-signaling marginal). The covariance result is then a \emph{conditional theorem over this structure} (\texttt{covariant\_\allowbreak{}selection\_\allowbreak{}of\_\allowbreak{}net}, \texttt{covariant\_\allowbreak{}selection\_\allowbreak{}exists}), and the projective marginal yields the convenience theorem \texttt{net\_\allowbreak{}no\_\allowbreak{}signaling} (the marginal onto a sub-diamond $K$ is the same regardless of which larger diamond $D\supseteq K$ it is computed from). The whole \texttt{Lorentz\allowbreak{}Selection} module now adds \textbf{zero project axioms} (every headline theorem depends on the standard Lean axioms only, per \texttt{Axiom\allowbreak{}Audit.\allowbreak{}lean}; the project-wide raw axiom count drops $44\to 40$).

\emph{What this does and does not establish (adversarial-review caveat, GPT-5.5-pro).} This is the corpus's \emph{interface-as-hypothesis, not axiom} discipline applied to the AQFT inputs, and as such it is a genuine improvement in formal hygiene, but it must not be overstated, and three honest limitations are recorded here. \textbf{(i) The covariance theorem is evaluation-equivariance, not covariance of the physics.} \texttt{covariant\_\allowbreak{}selection\_\allowbreak{}of\_\allowbreak{}net} consumes only the presheaf field \texttt{net.\allowbreak{}P}; its proof is the structural identity \texttt{evaluation\_\allowbreak{}covariance}, and the analytic fields (\texttt{N}, \texttt{recon}, \texttt{ω}, \texttt{ω\_\allowbreak{}marg}) play no role in it, so what is machine-checked is "a transformed global section, evaluated at the transformed diamond, equals the transform of the evaluation," not covariance of the decoherence measure or of a \emph{fixed} selection rule. \textbf{(ii) Several fields are non-rigid as written, so the structure admits degenerate models.} Because \texttt{N}, \texttt{Pb}/\texttt{recon}, and \texttt{ω} are chosen \emph{within} the same structure, the holographic bound is satisfiable by taking $N(D) := \#X_\Phi(D)$, boundary reconstruction by $P_b := P,\ \mathrm{recon} := \mathrm{id}$, and the weights by any finite PMF; in particular the one-point net ($X_\Phi(D) := \mathbf 1$) satisfies every field. **Consequently the bare existence statement "a \texttt{Recorded\allowbreak{}History\allowbreak{}Net} exists" is \emph{not} the open problem, it is trivially true.** The genuine Open Problem 3b is the \emph{realization} problem: existence of a net whose data are \emph{extracted from a fixed relativistic QFT and geometry}, the area-law $N(D)=\lfloor e^{Q_D}\rfloor$ and boundary algebra fixed externally, and the weights pinned to Born values $\omega_\Phi(P_i^D)$ of an actual global state $\Phi$, together with a genuine Poincaré \emph{group} action (the current \texttt{Poincare\allowbreak{}Action} is a single order-automorphism, not a representation) under which the measure is equivariant. \textbf{(iii) \texttt{net\_\allowbreak{}no\_\allowbreak{}signaling} is a two-line rewrite of the assumed marginal}, not a derivation of operational no-signaling from microcausality; the real content remains the \emph{assumption} \texttt{ω\_\allowbreak{}marg}. The honest status, then: the four opaque axioms are gone and the finite combinatorial skeleton (functorial restriction, global sections, evaluation-equivariance, the finite Type-I modular flow, and the free-field finite-mode record structure) is machine-checked and axiom-free; but the substantive Lorentz-covariance content, Born-pinned covariant measure, externally-rigid holographic/boundary data, a true group action, and the Type III$_1$ / Tomita–Takesaki / Haagerup-$L^p$ \emph{realization}, remains open, and is the agenda for the next formalization pass (§11.4 strengthening targets: externalize the geometry spec, replace the single automorphism by a group action with equivariant weights, and tie $\omega$ to the already-formalized Born functional).

\textbf{Open Problem 4, Quantitative form of $\epsilon(R)$.}
The (FQ)(iii) resolution floor satisfies $\epsilon(R) > 0$ (Theorem 3; formalized in \texttt{Resolution.\allowbreak{}lean}). Derive an explicit functional form $\epsilon(R) = f(\text{geometry}, Q_R, \text{macroscopic record dimension})$ tying the framework to numerical predictions. Downstream of Open Problems 2 and 3.
...
\textbf{Strategic note.} Of the central open problems, the audit work suggests two bottleneck chains:
  ...
  • \textbf{Foundations bottleneck:} Open Problem 1 (canonical $\mu$-selection / equivariance). Now decomposed into three sub-theorems A, B, C (see above), with the A1+A2+A4+A6 strengthening pass further narrowing the load-bearing content. Two of the three sub-theorems are direct applications of existing operator-algebra machinery (Mackey-Gleason + noncommutative Radon-Nikodym); the third requires adapting the Goldstein-Struyve 2007 uniqueness theorem from Bohmian dynamics to QIQT-H's Type II / (FQ) setting. Concrete progress: the permutation-symmetry collapse component of Goldstein-Struyve Step 1 sub-lemma 1c is \textbf{PROVED} under its necessary hypothesis (\texttt{step1c\_\allowbreak{}collapse\_\allowbreak{}of\_\allowbreak{}perm\_\allowbreak{}symmetric}) along with the foundational matrix-conjugation identities (\texttt{permutation\_\allowbreak{}conj\_\allowbreak{}matrix\allowbreak{}Unit}, \texttt{diagonal\allowbreak{}U\_\allowbreak{}conj\_\allowbreak{}matrix\allowbreak{}Unit}), while the full Schur classification remains a single named interface axiom; the marginal-locality step is \textbf{PROVED} as a theorem holding for any measure (\texttt{Marginal\allowbreak{}Locality.\allowbreak{}pushforward\_\allowbreak{}marginal\_\allowbreak{}local}), reducing locality to a named Hilbert→set-level bridge axiom rather than eliminating it; single-trial Chebyshev frequency concentration and the variance-addition (independence) lemma are \textbf{PROVED} (\texttt{Born\allowbreak{}Concentration}), with the $N$-trial scaling still on the LLN axiom; and the duplicate axiomatization of Goldstein-Struyve sub-steps in \texttt{FQEquivariance\allowbreak{}Uniqueness} is \textbf{CONSOLIDATED} to route through the concrete \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Fin\allowbreak{}Dim} proof (\texttt{canonical\_\allowbreak{}ic\_\allowbreak{}measure\_\allowbreak{}principle} now depends on exactly two project-specific axioms, down from nine), with the five false/unused Step-1 placeholder sub-axioms deleted (project axiom total 57 → 40 — \emph{and subsequently all the way to budget 0; the development is now entirely axiom-free, see §11.2a/§11.2b, and $\lambda$'s selection-and-consistency schema is machine-checked through the free-field level — conditional on a primitive seed and a Born premise}). Progress on the remaining load-bearing axioms transforms the Born section from conditional to substantive, and is the most pressing problem for foundations-of-QM defensibility.
...
\begin{qiqtquote}
\textbf{Note (2026-06-15/16):} the per-row "named interface axiom" / project-axiom labels in this matrix are a \emph{historical snapshot} of the strengthening pass. The development has since been driven to \textbf{budget 0 — entirely axiom-free} (every theorem depends only on Lean's standard \texttt{propext}, \texttt{Classical.\allowbreak{}choice}, \texttt{Quot.\allowbreak{}sound}), and $\lambda$'s selection-and-consistency schema (the Takesaki kinematic criterion, modular-invariance, Born-weight bookkeeping, inverse-CDF selection) is machine-checked at the finite, one-particle continuum, and second-quantized free-field levels (§11.2b) — conditional on a primitive seed and a Born premise; "modular-invariance" is \emph{not} physical irreversibility. Read the axiom-count cells as "as of the strengthening pass", superseded by the axiom-free milestone.
\end{qiqtquote}

The following table maps each major paper claim to its Lean theorem, with explicit status. Status labels:

  • \textbf{U}: unconditional (no project-specific axioms; depends only on standard Lean axioms \texttt{propext}, \texttt{Classical.\allowbreak{}choice}, \texttt{Quot.\allowbreak{}sound}).
...
\hline
Theorem 3 (resolution floor) & \texttt{Resolution.\allowbreak{}eps\_\allowbreak{}pos} & U & $\varepsilon(R) > 0$ from finite $Q_R$ \\
\hline
Lemma 1 (near-extreme indistinguishability) & \texttt{Resolution.\allowbreak{}lemma1\_\allowbreak{}zero} & U & Discrete-bin formalization \\
\hline
Theorem 6 (effective definiteness) & \texttt{Theorem6.\allowbreak{}effective\_\allowbreak{}definiteness} & U & Donald-bound chain \\
\hline
Theorem 6 inner step & \texttt{Theorem6.\allowbreak{}Branch\allowbreak{}Data.\allowbreak{}holevo\_\allowbreak{}le\_\allowbreak{}capacity} & U & $I_{\rm Hol}^R \le C(R)$ from $\chi_R \le C(R)$ and Klein \\
\hline
Theorem 7 (no-signaling) & \texttt{Theorem7.\allowbreak{}Setup.\allowbreak{}no\_\allowbreak{}signaling} & U & From microcausality + locality \\
\hline
Donald's identity & \texttt{Donald.\allowbreak{}donald\_\allowbreak{}identity} & C & Conditional on 3 cross-entropy axioms (A1, A2, A3) \\
\hline
Microcausality ⇒ locality (unitary) & \texttt{Unitarity\allowbreak{}Locality.\allowbreak{}locality\_\allowbreak{}of\_\allowbreak{}conjugation} & U & StarRing argument \\
\hline
Microcausality ⇒ locality (Kraus) & \texttt{Kraus\allowbreak{}Locality.\allowbreak{}kraus\_\allowbreak{}channel\_\allowbreak{}alice\allowbreak{}Fixing} & U & Generalization to CPTP channels \\
\hline
Bell/CHSH inequality (LHV bound) & \texttt{Bell.\allowbreak{}LHVModel.\allowbreak{}chsh\_\allowbreak{}le\_\allowbreak{}two} & U & Finite probability + ±1 algebra \\
\hline
Tsirelson saturation by singlet & \texttt{Tsirelson.\allowbreak{}singlet\_\allowbreak{}chsh\_\allowbreak{}abs\_\allowbreak{}gt\_\allowbreak{}two} & U & Explicit 4D real construction; attains $2\sqrt{2}$ exactly (does \emph{not} prove the universal upper bound) \\
\hline
(H1) ⇏ (H2) & \texttt{H1\allowbreak{}H2\allowbreak{}Audit.\allowbreak{}H1\_\allowbreak{}does\_\allowbreak{}not\_\allowbreak{}imply\_\allowbreak{}H2} & N & Classical KL countermodel \\
\hline
(H2) ⇔ reference-weight bound & \texttt{H1\allowbreak{}H2\allowbreak{}Audit.\allowbreak{}H2\_\allowbreak{}iff\_\allowbreak{}reference\_\allowbreak{}weight} & U & Sharp structural reformulation \\
\hline
Decoherence ⇏ concentration & \texttt{No\allowbreak{}Concentration.\allowbreak{}audit\_\allowbreak{}conclusion} & N & Linear unitary preserves branch weights \\
\hline
$S_{\rm ren}$ (paper) $\neq$ $S_{\rm ren}^{\rm CPW}$ & \texttt{Entropy\allowbreak{}Bridge.\allowbreak{}fq\_\allowbreak{}ambiguity\_\allowbreak{}counterexample} & N & Classical KL counterexample distinguishing the two \\
\hline
Branch-summed cost > Shannon possible & \texttt{Branch\allowbreak{}Ledger.\allowbreak{}branch\allowbreak{}Summed\_\allowbreak{}not\_\allowbreak{}bounded\_\allowbreak{}by\_\allowbreak{}Shannon} & N & Binary uniform witness \\
\hline
FQ-literal finite ⇒ trivial dynamics & \texttt{FQDynamics\allowbreak{}No\allowbreak{}Go.\allowbreak{}finite\_\allowbreak{}admissible\_\allowbreak{}flow\_\allowbreak{}fixed} & U & Continuous flow on finite T2 space (only, not arbitrary finite-info dynamics) \\
\hline
Compression-locality leakage identity & \texttt{Compression\allowbreak{}Locality.\allowbreak{}compressed\_\allowbreak{}commutator\_\allowbreak{}with\_\allowbreak{}commute} & U & Pure ring algebra \\
\hline
Any $p$ realizable (no Born from nothing) & \texttt{No\allowbreak{}Born\allowbreak{}From\allowbreak{}Nothing.\allowbreak{}exists\_\allowbreak{}probability\_\allowbreak{}realizing} & N & Concrete section-based construction \\
\hline
Support preservation $\neq$ Born equivariance & \texttt{Equivariance\allowbreak{}Gap.\allowbreak{}support\_\allowbreak{}preservation\_\allowbreak{}does\_\allowbreak{}not\_\allowbreak{}imply\_\allowbreak{}measure\_\allowbreak{}preservation} & N & Concrete \texttt{Fin 2} counterexample \\
\hline
Born typicality (mean form) & \texttt{Born\allowbreak{}Typicality.\allowbreak{}born\_\allowbreak{}mean\_\allowbreak{}conditional} & U & Conditional on canonical $\mu_\rho$ with Born marginal \\
\hline
Operational data ⇏ unique IC measure & \texttt{Operational\allowbreak{}No\allowbreak{}Go.\allowbreak{}operational\_\allowbreak{}data\_\allowbreak{}insufficient} & N & Concrete \texttt{Fin 3} witness \\
\hline
Sub-thm A (Typicality Mackey-Gleason) & \texttt{Typicality\allowbreak{}Mackey\allowbreak{}Gleason.\allowbreak{}qiqth\_\allowbreak{}typicality\_\allowbreak{}mackey\_\allowbreak{}gleason} & C & Conditional on Mackey-Gleason + noncommutative Radon-Nikodym (standard operator-algebra theorems, axiomatized) \\
\hline
Sub-thm C (FQ-equivariance uniqueness) & \texttt{FQEquivariance\allowbreak{}Uniqueness.\allowbreak{}canonical\_\allowbreak{}ic\_\allowbreak{}measure\_\allowbreak{}principle} & P & \textbf{Post-consolidation:} depends on exactly 2 project axioms (\texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Fin\allowbreak{}Dim.\allowbreak{}step1\_\allowbreak{}schur\_\allowbreak{}classification} + \texttt{step3\_\allowbreak{}tensor\_\allowbreak{}multiplicativity}), down from 9 (12 redundant abstract axioms eliminated) \\
\hline
GS Step 2 (normalization) & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Fin\allowbreak{}Dim.\allowbreak{}step2\_\allowbreak{}normalization} & U & Concrete \texttt{Matrix.\allowbreak{}trace} computation \\
\hline
GS Step 4 (non-degeneracy) & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Fin\allowbreak{}Dim.\allowbreak{}step4\_\allowbreak{}nondegeneracy} & U & Direct case analysis \\
\hline
GS Step 3 algebraic core & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Step3.\allowbreak{}step3\_\allowbreak{}algebraic\_\allowbreak{}core} & U & Polynomial identity via \texttt{ring} \\
\hline
GS Step 3 Kronecker bridge & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Kronecker.\allowbreak{}step3\_\allowbreak{}kronecker\_\allowbreak{}bridge} & U & Concrete \texttt{d = 2} witness with E₁₁ ⊗ E₁₁ \\
\hline
GS Step 1 (Schur classification) & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Step1.\allowbreak{}step1\_\allowbreak{}via\_\allowbreak{}sub\_\allowbreak{}lemmas} & P & Delegates to the single top-level interface axiom \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Fin\allowbreak{}Dim.\allowbreak{}step1\_\allowbreak{}schur\_\allowbreak{}classification}. The five former placeholder sub-axioms (1a–1e), three of them literally false as stated, all unused, have been \textbf{deleted}; the proved foundation lemmas \texttt{permutation\_\allowbreak{}conj\_\allowbreak{}matrix\allowbreak{}Unit}, \texttt{diagonal\allowbreak{}U\_\allowbreak{}conj\_\allowbreak{}matrix\allowbreak{}Unit}, and \texttt{step1c\_\allowbreak{}collapse\_\allowbreak{}of\_\allowbreak{}perm\_\allowbreak{}symmetric} are its concrete building blocks \\
\hline
GS Step 1c (coefficient unification, partial) & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Step1.\allowbreak{}step1c\_\allowbreak{}collapse\_\allowbreak{}of\_\allowbreak{}perm\_\allowbreak{}symmetric} & U & \textbf{NEW (A2):} the \emph{permutation-symmetry collapse component} of Step 1c, proved via \texttt{Equiv.\allowbreak{}swap} transitivity under its necessary permutation-symmetry hypothesis. The prior bare-coefficient axiom (no hypothesis) was literally false and has been \textbf{deleted}. The \emph{full} Schur classification (Step 1) remains the single named interface axiom \\
\hline
Permutation conjugation of matrix units & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Step1.\allowbreak{}permutation\_\allowbreak{}conj\_\allowbreak{}matrix\allowbreak{}Unit} & U & \textbf{NEW (A2):} $P_\sigma \cdot E_{ij} \cdot P_\sigma^* = E_{\sigma(i),\sigma(j)}$ via direct matrix-entry computation \\
\hline
Diagonal-unitary conjugation of matrix units & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Step1.\allowbreak{}diagonal\allowbreak{}U\_\allowbreak{}conj\_\allowbreak{}matrix\allowbreak{}Unit} & U & \textbf{NEW (A2):} $D(z)\cdot E_{ij}\cdot D(z)^* = (z_i\,\overline{z_j})\,E_{ij}$, self-contained diagonal parameterization (no \texttt{Complex.\allowbreak{}exp}) \\
\hline
Combined GS finite-dim theorem & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Fin\allowbreak{}Dim.\allowbreak{}goldstein\_\allowbreak{}struyve\_\allowbreak{}findim} & P & Proved by composition; inherits Step 1 + Step 3 axioms \\
\hline
Marginal locality (pure pushforward) & \texttt{Marginal\allowbreak{}Locality.\allowbreak{}pushforward\_\allowbreak{}marginal\_\allowbreak{}local} & U & \textbf{NEW (A1):} $r\circ T = r \Rightarrow r_*(T_*\mu) = r_*\mu$ for \textbf{any} $\mu$, no equivariance assumption; reindexing argument \\
\hline
Marginal locality (equivariant corollary) & \texttt{Marginal\allowbreak{}Locality.\allowbreak{}alice\_\allowbreak{}marginal\_\allowbreak{}unchanged\_\allowbreak{}by\_\allowbreak{}bob\_\allowbreak{}dynamics} & U & \textbf{NEW (A1):} thin corollary of the pure version; discharges the \emph{marginal-locality step} (not the Hilbert→set bridge, which stays a named axiom) \\
\hline
Chebyshev tail bound on finite probability space & \texttt{Born\allowbreak{}Concentration.\allowbreak{}chebyshev\_\allowbreak{}tail\_\allowbreak{}bound} & U & \textbf{NEW (A4):} abstract Chebyshev via variance-dominates-restriction \\
\hline
Bernoulli variance $p(1-p)$ & \texttt{Born\allowbreak{}Concentration.\allowbreak{}bernoulli\_\allowbreak{}variance} & U & \textbf{NEW (A4):} direct two-term sum computation \\
\hline
Variance addition (independence) & \texttt{Born\allowbreak{}Concentration.\allowbreak{}variance\_\allowbreak{}add\_\allowbreak{}of\_\allowbreak{}product} & U & \textbf{NEW (A4):} $\mathrm{Var}(X_1+X_2)=\mathrm{Var}\,X_1+\mathrm{Var}\,X_2$ on a product measure, the reusable lemma the $N$-fold LLN induction would iterate \\
\hline
Two-trial Bernoulli variance $2p(1-p)$ & \texttt{Born\allowbreak{}Concentration.\allowbreak{}two\_\allowbreak{}trial\_\allowbreak{}bernoulli\_\allowbreak{}variance} & U & \textbf{NEW (A4):} concrete instance of variance-addition \\
\hline
Born single-trial frequency concentration & \texttt{Born\allowbreak{}Concentration.\allowbreak{}born\_\allowbreak{}chebyshev\_\allowbreak{}single\_\allowbreak{}trial} & U & \textbf{NEW (A4):} single-trial bound; the $N$-trial scaling to full "Born frequencies" still rests on the LLN interface axiom \\
\hline
Minimality witnesses for P1, P2, P3 & \texttt{Born\allowbreak{}Minimality\allowbreak{}Table.\allowbreak{}\{P1, P2, P3\}\_\allowbreak{}…\_\allowbreak{}necessary} & N + U & \textbf{NEW (A6):} unified independence package showing each Born sub-axiom is necessary by concrete finite countermodel (minimality \emph{relative to the current decomposition}) \\
\hline
Locality (P4) is reducible & \texttt{Born\allowbreak{}Minimality\allowbreak{}Table.\allowbreak{}P4\_\allowbreak{}locality\_\allowbreak{}reducible\_\allowbreak{}to\_\allowbreak{}equivariance} & U & \textbf{NEW (A6):} marginal-locality theoremized (modulo the named bridge axiom); not an independent Born-selection sub-axiom \\
\hline
\textbf{GS Step 1 (Schur classification)} & \texttt{Goldstein\allowbreak{}Struyve\allowbreak{}Step1.\allowbreak{}schur\_\allowbreak{}classification\_\allowbreak{}real} & U & \textbf{2026-06 UPDATE:} the former interface axiom \texttt{step1\_\allowbreak{}schur\_\allowbreak{}classification} is \textbf{RETIRED}, Step 1 is now fully proved axiom-free (Hadamard relation → linear assembly → reality via \texttt{Is\allowbreak{}Hermitian\allowbreak{}Preserving}). Step 3 likewise proved. The finite Goldstein–Struyve chain is axiom-free; budget dropped to \textbf{35} \\
\hline
Finite effect (Busch) Gleason & \texttt{Effect\allowbreak{}Gleason.\allowbreak{}finite\_\allowbreak{}effect\_\allowbreak{}gleason} & U & normalized + positive + coexistent-additive functional on effects $=\mathrm{tr}(\rho\,\cdot)$, Born from positivity, finite-dim \\
\hline
Finite weak LLN (Chebyshev) & \texttt{Born\allowbreak{}Typicality\allowbreak{}Finite.\allowbreak{}chebyshev\_\allowbreak{}freq} / \texttt{chebyshev\_\allowbreak{}freq\_\allowbreak{}union\_\allowbreak{}le} & U & $P(\lvert\mathrm{freq}-p_k\rvert\ge\varepsilon)\le p_k(1-p_k)/(N\varepsilon^2)$; union bound $\le 1/(N\varepsilon^2)$ \\
\hline
Quantum bridge (tensor trace) & \texttt{Born\allowbreak{}Typicality\allowbreak{}Quantum.\allowbreak{}quantum\allowbreak{}Weight\_\allowbreak{}eq\_\allowbreak{}w} & U & $N$-copy product-measurement weight $=$ classical product weight of the Born vector \\
\hline
Product-measure uniqueness & \texttt{Born\allowbreak{}Measure\allowbreak{}Uniqueness.\allowbreak{}product\_\allowbreak{}born\_\allowbreak{}measure\_\allowbreak{}unique} & U & the product Born measure is the unique additive history measure with the Born marginals (independence explicit) \\
\hline
\textbf{C1} one record → one \textbf{value} & \texttt{Pointer\allowbreak{}Value.\allowbreak{}exists\allowbreak{}Unique\_\allowbreak{}actual\allowbreak{}Value} / \texttt{exists\allowbreak{}Unique\_\allowbreak{}actual\allowbreak{}History} & U & capacity forbids two distinct-valued coactual records; many same-value records may coexist \\
\hline
\textbf{C2} one-site Born (vector state) & \texttt{One\allowbreak{}Site\allowbreak{}Born.\allowbreak{}vector\allowbreak{}State\_\allowbreak{}eq\_\allowbreak{}weight} & U & $\mathrm{Re}\langle\psi,E_a\psi\rangle=\lVert E_a\psi\rVert^2$ on a PVM projection \\
\hline
\textbf{Single-trial Born from non-contextuality} & \texttt{One\allowbreak{}Site\allowbreak{}Gleason.\allowbreak{}one\allowbreak{}Site\_\allowbreak{}forced} & U & non-contextual \texttt{Effect\allowbreak{}Measure} $\Rightarrow$ $\mu(P_a)=\mathrm{Re}\,\mathrm{tr}(\rho P_a)$, \textbf{forced} by effect-Gleason (strong premise; not Born by hand) \\
\hline
Non-contextual $=$ Born (converse) & \texttt{One\allowbreak{}Site\allowbreak{}Gleason.\allowbreak{}trace\allowbreak{}Effect\allowbreak{}Measure} & U & every density matrix is a non-contextual \texttt{Effect\allowbreak{}Measure} \\
\hline
Subadditive capacity exclusion & \texttt{Core\allowbreak{}No\allowbreak{}Collapse.\allowbreak{}joint\_\allowbreak{}coactual\_\allowbreak{}subsingleton}; \texttt{Orthogonal\allowbreak{}Capacity.\allowbreak{}pair\_\allowbreak{}exceeds} & U & monotone joint cost + pairwise overflow (overflow \textbf{derived} from orthogonality; no additivity assumed) \\
\hline
\textbf{Finite no-collapse Born representation} & \texttt{Born\allowbreak{}Join.\allowbreak{}finite\_\allowbreak{}no\allowbreak{}Collapse\allowbreak{}Born\allowbreak{}Representation}; \texttt{Born\allowbreak{}Join\allowbreak{}Gleason.\allowbreak{}finite\_\allowbreak{}no\allowbreak{}Collapse\allowbreak{}Born\_\allowbreak{}from\allowbreak{}Noncontextuality} & U & \textbf{the join:} unique actual history + Born product law + Chebyshev typicality; single-trial law \textbf{forced Born} from non-contextuality. \emph{Conditional representation theorem, assumes non-contextuality + product preparation, NOT Born from $Q_{\max}$.} See \texttt{FINITE\_\allowbreak{}BORN\_\allowbreak{}REPRESENTATION.\allowbreak{}md} \\
\hline
Independence $=$ product preparation & \texttt{Born\allowbreak{}Typicality\allowbreak{}Finite.\allowbreak{}w\_\allowbreak{}history\_\allowbreak{}factorizes} & U & product world-measure $\Rightarrow$ trial independence; independence is the irreducible product-preparation input \\
\hline
World-measure observationally free & \texttt{Born\allowbreak{}Join.\allowbreak{}Actual\allowbreak{}Ensemble.\allowbreak{}history\_\allowbreak{}law\_\allowbreak{}unique} & U & same single-trial law $\Rightarrow$ identical actual-history statistics, independent of the posited world-measure on $\Omega$ \\
\hline
\end{tabularx}
\end{center}
...
bound alone. Full claim→theorem map and honest scope caveats: \texttt{FINITE\_\allowbreak{}BORN\_\allowbreak{}REPRESENTATION.\allowbreak{}md}.
...
\textbf{Tsirelson clarification.} The Lean module \texttt{QIQTH.\allowbreak{}Tsirelson} proves \emph{attainability} of the value $2\sqrt{2}$ by an explicit singlet-state construction in 4-dimensional real Euclidean space (which is the achievability content of Tsirelson's theorem). The \emph{upper bound} statement that CHSH $\le 2\sqrt{2}$ for \emph{every} quantum state and \emph{every} measurement choice, the full Tsirelson theorem, requires operator-norm machinery on C\textbackslash\{\}*-algebras and is \emph{not} proved in this formalization. The framework relies on the standard Tsirelson upper bound as a cited result.

\textbf{Markov suppression clarification.} The \texttt{Capacity\allowbreak{}Packing} module formalizes only the \emph{Markov-style} multi-record suppression (polynomial tail in the modular slack), explicitly \emph{not} exponential suppression. Where the paper says "exponentially suppressed", the reference is to \emph{decoherence-driven} exponential suppression of off-diagonal coherence (which is genuinely exponential, e.g., $e^{-10^{20}}$ for macroscopic environment dimensions), not to Markov-style multi-record suppression from the entropy bound alone.

\subsection{Machine-checked modular substrate (companion formalization)}

A companion Lean 4/Mathlib development (pinned commit \texttt{4720763a7b59}, toolchain \texttt{leanprover/\allowbreak{}lean4:v4.\allowbreak{}30.\allowbreak{}0}) machine-checks the standard modular / relative-entropy \emph{calculus} underlying the regional cost functional $\chi_R$, for the free-field coherent-state sector. Every result below has status \textbf{U} (no project-specific axioms; depends only on \texttt{propext}, \texttt{Classical.\allowbreak{}choice}, \texttt{Quot.\allowbreak{}sound}, verified in \texttt{QIQTH/\allowbreak{}Axiom\allowbreak{}Audit.\allowbreak{}lean}); none formalizes the holographic axiom (FQ), the Macroscopic Definiteness Conjecture, or Born-from-typicality. A full statement-level index with \texttt{file:line} references and the companion paper appears in \texttt{paper\_\allowbreak{}strategy/\allowbreak{}45\_\allowbreak{}Theorem\_\allowbreak{}Paper\_\allowbreak{}Index.\allowbreak{}md}.

\emph{Finite Araki relative entropy.}

  • \texttt{araki\allowbreak{}Entropy\_\allowbreak{}eq\_\allowbreak{}rel\allowbreak{}Entropy}: $S_{\rm Araki}(\rho\,\|\,\sigma)=\operatorname{tr}\rho(\log\rho-\log\sigma)$ (the Umegaki form).

\emph{Bounded Tomita-Takesaki (standard subspace).}

  • \texttt{mod\allowbreak{}Conj\_\allowbreak{}rvd\allowbreak{}RC\_\allowbreak{}mod\allowbreak{}Conj}: $JRJ=2-R$.
  • \texttt{mod\allowbreak{}Conj\_\allowbreak{}rvd\allowbreak{}T\_\allowbreak{}of\_\allowbreak{}mem\_\allowbreak{}K}: $J(T\xi)=(2-R)\xi$ for $\xi\in\mathcal{K}$.
  • \texttt{mod\allowbreak{}Unitary} (\texttt{\_\allowbreak{}add}, \texttt{\_\allowbreak{}unitary}, \texttt{\_\allowbreak{}strongly\allowbreak{}Continuous}): $\Delta^{it}$, with group law, unitarity, and strong continuity.

\emph{One-particle CGP relative entropy.}

  • \texttt{cgp\allowbreak{}Entropy}: $S(\xi)=-\int\log((2-r)/r)\,d\mu^R_\xi$.
  • \texttt{rvd\allowbreak{}Spec\_\allowbreak{}balance}: the CGP spectral balance.
  • \texttt{cgp\allowbreak{}Entropy\_\allowbreak{}nonneg}: $S(\xi)\ge 0$ for localized $\xi\in\mathcal{K}$.

\emph{Free-field (Fock) modular flow.}

  • \texttt{second\allowbreak{}Quant\allowbreak{}Mod\allowbreak{}Flow\allowbreak{}H}: $\Gamma(\Delta^{it})$ (a one-parameter group, vacuum-fixing, strongly continuous on coherent vectors).
  • \texttt{second\allowbreak{}Quant\allowbreak{}Mod\allowbreak{}Flow\allowbreak{}H\_\allowbreak{}weyl\allowbreak{}H}: $\sigma_t(W(u))=W(\Delta^{it}u)$.

\emph{Coherent-state relative modular operator and reduction.}

  • \texttt{rel\allowbreak{}Mod\allowbreak{}Flow\allowbreak{}H}: $\Delta^{it}_{W(f)\Omega\,|\,\Omega}=W(f)\,\Gamma(\Delta^{it})\,W(f)^*$.
  • \texttt{connes\allowbreak{}Cocycle\allowbreak{}H}, \texttt{connes\allowbreak{}Cocycle\allowbreak{}H\_\allowbreak{}chain}: $[D\omega_{W(f)\Omega}:D\omega_\Omega]_t=W(f)W(-\Delta^{it}f)$, together with the cocycle chain rule.
  • \texttt{has\allowbreak{}Deriv\allowbreak{}At\_\allowbreak{}rel\allowbreak{}Mod\allowbreak{}Flow\_\allowbreak{}vacuum}: $S_{\rm Araki}(\omega_{W(f)\Omega}\,\|\,\omega_\Omega)=S_{\rm CGP}(f)$ (the Casini-Grillo-Pontello identity).
