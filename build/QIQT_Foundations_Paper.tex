...
\textbf{Prior published work — QIQT-H as its independent machine-verified formalization (2026).} Dorau and Much (\emph{From Quantum Relative Entropy to the Semiclassical Einstein Equations}, Phys. Rev. Lett. \textbf{136}, 091602 (2026); arXiv:2510.24491, public October 2025) gave a quantum-field-theoretic extension of Jacobson's argument, deriving the \textbf{semiclassical Einstein equations} from the \textbf{Araki–Uhlmann relative entropy} between the vacuum and a coherent excitation of a Klein–Gordon field on a local Rindler (bifurcate Killing) horizon. Their chain is, step for step, the free-field GR chain machine-checked here: (i) the modular flow acts as the geometric boost — Bisognano–Wichmann / Summers–Verch, $\Delta_\mathcal{R}^{it}=\mathfrak{D}_{2\pi t}$ — our unconditional \texttt{Fock.OneParticleBW}; (ii) the coherent-state relative entropy equals the horizon energy flux $\delta Q$, $S^{\rm rel}(\omega_0\|\omega_\phi)=-2\pi\!\int U\langle{:}T_{ab}{:}\rangle\xi^a\xi^b$ (Casini–Grillo–Pontello) — our \texttt{ModularEnergyBound} (the first law $\Delta S=\Delta\langle K_W\rangle=2\pi\,\Delta\langle B_{\rm boost}\rangle$); (iii) the area variation is fixed by Raychaudhuri focusing on the horizon — our \texttt{DifferentialAreaLaw.differential\_area\_law} together with the Raychaudhuri machinery; and (iv) stress-energy conservation closes it to $R_{ab}-\tfrac12 R\,g_{ab}+\Lambda g_{ab}=\alpha\langle{:}T_{ab}{:}\rangle$ — our capstone \texttt{qiqt\_gr\_freefield\_complete} / \texttt{qiqt\_bekenstein\_gives\_gr}. Where they \emph{assume} the Bekenstein–Hawking relation $S^{\rm rel}=\delta A/4$ to fix $\alpha=8\pi$, QIQT-H additionally \emph{re-derives} the $1/4$ ratio (Sakharov, \texttt{SakharovRatio.sakharov\_ratio}). **Crucially, their assumed $S^{\rm rel}=\delta A/4$ is the \emph{same} load-bearing input QIQT-H supplies as (P4) / the area law: both derivations take the entropy–area relation as given, so their Letter confirms the shared \emph{derivation chain} — relative entropy $\to$ modular theory $\to$ Jacobson $\to$ Einstein — and \emph{not} the finiteness postulate itself; QIQT-H's only surplus at this exact step is the machine-checked re-derivation of the $1/4$ coefficient.** Their Letter — public on arXiv in October 2025, \emph{before} QIQT-H's GR chain was formalized (mid-2026) — establishes this relative-entropy $\to$ modular-theory $\to$ Jacobson route to the Einstein equations in a peer-reviewed venue. QIQT-H does \textbf{not} claim priority here: it independently and \emph{subsequently} \textbf{machine-verifies the same chain}, reducing it to a kernel-checked derivation with every physical input in an explicit hypothesis ledger (the claim card of \texttt{qiqt\_gr\_freefield\_complete}), and re-deriving the $1/4$ they assume. QIQT-H's contribution at this step is formalization and verification, not discovery. Both meet the \emph{same} honest frontier: their closing caveat — higher-order corrections on a genuinely curved horizon, "\emph{technically demanding, especially regarding the modular data}" — is precisely QIQT-H's cited continuum/curved-correction frontier (the localization map and the Riemannian-heat-kernel / Seeley–DeWitt $(1/6-\xi)$ gap).
...
\begin{enumerate}
  ...
  \item \textbf{Dorau, P., \& Much, A. (2026).} \emph{From Quantum Relative Entropy to the Semiclassical Einstein Equations.} Phys. Rev. Lett. 136, 091602 (2026). DOI: 10.1103/lmq8-nsty. arXiv:2510.24491 [hep-th] (submitted October 2025).
