...
\emph{Lean status (\texttt{GleasonSelector.lean}) — axiom-free.} The architecture is formalized in the finite-dimensional record model. A first version named the Busch / POVM-Gleason step as an interface axiom; a soundness review (GPT-5.5-pro) showed that axiom was \textbf{false as stated} — it omitted \emph{positivity}, and the explicit weight $w(E) = E_{00} + E_{01}$ on $\mathbb{C}^2$ satisfies normalization, additivity, homogeneity, and ray-certainty yet is not the Born functional. The axiom was therefore \textbf{retired}, and the module now contains \textbf{no project axiom}; everything is proved (standard axioms only, verified by \texttt{AxiomAudit.lean}):
\begin{itemize}
  ...
  \item \texttt{proj\_sandwich} — proved: $P_\psi\, E\, P_\psi = \langle\psi|E|\psi\rangle \cdot P_\psi$ on the ray projector (the algebraic heart of "ray-certainty $\Rightarrow$ Born");
  \item \texttt{born\_is\_forced} — proved (no axiom): a linear, \textbf{ray-supported} weight ($w(E) = w(P_\psi E P_\psi)$ — the explicit positivity/certainty hypothesis the counterexample violates) that is ray-certain \emph{is} the Born functional;
