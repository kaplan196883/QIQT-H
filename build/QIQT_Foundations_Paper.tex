...
The capstone originally rested on two \emph{standard} linear-algebra interface axioms; one has since been \textbf{discharged}: \texttt{positive\_functional\_hermitian} (a positive functional is a $*$-functional) is now \emph{proved} in Lean by the standard polarization argument (realness of $w((A+X)^\dagger(A+X))$ and $w((A+iX)^\dagger(A+iX))$ forces conjugate symmetry), as is the real-quadratic core \texttt{quadratic\_nonneg\_forall\_linear\_zero}. The single remaining named axiom is \texttt{psd\_null\_radical} (a null vector of a positive-semidefinite sesquilinear form lies in its radical — the Cauchy–Schwarz core); it is finite, standard, true, and far weaker than the retired false "Gleason $\Rightarrow$ Born" axiom. Discharging it (the complex re/im bookkeeping of the expansion, on top of the already-proved real-quadratic core) and the continuum Type II / history-net version are the honest residual.
