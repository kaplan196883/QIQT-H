...
\textbf{Three things cannot select the framework.} (i) \emph{Capacity} — a holographic cardinality bound does not pick a realm (\texttt{RealmSelection.capacity\_underdetermines\_realm}: two distinct orthogonal $2$-record realms in $\mathbb{C}^2$ are equally capacity-maximal). (ii) \emph{Symmetry / typicality} — the unitary group acts \textbf{transitively} on frameworks (orthonormal bases), so \emph{any} unitarily-invariant typicality score is \textbf{constant} and selects none (\texttt{SymmetryNoGo.unitary\_invariant\_score\_constant}, the continuous form of (i); \texttt{exists\_unitary\_map} exhibits the single orbit; the abstract principle \texttt{invariant\_of\_pretransitive\_constant} needs no Lean axioms at all). (iii) \emph{The state alone} — a single projection $P=|\Phi\rangle\langle\Phi|$ generates only the trivial framework $\{0,P,1-P,1\}$ (\texttt{StateAloneNoGo.state\_records\_trivial}: any idempotent $a\,P+b(1-P)$ has $a,b\in\{0,1\}$ — Bub–Clifton). So neither capacity, nor symmetry, nor $\Phi$ fixes the records.

\textbf{Einselection does.} The positive selector is Zurek's commutativity criterion, machine-checked: a record observable commuting with the monitored observable $A$ commutes with the interaction $A\otimes B$ (\texttt{MetaselectorSelection.pointer\_commutes}, i.e. $[P,A]=0 \Rightarrow [P\otimes 1, A\otimes B]=0$), and an $A$-eigenstate stays a \emph{product} under the coupling (\texttt{pointer\_invariant}: $a\otimes e \mapsto a\otimes(\alpha\, Be)$ — decoherence-free, no entanglement generated). The record framework is therefore the spectral algebra of the \textbf{interaction Hamiltonian's} monitored observable: the metaselector is einselection (predictability sieve + Darwinist redundancy, the latter the \texttt{SBSSuppression} spectrum-broadcast structure), \emph{not} symmetry or capacity. This is conditional on a system–environment factorization, which remains a genuine residual (the Dowker–Kent set-selection / Heisenberg-cut problem; einselection is the empirically-correct but \emph{conditional} selector — a partial, not complete, answer).

\textbf{The record/area contract.} The bookkeeping around $\lambda$ is made category-error-proof in a labeled scaffold (\texttt{RecordContract}), separating four inputs — \emph{factorization} (cut) $\to$ \emph{einselection} (which framework) $\to$ \emph{area capacity} (how big) $\to$ \emph{$\lambda$} (which atom) — and threading them with proved bridges: coarse-graining is a capacity-non-increasing pushforward (\texttt{coarsen\_card\_le}), the record information obeys $H(R)\le\log|R|$ (Jensen on $-x\log x$, \texttt{shannon\_le\_log\_card}), and the holographic bound enters as a \textbf{one-sided capacity boundary condition} in \emph{two distinct versions kept separate} — the \textbf{entropy} form $H(R)\le S_{\text{area}}$ (Bousso, on the decohered record) and the strictly stronger \textbf{capacity} form $\log|R|\le S_{\text{area}}$ (a $\dim\mathcal{H}_{\text{rec}}\le e^{S_{\text{area}}}$ postulate, \emph{not} implied by Bousso, since a low-entropy law can have huge support). The contract rests on Born-\emph{from-projectors}: for a finite orthogonal PVM and normalized $\Phi$, $\mu(r)=\|P_r\Phi\|^2=\langle\Phi,P_r\Phi\rangle$ is a genuine record law (\texttt{BornProjBridge.bornRecordLaw}), and the selector's event statistics \emph{are} this Born measure (\texttt{RecordContract.eventProb}). The one finite-budget effect that is a theorem: a $D$-dimensional record space cannot hold $M>D$ mutually orthogonal records (\texttt{finite\_budget\_forces\_overlap}) — demanding more forces a residual-interference floor (astronomically small for realistic horizon entropies). All postulates are carried as Lean \emph{hypotheses}, never axioms, so the budget stays $0$.

\textbf{Honest reach.} This is \emph{verified scaffold}, not new physics: it answers "what fixes the framework" (einselection) and fences off the shortcuts (capacity/symmetry/state), but $\lambda$ stays inert and the Born weights are assumed, so the scheme remains \textbf{operationally Everett} with zero empirical content (\texttt{everett\_equivalence}). In particular the tempting "select the most typical framework by symmetry, \emph{with physical impact}" route is \textbf{provably closed} (no-go (ii)): a non-dynamical Born-sampled $\lambda$ cannot have observable impact without \emph{becoming} an operational deviation from standard QM, whose only weakly-constrained home is the cosmological/holographic regime (an early-universe non-equilibrium deviation, e.g. a low-$\ell$ CMB $\xi_0\ne1$) — out of scope here, and not yet a theory. The pedagogical one/two/three-qubit "universes" used to \emph{illustrate} this layer (\texttt{TinyUniverse}, \texttt{TwoBitUniverse}, \texttt{ThreeQubitUniverse}) are single-basis toys, explicitly caveated (their "entanglement" is single-basis correlation, not a context-crossing witness). Five GPT-5.5-pro red-team passes kept the framing honest; every claim was checked against the Lean before adoption.
