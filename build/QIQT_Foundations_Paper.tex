...
\emph{(H3) \textbf{Operational distinguishability.} There exists a normal measurement instrument on $\hat{\mathcal{A}}(R)$ decoding the active record index with error at most $\eta_{\rm def}/\log|\mathcal{A}_\epsilon|$, so that the Fano-type bound $H_\epsilon \le I_{\rm Hol}^R + \eta_{\rm def}$ holds.}
...
\emph{Wedge case (Bisognano-Wichmann).} In the Minkowski-vacuum sector, let $W$ be a Rindler wedge with $R \subset W$. By the Bisognano-Wichmann theorem, for states normal to the vacuum representation and in the domain of the boost / modular Hamiltonian, the \emph{exact first-moment expression} is
...
The open-problems list has been restructured after the modular-local refactor (§7.6) and Theorem 6's Donald's-identity reformulation. Items related to \emph{global} branch-summed admissibility have been retired or rephrased; the framework's commitments are now sharply located in the \textbf{record-instantiation-cost postulate} (H2 in Theorem 6) and the \textbf{stagewise causal admissibility structure} (§7.6).
...
\textbf{Resolved or retired:}

\begin{itemize}
  \item \textasciitilde{}\textasciitilde{}"No-signaling theorem under regional admissibility"\textasciitilde{}\textasciitilde{} \textbf{RESOLVED in §7.7} via modular-local admissibility + AQFT microcausality. The earlier Bell-style counter-example with hard joint-diamond cutoff (see \texttt{paper\_strategy/38}, \texttt{paper\_strategy/39}) motivated the modular-local refactor.
  \item \textasciitilde{}\textasciitilde{}"Precise formulation of the branch-summed cost"\textasciitilde{}\textasciitilde{} \textbf{RETIRED}: the branch-summed cost $I_\Sigma^\epsilon$ is now a \emph{derived classical-mixture approximation} of $\chi_R$, not a fundamental quantity.
  \item \textasciitilde{}\textasciitilde{}"Characterization of physical Hamiltonians on $\mathcal{H}_{\rm phys}$"\textasciitilde{}\textasciitilde{} \textbf{REPHRASED} as item 3 (stagewise adapted process): the framework's dynamics is now stated as admissibility-preserving normal CP maps on the algebraic state space, not Hamiltonian flow on a Hilbert subspace.
\end{itemize}
