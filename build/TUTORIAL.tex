...
\section{Bell’s theorem: QIQT-H violates Parameter Independence}
...
\subsection{Which premise QIQT-H gives up: Parameter Independence (not measurement independence)}

Every interpretation that reproduces quantum predictions must give up \emph{one} of Bell's premises — outcome definiteness, measurement independence (free settings), or local causality. This is not special to QIQT-H; orthodox quantum mechanics already gives one up.

QIQT-H's commitments fix the choice with no freedom left:

\begin{itemize}
  \item It \textbf{keeps outcome definiteness} (one record per run) — so it is not Everett.
  \item It \textbf{keeps measurement independence}, $\rho(\lambda|a,b)=\rho(\lambda)$: the settings $a,b$ are genuinely free of the run's actuality fact $\lambda$. So QIQT-H \textbf{rejects superdeterminism} — it deliberately does \emph{not} take the measurement-dependence escape.
  \item Because the per-run selection is deterministic in $(\Phi,\lambda)$, \textbf{Outcome Independence holds automatically} (a definite outcome can't depend on the distant \emph{outcome} given $\lambda$).
\end{itemize}

Bell's local causality splits (Jarrett/Shimony) into Outcome Independence + Parameter Independence. With the first two premises kept and Outcome Independence automatic, the \textbf{only door left is Parameter Independence} — your outcome may depend on the distant \emph{setting}, given $\lambda$. QIQT-H violates Parameter Independence at the ontic level.

This is \emph{Bell-nonlocal by necessity}: every single-world, non-superdeterministic, definite-outcome theory must land here. It is the generic price, not a holographic trick.

\textbf{What you keep, despite ontic Parameter-Independence violation:} \emph{operational no-signaling.} After averaging over the actuality measure $\mu$,
...
\textbf{The honest open burden (Wood–Spekkens).} That this no-signaling cancellation is \emph{not fine-tuned} — that the same $\mu$ which yields Born weights also enforces no-signaling without an experiment-by-experiment conspiracy — is a genuine open problem, part of the Born-typicality program. We flag it rather than hide it.

\textbf{Contrast with Palmer's RaQM.} Palmer takes the \emph{opposite} horn: he denies measurement independence (a principled, non-conspiratorial measurement-dependence / superdeterminism-family escape via the invariant set). QIQT-H keeps measurement independence and pays at Parameter Independence instead. The two share single-world realism and a finite-information motivation, but on the central Bell move they are contrasting, not allied.
...
An effectively noninterfering component of the wavefunction produced by decoherence. In QIQT-H a branch is \emph{not} a separate world: decoherence makes branches stop interfering, but that does not make one of them \emph{actual}. \textbf{Correction (2026):} an earlier version said a $\ge 2$-record content \emph{exceeds the regional capacity} $Q_R$ and so is not instantiable — that is a \textbf{category error} (the holographic bound counts independent degrees of freedom, not a sum of redundant records, and ordinary record entropy is capped far below the bound). Capacity is the finite record \emph{stage} (how many distinguishable records exist, $\le e^{Q_R}$), not a single-outcome rule. The resident regional content is single-record because the actuality selector $\lambda$ — a non-dynamical, atemporal fact about which macroscopic realization the run is — marks exactly one record actual; the unrealized branches remain in $\Phi$ but have no per-run physical referent (they are not "real elsewhere").
