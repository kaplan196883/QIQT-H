...
\textbf{The decisive role of (FQ)}: once the per-run amplitudes shrink below the resolution floor $\epsilon(R)$, \emph{they are physically the same state as amplitude $0$}. The branch is not "weighted small"; it physically does not exist. Once one amplitude grows above $1 - \epsilon(R)$, \emph{it is physically the same state as amplitude $1$}. The realized outcome is the wave function's actual physical state.
