...
with $c_{R_S}(r_k) = S(\omega_k \| \Omega_{R_S})$ the per-record relative entropy and $H_{\rm Shannon}$ the entropy of the mixture weights. This recovers the \textbf{branch-summed cost} $I_\Sigma^\varepsilon \approx N \cdot I_0$ used in earlier formulations as a \textbf{derived approximation} in the appropriate (single-region, classical-mixture) regime, which is precisely the regime that applies to the double-slit screen after detection.
