...
\begin{enumerate}
  ...
  \item \textbf{three brackets on the exponent} (§10): a rank-count no-go (records can't count the weights), the rotation/Banach–Lamperti uniqueness (only the square survives unitary mixing — the same square as the Gaussian bell curve, while a Lorentz boost admits no positive norm), and Gleason (the noncontextual functionals are exactly the Born functionals).
\end{enumerate}
...
\section{Why the exponent is 2: counting, symmetry, and uniqueness}

Granting that the weight must be assumed, the reader will press: \emph{why the square}? Four further
machine-checked results sharpen this and bracket the Born posit from independent directions.

\textbf{Records cannot count the weights.} One might hope the multiplicity of environment records of a branch
...
(\texttt{RankCountNoGo.no\_multiplicity\_rule\_is\_born}). Decoherence delivers definite records, not their weights.

\textbf{Only the square survives mixing.} Unitary evolution continuously mixes amplitudes; ask which normalization
...
(\texttt{RotationBorn.rotation\_invariant\_iff\_exponent\_two}). The \textbf{same square} governs the bell curve: a
rotation-invariant product density is forced Gaussian with $|z|^2$ in the exponent (Maxwell–Herschel;
\texttt{SymmetrySquare.gaussian\_profile\_from\_rotation}) — the Gaussian's square and Born's square are one and the
same rotation invariant. A \textbf{Lorentzian boost}, by contrast, preserves the indefinite $t^2-x^2$ and so admits
\emph{no} positive probability norm — it must vanish on the light cone (\texttt{SymmetrySquare.no\_boost\_invariant\_positive\_norm}).
Relativistic Born therefore descends from the unitary Wigner representation on the positive-definite Hilbert
space, never from spacetime geometry (compact rotation \emph{forces} $\alpha=2$; the non-compact boost \emph{forbids}
any positive norm).

\textbf{Gleason makes the posit unique.} The irreducible input — a noncontextual probability assignment on effects
— is forced to the Born/trace form $\operatorname{Re}\operatorname{tr}(\rho E)$ and conversely realized by
every density matrix (\texttt{BornChain.noncontextual\_forces\_born}, \texttt{born\_is\_noncontextual}), so the noncontextual
functionals are \emph{exactly} the Born functionals. With the meta no-go (§6) and the rank-count no-go above, this
is the honest ceiling: Born reduces to one maximally-natural posit, made unique by Gleason and proven
irreducible from three directions. \emph{Honest caveat:} the rotation/symmetry results confirm the square is the
...
\hline
records ⇏ Born by counting & \texttt{no\_multiplicity\_rule\_is\_born} & \texttt{RankCountNoGo.lean} \\
\hline
noncontextual ⇒ Born (Gleason) & \texttt{noncontextual\_forces\_born}, \texttt{born\_is\_noncontextual} & \texttt{BornChain.lean} \\
\hline
only α=2 survives mixing & \texttt{rotation\_invariant\_iff\_exponent\_two} & \texttt{RotationBorn.lean} \\
\hline
bell curve = Born's square & \texttt{gaussian\_profile\_from\_rotation} & \texttt{SymmetrySquare.lean} \\
\hline
boost no-go (no positive norm) & \texttt{no\_boost\_invariant\_positive\_norm} & \texttt{SymmetrySquare.lean} \\
\hline
