...
<strong>Why the capacity scales with area, not volume.</strong> Grow a ball of matter and its Bekenstein
(energy) information rises as ∝ ρR⁴ — faster than the holographic ceiling ∝ R² — so the two meet exactly at the
Schwarzschild radius, where the ball becomes a black hole. The area law is the <em>collapse envelope</em>:
matter cannot pack volume-much information without collapsing. Ordinary matter's <em>realized</em> entropy
(∝ R³ ≈ particle number, purple) sits far below — the Sun realizes ~10⁻³¹ of its capacity. Only horizons
saturate: black holes, and the cosmological horizon (~10¹²² bits), which the universe's realized entropy
(~10¹⁰⁴) fills to just 10⁻¹⁸. <span style="opacity:0.7">(Generated by <code>scripts/bekenstein_flow_plot.py</code>.)</span>
</figcaption>
</figure>
