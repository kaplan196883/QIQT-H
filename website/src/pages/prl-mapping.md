...
| Numerical value of `G` | not addressed (out of scope) | a named frontier — the relation `G = 1/(N Λ_s²)` is derived; the *number* needs the curved-space Seeley–DeWitt coefficient `a₁ = R/6`, now advanced to a **machine-checked *conditional*** result (`a1_R6_from_data`, the consolidated capstone, carrying the genuine Ricci scalar `(∑ᵢ Ricᵢᵢ)/6 = R/6`) reduced to four semantic input groups — three now discharged or derived in-bank, leaving one deep residue (the Duhamel/convergence-trio parabolic-PDE wall); the *unconditional* true-kernel Seeley–DeWitt identification stays a Mathlib-wide frontier |
