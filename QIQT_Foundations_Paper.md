...
**Honest characterization of math status:** the framework provides the *scaffolding* (Type II algebras from CPW/Witten) and the *axiom* ((FQ) literal reading). It establishes *qualitative* consequences, and now an axiom-free, machine-checked *kinematic, persistence, Born, and selection-event law* for $\lambda$ — at the finite, one-particle continuum, and second-quantized free-field levels (§11.2b). What it does not yet have are theorems for the two cited research walls (the Haagerup natural-cone existence, the interacting case) and for the *dynamical origin* of the actuality datum itself. This is the status of a research program with a sharply-localized residual, not a completed mathematical theory.
...
### 11.2b $\lambda$'s machine-checked law — the complete picture

As of 2026-06-16, the actuality selector $\lambda$ has gone from a bare postulate to a *law whose every checkable piece is machine-checked* (Lean 4 / Mathlib, axiom-free, raw axiom count 0). The earlier idea that $\lambda$ needs the gravitational crossed product to live on a Type II algebra is **retired as a category error** (Type II factors also have no minimal projections, so the records $\lambda$ selects come from a chosen abelian *pointer* subalgebra $\mathfrak{A}$ that already lives inside the Type III$_1$ local algebra); $\lambda$ is **Type-III-native**, riding the standard form and the modular automorphism. The verified law has four layers, each established at the **finite**, the **one-particle continuum** (the Rieffel–Van Daele bounded modular flow $\Delta^{it}$), *and* the **second-quantized free-field** (the unitary group $\Gamma(\Delta^{it})$) levels:

1. **Which pointer algebra (kinematics).** The pointer algebra $\mathfrak{A}$ carrying $\lambda$'s records is fixed by **Takesaki's criterion**: an $\omega$-preserving conditional expectation onto $\mathfrak{A}$ exists iff $\sigma_t^\omega(\mathfrak{A}) = \mathfrak{A}$, which in the spectral/finite picture is $[\rho, P_\alpha] = 0$ — *exact decoherence*. The decoherence map $E(x) = \sum_\alpha P_\alpha x P_\alpha$ is then that $\omega$-preserving conditional expectation (`LambdaPointer`, `ContinuumLambda.modAutOp_fixes_iff_commute`, `ContinuumLambdaField`).

2. **Born weights (Type-independent).** The weight of a record is the algebraic state value $\omega(P_\alpha) = \lVert P_\alpha \Phi\rVert^2$ — via the natural cone / vacuum state, *no trace required*. Over a resolution of the pointer family these are a genuine probability (`bornWeights_sum`, `NaturalConeBorn`, and on the genuine Fock vacuum state `FieldBorn`: a POVM's vacuum weights sum to one, the Weyl-bit record giving $(1\pm e^{-\lVert u\rVert^2/2})/2$).

3. **Persistence (the static $\to$ dynamical bridge).** The decoherence map **commutes with the modular flow** $\sigma_t$ for *every* $t$ (`dephase_sigmaDiag_commute`, `dephaseOp_specProj_commute`, `dephaseFock_modAutFock_commute`): a records-definite state stays records-definite, coherence between pointer sectors never regenerates, and the selected record is a fixed point of the dynamics with constant Born weights. This is the first genuinely *dynamical* result — the selection does not un-select under evolution.

4. **The selection event (one record, not zero or two).** An explicit inverse-CDF constructor takes an "actuality seed" $s \in [0,1)$ to **exactly one** record (`selects_exists_unique`, `continuum_selects_exists_unique`, `field_selects_exists_unique` — totality *and* uniqueness), and the uniform seed measure of a record equals its Born weight (`volume_selects`), so the selection *realizes* Born as an across-run frequency. The selector is order-dependent, not equivariant — exactly as the **no-covariant-selector** result requires (a covariant *measure* but no covariant *selector*, the $S^2$ obstruction), while the measure is covariant; the construction is contextuality-safe (no global value-map) with state-independent no-signaling.

**Born reduction and its irreducible core.** Born is reduced (axiom-free) to a *state-supervenience* premise, and that premise is **split exactly**: its weak half (naturality / permutation-equivariance) is machine-checked to be *blind to the exponent* — it holds for every reprocessing $f$, so it cannot force Born (`weight_naturality`; the $w^2$ witness is natural yet non-Born, `weak_underdetermines_born`) — while its strong half (refinement-additivity) is what discriminates the square and linearizes into equiprobability (`refinementAdditive_nsmul`). A **no-go** (`NoBornFromNothing`) proves *some* such premise is unavoidable.
...
> **Note (2026-06-15/16):** the per-row "named interface axiom" / project-axiom labels in this matrix are a *historical snapshot* of the strengthening pass. The development has since been driven to **budget 0 — entirely axiom-free** (every theorem depends only on Lean's standard `propext`, `Classical.choice`, `Quot.sound`), and λ's law (kinematics, persistence, Born, selection event) is machine-checked at the finite, one-particle continuum, and second-quantized free-field levels (§11.2b). Read the axiom-count cells as "as of the strengthening pass", superseded by the axiom-free milestone.
