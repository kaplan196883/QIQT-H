...
*Lean status (`GleasonSelector.lean`).* The architecture is formalized in the finite-dimensional record model, in the project's standing discipline: the deep finite-dim Busch / POVM-Gleason representation step is the *single named interface axiom* `effect_gleason_representation` (the additive→linear→trace-duality→ray-certainty chain, not in current Mathlib), and from it the consequences that matter are *proved* — `history_measure_is_born` / `history_measure_total` (the record measure is forced to the Born / decoherence-functional weights and is normalized) and `no_signaling_marginal` (requirement (e)/no-signaling, honest form: one bilinear correlation gives spacelike-marginal independence for *all* of Bob's settings, depending on **no** project axiom). The Born functional's own linearity/normalization (`born_add`, `born_smul`, `born_one`) is proved outright.
