...
The capstone rests on exactly two *standard* linear-algebra interface axioms: `psd_null_radical` (a null vector of a positive-semidefinite sesquilinear form lies in its radical — the Cauchy–Schwarz core) and `positive_functional_hermitian` (a positive functional is a $*$-functional). Both are finite, true, and far weaker than the retired false "Gleason $\Rightarrow$ Born" axiom; discharging them from first principles (a polarization / PSD-form development) is the honest residual, together with the continuum Type II / history-net version.
