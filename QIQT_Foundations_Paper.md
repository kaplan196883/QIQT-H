...
6. The standard textbook formal wave function, a superposition $\sum_i c_i |s_i\rangle|A_i\rangle|E_i\rangle$, is an idealized statistical description of an ensemble of per-run wave functions across many runs. The per-run wave function evolves unitarily from specific microscopic initial conditions; decoherence renders the macroscopic record components non-interfering; and the actuality selector $\lambda$ leaves the per-run regional content single-record. (An earlier version attributed the single record to a *capacity overflow* — that a $\ge 2$-record content "exceeds $Q_R$"; this is **retired as a category error**, see §11.2a. $Q_R$ is the finite record *stage*, bounding how many distinguishable records exist, not a single-outcome rule; the single outcome is $\lambda$'s.) The run's microscopic initial conditions index which record.
...
**Does.** Proposes the **$(\Phi,\lambda)$ single-world no-collapse ontology**: an exactly-unitary global wave function $\Phi$ together with a non-dynamical actuality selector $\lambda$ that marks one decohered record as actual. Introduces (FQ) in its rigorous algebraic form using Type II crossed-product algebras, and identifies $Q_R$'s role as the finite record *stage* (a count/cardinality bound, $\le e^{Q_R}$, with ordinary storage capped at $\sim(A/\ell_P^2)^{3/4}$ — see §7.6, H2′). Distinguishes formal vs per-run wave functions. Establishes the finite-physical-resolution consequence of Type II regional algebras. Contributes a **machine-checked, axiom-free reduction of Born-from-typicality** for the free-field sector: a Poincaré-covariant, $\sigma$-additive, decoherent-histories-*consistent* Born measure on the record net, with Born statistics reduced to a single state-supervenience premise and a no-go showing some such premise is unavoidable. States the formal results and identifies the explicit open problems.

**Does not.** Claim that finite capacity *forbids* macroscopic superpositions or *selects* outcomes (the original Macroscopic Definiteness Conjecture H2 is **retired** as a category error, §7.6 / Open Problem 3); the single outcome is supplied by $\lambda$, not by $Q_R$. Derive the resolution floor as an explicit function of regional geometry. Provide a fully rigorous, premise-free typicality theorem for Born weights — the central open problem is to give $\lambda$ a precise *law* (a covariant typicality measure). Modify the Schrödinger equation, posit Bohmian particles with a guidance law, MWI *actual* parallel branches, or a stochastic/collapse dynamics. (It **does** posit one actuality primitive: $\lambda$, a non-dynamical, broad-sense beable in the modal / hidden-variable family — there is no guidance law, no back-reaction on $\Phi$, and no second dynamics.)
...
- **Single-world hidden-variable / modal accounts** (Bohmian mechanics; modal interpretations, Dieks, Vermaas; Kent's late-time / final-condition real-world; consistent histories with a selected realm, Griffiths, Omnès, Gell-Mann–Hartle): with H2 retired, $\lambda$ places QIQT-H squarely in this family — a single actual history selected from the unitarily-evolved alternatives. We are explicit that, like these, QIQT-H is a *completion* of standard QM rather than new physics **unless $\lambda$ is given a precise law**. The intended distinctive content is exactly that law: a Poincaré-covariant typicality measure on the holographically-finite record net, of which the machine-checked free-field reduction (covariant $\sigma$-additive consistent Born measure) is the first installment. Absent that, the honest reading is "Everett minus the unrealized branches, via a primitive $\lambda$" — coherent, but not yet a new theory.

- **Deterministic-generator ('t Hooft Cellular Automaton) reading — considered and not adopted.** A distinct way to give $\lambda$ a "law" would make it a *finite-information deterministic generator* of the actual history (a small seed + rule, in the spirit of 't Hooft's deterministic quantum mechanics) rather than a typicality-sampled brute fact. We flag this only to set it aside, with the honest verdict (developed in the supplementary explorations): it is empirically equivalent to standard QM; it must pay Bell's price (nonlocality or superdeterminism, since a finite *local* generator is CHSH-bounded at $2 < 2\sqrt2$); and it is untestable in practice — a Born-typical history is algorithmically *incompressible* (Martin–Löf random), so a compressible generator would deviate from Born only at its repetition scale, which for the realized cosmic entropy ($\sim 10^{104}$ bits) is the **Poincaré recurrence time** $\sim 2^{10^{104}}$. The program therefore keeps $\lambda$ as a *brute, Born-typical (incompressible)* actuality fact, not a pseudo-random generator; the generator picture is recorded as a mapped-but-not-adopted alternative.
...
*Scope.* What is verified is the modular/relative-entropy *calculus* motivating $\chi_R$, in the finite and free-field coherent sectors, **not** the dressed Type II regional content map itself, the CPW/Witten crossed product, the holographic capacity axiom (FQ), the Macroscopic Definiteness Conjecture, the Donald/Fano argument, or Born-from-typicality, all of which remain explicit physical assumptions or open problems of this paper. In particular the single-record results (Theorem 4) are *static*-exclusion statements conditional on the Macroscopic Definiteness Conjecture. Their *dynamical* realization (that unitary measurement evolution produces exactly one admissible record and preserves the admissible set), the conjecture itself, and Born-from-typicality are the load-bearing gaps the formalization does not close.
...
3. A regional content carrying $\ge 2$ macroscopically distinct records exceeds the region's information capacity $Q_R$ and is therefore not an instantiable physical state: so the resident content is single-record (this is what makes the outcome single)
...
**Theorem 4 (Single-record per-run).** *Under (FQ) in its literal physical-instantiation reading, together with (i) standard decoherence (which renders the macroscopic records non-interfering) and (ii) the Macroscopic Definiteness Conjecture of §7.6 (two or more macroscopically distinct records being *actual* together in a region have joint cost exceeding the regional capacity $Q_R$), the per-run **actual** (instantiated) regional content $C_R$ in any region $R$ containing apparatus + environment after measurement is single-record — while the wave function $\Phi$ itself keeps every branch and evolves exactly unitarily. The constraint is on *actuality*, not on $\Phi$: by linearity $\Phi$ does carry the multi-record superposition, and finite capacity cannot forbid it (a superposition of records is one vector at no extra cost); what the bound limits is how many records can be jointly *actual*, where classical record-contents add. This exclusion is machine-checked $\Phi$-independently (`CoreNoCollapse`/`CapacityModel`). Which record is realized is indexed by the run's microscopic initial conditions; the unrealized records have no physical referent in $R$ (they are not instantiable, not "real elsewhere"). No amplitude is trimmed and the global evolution is exactly unitary throughout.*
...
*(This proof inherits the open status of the §7.6 Macroscopic Definiteness Conjecture, the $I_\Sigma > Q_R$ step, which is Open Problem 3; the rest is standard decoherence plus the (FQ) resolution-equivalence of §5.1. The **finite-dimensional number-bound core** of that step, at most one macroscopic record fits a finite additive capacity, with the threshold derived rather than stipulated, is now a machine-checked conditional theorem, conditional on a factorized $\gamma<1$ scattering premise; see the formalization note in §6.8. The continuum entropy-scaling realization on the Type II algebra, and the strong superposition-exclusion form, remain open.)*

> **Correction (2026-06-15, two GPT-5.5-pro consults checked against Bousso/Bekenstein + decoherence theory).** The premise (ii) above — that "classical record-contents **add**," so $I_\Sigma \approx K\cdot I_0 > Q_R$ — is **a category error and cannot carry the single-record conclusion.** (a) The holographic bound counts *independent* degrees of freedom (joint entropy / code-dimension), **not** a sum of redundant marginal records: $R$ redundant Quantum-Darwinism imprints of one classical fact have joint entropy $H(X)$, not $R\,H(X)$ (they are compressible; $\sum_i I(X{:}F_i)\sim R\,H(X)$ double-counts). So actual classical records do **not** add against $Q_R$. (b) Ordinary weakly-gravitating record-carrier entropy is capped at $S\sim (A/\ell_P^2)^{3/4}$ (the maximum thermal entropy before gravitational collapse: $E\lesssim R/\ell_P^2$ with $S\sim (ER)^{3/4}$ gives $S\sim (A/\ell_P^2)^{3/4}$), $\approx 10^{91.5}$ for the observable universe against $Q_{\max}\sim 10^{122}$ — a **permanent $\sim 31$-order gap** (power $3/4$ vs $1$; only black holes reach $A/4$, and a black hole carries no records). Hence $I_0\approx Q_R$ is false by $\sim 31$ orders on the ordinary-record ceiling; even the universe's total *realized* entropy ($\sim 10^{104}$, black-hole-dominated) is only $\sim 10^{-18}$ of its holographic capacity, at every scale up to the cosmological horizon. (c) The genuine exclusion of two simultaneously-actual records is **local single-valuedness** (a classical carrier holds one value) together with the actuality fact $\lambda$ — and "a carrier has one value" is itself the single-world claim supplied by $\lambda$, not by $Q_R$ and not by decoherence (decoherence supplies near-orthogonal branch structure and suppressed interference, **not** definiteness). This is consistent with, and sharpens, the §7.6 *Scope* disavowal: the single-record-per-run content rests on $\lambda$; $Q_R$'s honest role is the **finite stage only** — bounding the *number* of distinguishable records ($\le e^{Q_R}$), the finite-trace (Type III$\to$II) record algebra, the decoherence-redundancy regulator, and the area-grain — never an exclusion mechanism. Theorem 4 should accordingly be read as "decoherence + $\lambda$ (with $Q_R$ supplying only finiteness of the record set)," and the strong "$I_\Sigma>Q_R$ forbids the multi-record content" form is withdrawn, not merely left open.*
...
> **Lemma (Maximum record-carrier entropy scales as $A^{3/4}$, not $A$).** *The largest entropy ordinary, weakly self-gravitating matter can carry in a region of boundary area $A$, before it collapses to a black hole, scales as $S_{\max}\sim (A/\ell_P^2)^{3/4}$ — strictly below the holographic value $A/4\ell_P^2\sim (A/\ell_P^2)^{1}$, by a power $3/4<1$. Hence the holographic bound is saturated only at black-hole density, where there are no classical records; ordinary record carriers fall parametrically short.*
>
> *Proof (textbook; 't Hooft 1993, Susskind 1995).* Work in natural units $\hbar=c=k_B=1$, with $G=\ell_P^2$. Fill a region of radius $R$ (so area $A\sim R^2$, volume $V\sim R^3$) with thermal/relativistic matter at temperature $T$. Standard thermodynamics gives energy and entropy
...
>
> *Corollary (the capacity-exclusion margin).* The ratio to the holographic bound is $S_{\max}/(A/4\ell_P^2)\sim (A/\ell_P^2)^{-1/4}\to 0$. For the observable universe, $A/\ell_P^2\sim 10^{122}$ gives $S_{\max}\sim 10^{91.5}$ against $Q_R\sim 10^{122}$ — a structural $\sim\!31$-order gap on ordinary record carriers; even the total *realized* entropy (black-hole-dominated, $\sim 10^{104}$, which carries no records) is itself only $\sim 10^{-18}$ of the holographic capacity. So even at maximal ordinary entropy, two records cost $\sim 2\,S_{\max}\ll Q_R$: the would-be capacity exclusion of two macroscopic records fails not by a small margin but parametrically, and closes only in the gravitational-collapse limit $S\to A/4$, which carries no records. This is the quantitative form of the disavowal above: $Q_R$ bounds the record *count*, never the coexistence of two actual macroscopic records.

> **Remark (why redundancy does not help).** One might try to charge $Q_R$ for the *uncompressed* count of redundant Quantum-Darwinism imprints. This double-counts: $R$ redundant copies of a classical fact $X$, $|x\rangle\mapsto |x\rangle_{F_1}\cdots|x\rangle_{F_R}$, occupy a code subspace of dimension $2$ with joint entropy $H(F_1,\dots,F_R)=H(X)$, not $R\,H(X)$ (the Darwinist sum $\sum_i I(X{:}F_i)\sim R\,H(X)$ counts the *same* bit $R$ times). The holographic bound constrains independent degrees of freedom (joint entropy / code dimension), not the sum of redundant marginals — so redundancy cannot lift ordinary record entropy above the $A^{3/4}$ ceiling of the Lemma. The exclusion of two simultaneously-*actual* records therefore rests on local single-valuedness (a classical carrier holds one value) together with $\lambda$, not on the area bound. *(This Remark's combinatorial core is **machine-checked**, axiom-free, in `QIQTH/RedundancyCompressible.lean`: `card_redundantCodewords` — the number of distinguishable $R$-fold redundant records is $|X|$, independent of $R$, not $|X|^R$; `naive_overcounts` — the naive $R\cdot\log|X|$ strictly exceeds the true $\log|X|$; `code_subspace_dim` — orthonormal redundant-copy states span a subspace of dimension $|X|$, not exponential in $R$. The continuum $A^{3/4}$ Lemma above is the cited physics input, not formalized.)*
...
**Honest empirical status.** We flag this plainly: the finite-capacity postulate is *kinematic*, and with the holographic value $Q_R \sim 10^{70}$ for a 1\,m region — vastly above plausible record costs $I_0 \sim 10^{25}$ — the bound is never approached at any laboratory-accessible scale. A genuine observable deviation from standard quantum mechanics (a macroscopic-superposition cutoff, or the neutrino decoherence below) requires an *additional dynamical law* with a much smaller effective capacity $Q^{\mathrm{eff}}$, which is at present a free parameter rather than a consequence of the bound. Absent such an addition, finite $Q_{\max}$ is **empirically equivalent to standard quantum mechanics** — i.e., this is an interpretation/architecture, not (yet) new physics. The "predictions" here should be read as conditional on such a dynamical extension.
...
### 9.5 Emergent gravity: the Einstein equations as a machine-checked equation of state

If the holographic capacity bound is taken seriously as a thermodynamic constraint on regional information, the natural question is whether it reproduces gravitational dynamics in the manner of Jacobson's (1995) "Einstein equation of state." It does, as a **conditional** result that we have formalized end to end in Lean 4 / Mathlib — axiom-free in the same sense as the rest of the development (`#print axioms` returns only `propext`, `Classical.choice`, `Quot.sound`).

The headline is a single machine-checked theorem (`qiqt_gr_from_wedge_kms`): the **Einstein field equations** $a\,T_{\mu\nu} = G_{\mu\nu} + \Lambda\,g_{\mu\nu}$, with a genuine Einstein tensor and a *constant* $\Lambda$, follow from QIQT-H's holographic **capacity bound** $S \le \eta A$ together with **Klein positivity** $S(\rho\Vert\sigma)\ge 0$, modulo **exactly three clearly-labelled physics inputs**: the wedge **KMS property**, **matter conservation** $\nabla^\mu T_{\mu\nu}=0$, and standard **structural regularity** (a Lorentzian congruence, Raychaudhuri kinematics, and smoothness).

What is genuinely *derived* in the chain — no hypothesis assuming the conclusion:

- the **differential area law** $\delta S = \eta\,\delta A$, obtained from the capacity *bound* together with saturation at the reference state and Klein positivity (the bound, its saturation, and positivity are theorems here; no hypothesis asserts the equality $S = \eta A$);
- the **boost-charge** content of the wedge KMS input: the wedge modular flow is the geometric Lorentz boost (one-particle Bisognano–Wichmann), and the boost-energy derivative is purely imaginary, $i\cdot(\text{real})$, derived from the explicit boost generator together with unitarity;
- the **focusing** content of the structural input: at a stationary horizon the Raychaudhuri equation collapses to pure Ricci focusing, $\mathrm{d}\theta/\mathrm{d}\lambda = -R_{kk}$;
- the entire **differential geometry**: the Bianchi identities, $\nabla^\mu G_{\mu\nu}=0$, the null-cone $\Rightarrow$ tensor step, and the constancy of $\Lambda$.

**Honest scope.** This is a verified *formalization* of a *conditional* chain, not a derivation of general relativity from nothing, and not a new physical prediction. The three labelled inputs are kept as explicit hypotheses and never as axioms. The wedge KMS property bottoms out in cited algebraic-QFT facts (KMS-uniqueness, the strip property, standardness — the Bisognano–Wichmann package), which we cite rather than formalize: these are standard, true theorems, but a Lean proof would require operator-algebra infrastructure (unbounded Tomita–Takesaki theory, Hardy-strip methods) not yet present in Mathlib — a substantial separate undertaking, not an impossibility in principle. The boost charge's final identification requires the free-field stress tensor (one scalar identity, not yet constructed); and matter conservation $\nabla^\mu T_{\mu\nu}=0$ is a physical postulate. What QIQT-H supplies as *theorems* is the inequality side of the area law; what turns it into general relativity is Jacobson's argument, here machine-checked. The contribution is to have *drained* the analytic and geometric content out of two of the three inputs into theorems (modular flow $=$ boost; Raychaudhuri focusing), sharpening each labelled input to its irreducible kernel, and to have certified the whole chain at the level of the proof-assistant kernel. It is consistent with, and complementary to, the Wald reading of §8.3: there the gravitational theory fixes the regional trace and hence $Q_R$; here the capacity bound, read as a Clausius relation, returns the field equations. The development is documented in the formalization companion; a statement-level index is public.
...
**Machine-checked progress on $\lambda$'s law (2026-06-15).** Since the first draft, the actuality selector's *kinematic* law has been made precise and machine-checked (axiom-free, Lean 4 / Mathlib, finite / Type I shadow; `lean/mathlib/QIQTH/LambdaPointer.lean`). Two findings. First, $\lambda$ is **Type-III-native**: an operator-algebra red-team retired the idea that the Type II crossed product is needed to give $\lambda$ a *home*. Type II factors also lack minimal projections, so the records $\lambda$ selects come from a chosen abelian *pointer* subalgebra $\mathfrak A$ that already exists inside the Type III$_1$ factor, and the Born weights are the algebraic state value $\omega(P_\alpha)=\lVert P_\alpha\Phi\rVert^2$ (via the natural cone, no trace required). The Type II framework remains the rigorous home for the (FQ) *capacity / entropy* bound, its proper use, but it is not load-bearing for *selection*. Second, *which* pointer algebra is consistent is fixed by **Takesaki's criterion**: an $\omega$-preserving conditional expectation onto $\mathfrak A$ exists iff $\sigma_t^\omega(\mathfrak A)=\mathfrak A$, which in finite dimensions is $[\rho,P_\alpha]=0$, exact decoherence (`modAut_fixes_iff_commute`); the decoherence map $E(x)=\sum_\alpha P_\alpha x P_\alpha$ is then that $\omega$-preserving conditional expectation (`dephase_preserves_state`); the Born weights are a genuine probability (`bornWeights_sum`); and, the first *dynamical* result, that selection **persists for all time** under the real-time modular flow $\sigma_t(x)=\rho^{it}x\rho^{-it}$ (`dephase_sigmaDiag_commute`; unconditional in the einselected basis, `dephase_sigmaDiag_commute_diagonal`), so coherence between pointer sectors never regenerates and the selected record is a fixed point of the dynamics with constant Born weights. What remains open for $\lambda$: a constructor for the *selection event* (why exactly one record becomes actual), the across-run *frequency* derivation (Open Problem 1, bounded by the Born no-go), and the continuum Type III$_1$ realization, now targeting the **standard form / natural cone** with the modular $\sigma_t^\omega$ via Connes cocycles rather than the retired crossed-product route.

**Honest characterization of math status:** the framework provides the *scaffolding* (Type II algebras from CPW/Witten) and the *axiom* ((FQ) literal reading). It establishes *qualitative* consequences, and now an axiom-free, machine-checked *selection-and-consistency schema* for $\lambda$ — the kinematic criterion, the modular-invariance, the Born-weight bookkeeping, and the inverse-CDF selection representation — at the finite, one-particle continuum, and second-quantized free-field levels (§11.2b), conditional on a primitive actuality datum and a Born premise. What it does *not* establish are the *physical* dynamical claims (irreversibility under the actual Hamiltonian; the origin of the seed), nor theorems for the two cited research walls (the Haagerup natural-cone existence, the interacting case). This is the status of a research program with a sharply-localized residual — and a verified consistency scaffold around an admitted primitive — not a completed mathematical theory.
...
### 11.2b $\lambda$'s machine-checked *selection schema* — the verified scaffold and its honest reach

What is machine-checked around $\lambda$ (Lean 4 / Mathlib, axiom-free, raw axiom count 0) is best stated precisely, because the word "law" would overclaim. It is a **selection-and-consistency schema conditional on a primitive actuality datum**: *given* a chosen decohered record context, *given* the Born weights, *given* a refinement-additive typicality premise, and *given* an actual seed, the formalism selects exactly one record and verifies a body of modular-invariance, normalization, covariance, and contextuality facts. "Axiom-free in Lean" means no *Lean* axioms beyond the standard foundations; it does **not** mean no physical postulates — those sit in the hypotheses (the chosen record algebra, exact decoherence, the Born weights, the uniform seed measure, refinement-additivity). With that framing, the verified content has four parts, each established at the **finite**, the **one-particle continuum** (the Rieffel–Van Daele bounded modular flow $\Delta^{it}$), *and* the **second-quantized free-field** (the unitary group $\Gamma(\Delta^{it})$) levels. (The earlier idea that $\lambda$ needs the gravitational crossed product to live on a Type II algebra is **retired as a category error**: Type II factors also have no minimal projections, so the records $\lambda$ selects come from a chosen abelian coarse-graining $\mathfrak{A}=\operatorname{span}\{P_\alpha\}$ associated with — not an atom of — the Type III$_1$ local algebra. $\lambda$ rides the standard form and the modular automorphism, *not* a forced trace; "Type-III-native" should be read in that sense, not as "$\lambda$ is an atom of the factor.")

1. **Which record context (kinematics).** The condition for a record context to carry an $\omega$-preserving coarse-graining is **Takesaki's criterion**: an $\omega$-preserving conditional expectation exists iff the subalgebra is invariant under the modular automorphism $\sigma_t^\omega$, which in the spectral/finite picture is $[\rho, P_\alpha] = 0$ — *exact decoherence*. The dephasing map $E(x) = \sum_\alpha P_\alpha x P_\alpha$ is then the $\omega$-preserving conditional expectation onto the **block-diagonal algebra** $\{P_\alpha\}'$ (generally nonabelian; the *classical record labels* are the atoms of the abelian sub-coarse-graining $\operatorname{span}\{P_\alpha\}$ within it). Two honest caveats: (i) here $\rho/\omega$ is a *faithful normal* (hence mixed/reduced) state — for a *pure* global $\Phi$, $[P_\alpha,|\Phi\rangle\langle\Phi|]=0$ would force all but one weight to vanish; the cyclic-separating-vector ($\lVert P_\alpha\Phi\rVert^2$) reading is the AQFT/Reeh–Schlieder one; (ii) exact $[\rho,P_\alpha]=0$ is an idealization (a superselection-like exactness), and an approximate-robustness version is future work (`LambdaPointer`, `ContinuumLambda.modAutOp_fixes_iff_commute`, `ContinuumLambdaField`).

2. **Born weights.** The weight of a record is the algebraic value of the (faithful normal) state on its effect, $\omega(P_\alpha)$ — via the natural cone / vacuum state, *no trace required* — and over a resolution these are a genuine probability (`bornWeights_sum`, `NaturalConeBorn`; on the genuine Fock vacuum state `FieldBorn`: a POVM's vacuum weights sum to one, the Weyl-bit record effect $(1\pm \operatorname{Re}W(u))/2$ giving $(1\pm e^{-\lVert u\rVert^2/2})/2$).

3. **Modular invariance (not physical dynamics).** The dephasing map **commutes with the modular flow** $\sigma_t$ for *every* $t$ (`dephase_sigmaDiag_commute`, `dephaseOp_specProj_commute`, `dephaseFock_modAutFock_commute`). This is a genuine consistency fact — no *modular* recoherence inside the chosen invariant record algebra — but it is essentially a functional-calculus consequence of building the records from operators invariant under the same generator, and the **modular flow is not the physical Hamiltonian evolution** (the two coincide only in special KMS / Bisognano–Wichmann cases). So it is *modular-invariance of an exactly-decohered record algebra*, **not** a proof that actual records do not recohere under the physical dynamics; that physical-irreversibility statement is not established.

4. **The selection event (a sampling representation).** An explicit **inverse-CDF** constructor takes an "actuality seed" $s \in [0,1)$ to exactly one record (`selects_exists_unique`, `continuum_selects_exists_unique`, `field_selects_exists_unique` — totality *and* uniqueness of a standard sampling map), and the **single-shot** Lebesgue measure of the seeds selecting a record equals its Born weight (`volume_selects`). This is a clean representation of "one outcome, Born-weighted"; it adds no actualization *mechanism* (the seed's origin and typicality are postulated), and a single-shot measure equality is *not yet* an across-run frequency — that requires a product/history measure and a law of large numbers (a separate finite-LLN module). The selector is order-dependent, not equivariant — exactly as the **no-covariant-selector** result requires (a covariant *measure* but no covariant *point-selector*, an $S^2$-type obstruction); the construction is contextuality-safe (no global value-map) with state-independent no-signaling (which is operationally weaker than Bell local causality — the selector remains globally contextual/nonlocal).

**Born reduction and its irreducible core.** Born is reduced (axiom-free) to a *state-supervenience* premise, and that premise is **split exactly** — a Gleason/Busch-style assumption audit: its weak half (naturality / permutation-equivariance) is machine-checked to be *blind to the exponent* — it holds for every reprocessing $f$, so it cannot force Born (`weight_naturality`; the $w^2$ witness is natural yet non-Born, `weak_underdetermines_born`) — while its strong half (refinement-additivity) is what discriminates the square and linearizes into equiprobability (`refinementAdditive_nsmul`). A **no-go** (`NoBornFromNothing`) proves *some* such premise is unavoidable. This is honest assumption-auditing, not "Born from nothing."

**Conservativity, and what this does and does not buy.** Because $\lambda$ has no back-reaction and the Born weights are assumed, the scheme is **operationally equivalent to standard (Everettian/orthodox) quantum mechanics**: it makes no prediction beyond standard Born statistics, and $\lambda$ is unobservable. Its content is interpretive — a single actual record by stipulation, with no collapse dynamics and no second substance — placing it in the **modal / Everett-plus-actuality-tag** family rather than as a new dynamical law. The irreducible residuals are exactly two: the **actuality datum itself** (the selection-event constructor reduces the whole selection to *which seed is actual*, and that seed *is* $\lambda$ — a primitive a non-dynamical single-world ontology must take as given; note the seed is a real number carrying unbounded information, consistent only because $\lambda$ is not a physical record counted by $Q_R$), and the **strong Born premise** (proven unavoidable). Beyond the free-field sector the remaining mathematics is two cited research walls — the **Haagerup natural-cone existence** in Mathlib (we state the Born rule on vector states; the canonical state$\mapsto$vector identification is cited) and the **interacting (non-free) case** — and the natural next *content*-adding targets are a **global decoherent-history** selector (one coherent actual world, not one atom of one finite resolution) and an explicit **across-run frequency** theorem.

**What remains, sharply localized.** Two residuals, both honest. (i) The actuality datum itself: the selection-event constructor reduces the whole selection to *which seed $s$ is actual* — and the seed *is* $\lambda$, the one primitive a non-dynamical single-world ontology must take as given; its *dynamical origin* is not derived (and arguably cannot be, $\lambda$ being non-dynamical by construction). (ii) The continuum walls beyond the free-field sector: the **Haagerup natural-cone existence** in Mathlib (we state the Born rule on vector states; the canonical state$\mapsto$vector identification is cited) and the **interacting (non-free) case**. Both are cited, research-grade boundaries, not gaps in the account. In short: $\lambda$'s law is precise and verified where it can be; what is left is either *provably irreducible* (the seed; the strong Born premise) or *genuine multi-year mathematics* (natural-cone existence, interacting QFT).

### 11.2c The *metaselector*: what fixes the record framework (a machine-checked no-go trilogy + einselection)

§11.2b part 1 takes the record context $\mathfrak{A}=\operatorname{span}\{P_\alpha\}$ as *given*. What fixes it — the "**metaselector**," the choice of abelian record framework — is itself a question, now resolved (Lean 4 / Mathlib, axiom-free, raw axiom count 0) as a **no-go trilogy plus a positive selector**, sharpening the division of labour of §11.3.

**Three things cannot select the framework.** (i) *Capacity* — a holographic cardinality bound does not pick a realm (`RealmSelection.capacity_underdetermines_realm`: two distinct orthogonal $2$-record realms in $\mathbb{C}^2$ are equally capacity-maximal). (ii) *Symmetry / typicality* — the unitary group acts **transitively** on frameworks (orthonormal bases), so *any* unitarily-invariant typicality score is **constant** and selects none (`SymmetryNoGo.unitary_invariant_score_constant`, the continuous form of (i); `exists_unitary_map` exhibits the single orbit; the abstract principle `invariant_of_pretransitive_constant` needs no Lean axioms at all). (iii) *The state alone* — a single projection $P=|\Phi\rangle\langle\Phi|$ generates only the trivial framework $\{0,P,1-P,1\}$ (`StateAloneNoGo.state_records_trivial`: any idempotent $a\,P+b(1-P)$ has $a,b\in\{0,1\}$ — Bub–Clifton). So neither capacity, nor symmetry, nor $\Phi$ fixes the records.

**Einselection does.** The positive selector is Zurek's commutativity criterion, machine-checked: a record observable commuting with the monitored observable $A$ commutes with the interaction $A\otimes B$ (`MetaselectorSelection.pointer_commutes`, i.e. $[P,A]=0 \Rightarrow [P\otimes 1, A\otimes B]=0$), and an $A$-eigenstate stays a *product* under the coupling (`pointer_invariant`: $a\otimes e \mapsto a\otimes(\alpha\, Be)$ — decoherence-free, no entanglement generated). The record framework is therefore the spectral algebra of the **interaction Hamiltonian's** monitored observable: the metaselector is einselection (predictability sieve + Darwinist redundancy, the latter the `SBSSuppression` spectrum-broadcast structure), *not* symmetry or capacity. This is conditional on a system–environment factorization, which remains a genuine residual (the Dowker–Kent set-selection / Heisenberg-cut problem; einselection is the empirically-correct but *conditional* selector — a partial, not complete, answer).

**The record/area contract.** The bookkeeping around $\lambda$ is made category-error-proof in a labeled scaffold (`RecordContract`), separating four inputs — *factorization* (cut) $\to$ *einselection* (which framework) $\to$ *area capacity* (how big) $\to$ *$\lambda$* (which atom) — and threading them with proved bridges: coarse-graining is a capacity-non-increasing pushforward (`coarsen_card_le`), the record information obeys $H(R)\le\log|R|$ (Jensen on $-x\log x$, `shannon_le_log_card`), and the holographic bound enters as a **one-sided capacity boundary condition** in *two distinct versions kept separate* — the **entropy** form $H(R)\le S_{\text{area}}$ (Bousso, on the decohered record) and the strictly stronger **capacity** form $\log|R|\le S_{\text{area}}$ (a $\dim\mathcal{H}_{\text{rec}}\le e^{S_{\text{area}}}$ postulate, *not* implied by Bousso, since a low-entropy law can have huge support). The contract rests on Born-*from-projectors*: for a finite orthogonal PVM and normalized $\Phi$, $\mu(r)=\|P_r\Phi\|^2=\langle\Phi,P_r\Phi\rangle$ is a genuine record law (`BornProjBridge.bornRecordLaw`), and the selector's event statistics *are* this Born measure (`RecordContract.eventProb`). The one finite-budget effect that is a theorem: a $D$-dimensional record space cannot hold $M>D$ mutually orthogonal records (`finite_budget_forces_overlap`) — demanding more forces a residual-interference floor (astronomically small for realistic horizon entropies). All postulates are carried as Lean *hypotheses*, never axioms, so the budget stays $0$.

**Honest reach.** This is *verified scaffold*, not new physics: it answers "what fixes the framework" (einselection) and fences off the shortcuts (capacity/symmetry/state), but $\lambda$ stays inert and the Born weights are assumed, so the scheme remains **operationally Everett** with zero empirical content (`everett_equivalence`). In particular the tempting "select the most typical framework by symmetry, *with physical impact*" route is **provably closed** (no-go (ii)): a non-dynamical Born-sampled $\lambda$ cannot have observable impact without *becoming* an operational deviation from standard QM, whose only weakly-constrained home is the cosmological/holographic regime (an early-universe non-equilibrium deviation, e.g. a low-$\ell$ CMB $\xi_0\ne1$) — out of scope here, and not yet a theory. The pedagogical one/two/three-qubit "universes" used to *illustrate* this layer (`TinyUniverse`, `TwoBitUniverse`, `ThreeQubitUniverse`) are single-basis toys, explicitly caveated (their "entanglement" is single-basis correlation, not a context-crossing witness). Five GPT-5.5-pro red-team passes kept the framing honest; every claim was checked against the Lean before adoption.
...
**Open Problem 3 — RETIRED (2026-06-15): the Macroscopic Definiteness Conjecture is a category error.** The conjecture below — that a regional content with $K \ge 2$ macroscopic records has summed cost $I_\Sigma \approx K\cdot I_0 > Q_R$, so multi-record contents are non-instantiable — rested on treating distinct/redundant classical records as *adding* against the holographic bound. That is invalid: (i) a holographic bound counts *independent* degrees of freedom (joint entropy / code dimension), not a sum of redundant marginals — $R$ redundant broadcast imprints of one record carry joint entropy $H(X)$, not $R\,H(X)$ (machine-checked, `RedundancyCompressible.lean`); (ii) ordinary weakly-gravitating record entropy is capped at $S\sim(A/\ell_P^2)^{3/4}$ (§7.6 Lemma), parametrically below $A/4\ell_P^2$ — only black holes saturate $A/4$, and a black hole carries no records; (iii) a superposition of two records costs $\sim 1$ bit (DPI), and by exact unitary linearity finite capacity can neither forbid it nor select a branch. The finite-dimensional mechanization (`SBSBridge.sbs_single_outcome`, `CoreNoCollapse.coactual_subsingleton`, `CapacityModel.capacity_total`) remains *sound as a conditional theorem* — IF actual-record costs were additive-and-saturating THEN at most one record is actual — but its antecedent (the additive/SBS cost, $I_0\approx Q_R$) is exactly the invalid premise (i)–(iii): the theorems are correct, their physical premise is not. Consequently **the single-outcome content does not rest on H2; it rests on the actuality selector $\lambda$** (§7.6) together with the local single-valuedness of classical records.

**What survives — H2′ (a capacity bound, not a definiteness principle).** A bounded region holds at most $e^{Q_R}$ mutually *distinguishable* records, with ordinary (non-black-hole) storage further limited to $\sim(A/\ell_P^2)^{3/4}$. H2′ constrains record *count/cardinality* (and is what makes the covariant record measure constructible via finite fibers); it does **not** exclude superpositions and does **not** select outcomes. The two subsidiary requirements survive as constraints on $\lambda$'s law rather than on a capacity rule: (a) **recoherence stability** (the resolution-equivalence must be dynamically stable) and (b) **no-signaling consistency**. The genuinely open problem is no longer "prove the Branch-Summed bound" but **"give $\lambda$ a precise law"** — a covariant typicality measure on record histories (the Born-typicality program, §7.4, §11.4), on which this paper contributes a machine-checked, axiom-free reduction for the free-field sector.

*The original (now-retired) conjecture, retained for the record.* Make rigorous the claim that, per run, a single macroscopic record physically *resides* in a region $R$ containing apparatus + environment after measurement. The mechanism is *not* a dynamical concentration of amplitudes (the global wave function evolves by exact unitary dynamics and (FQ) trims no amplitude; a machine-verified audit, `NoConcentration.lean`, confirms that *linear unitary decoherence alone* conserves branch weights and cannot push them toward $0$ or $1$). It is also not microscopic-IC selection, which only *indexes* which single-record universe a given run is. The one-world content comes from the **finite-information restriction on the physical state space**: a regional content carrying $K \ge 2$ macroscopically distinct records (coherent or classically mixed) has summed information cost $I_\Sigma \approx K \cdot I_0 > Q_R$ once $I_0 \approx Q_R$ at macroscopic scale, so it is not a point of the region's physical state space. The open problem is to **prove this Branch-Summed bound**, i.e. that the summed information of a $\ge 2$-record regional algebra-state genuinely exceeds $Q_R$ (the *Macroscopic Definiteness Conjecture*, §7.6), and to characterize the regional-capacity threshold sharply. Two subsidiary requirements: (a) **recoherence stability**, show that two regional states declared physically identical at the $\epsilon(R)$ resolution cannot later be unitarily amplified into distinct records (else the resolution-equivalence is not a well-defined physical identity); and (b) **no-signaling consistency** of the information restriction (Open Problem 10), since a regional/global admissibility constraint is structurally postselection-like.

*Status (2026-06), the finite number-bound core is now mechanized.* In a tractable finite-dimensional model the Branch-Summed bound is no longer conjectural: realizing records as a redundant Spectrum Broadcast Structure makes the summed cost a genuine *information* quantity $\ge R\log n$ (broadcasting tensors the fragments, so dimensions multiply), and a finite additive capacity then admits at most one macroscopic record, an axiom-free Lean theorem (`SBSBridge.sbs_single_outcome`, `CoreNoCollapse.coactual_subsingleton`), with the threshold *derived* from pointer-state orthonormality (`CapacityModel.capacity_total`) and robust to approximate decoherence (`fragment_finrank_ge_approx`). This is the *number-bound* (one-record) content; what remains genuinely open is (i) the continuum realization, reproducing this scaling as a renormalized-entropy bound on the Type II regional algebra rather than a finite register, and (ii) the *field-theoretic* origin of the scattering premise. Input (ii) has been **discharged for the standard collisional/QND toy Hamiltonian** $H_{\rm int}=g\,\sigma_z^S\otimes\sigma_x^E$ (`CollisionalGamma.lean`): the branch overlap is $\cos 2\theta$, so $\gamma=|\cos 2\theta|<1$ is derived and amplifies to $\gamma^L\to0$ (§6.8); what stays open is deriving the *uniform-$\gamma$, factorized, no-recoherence* structure itself from a realistic field-theoretic Hamiltonian (the constructive form of the Strasberg–Winter branch-selection problem, arXiv:2601.19703). The strong superposition-exclusion form (forbidding the cat state) is *not* claimed and is not needed for the single-record content.
...
> **Note (2026-06-15/16):** the per-row "named interface axiom" / project-axiom labels in this matrix are a *historical snapshot* of the strengthening pass. The development has since been driven to **budget 0 — entirely axiom-free** (every theorem depends only on Lean's standard `propext`, `Classical.choice`, `Quot.sound`), and $\lambda$'s selection-and-consistency schema (the Takesaki kinematic criterion, modular-invariance, Born-weight bookkeeping, inverse-CDF selection) is machine-checked at the finite, one-particle continuum, and second-quantized free-field levels (§11.2b) — conditional on a primitive seed and a Born premise; "modular-invariance" is *not* physical irreversibility. Read the axiom-count cells as "as of the strengthening pass", superseded by the axiom-free milestone.
