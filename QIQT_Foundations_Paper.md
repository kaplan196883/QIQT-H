...
**Open Problem 3 — Single-record residence (the "Concentration" problem, correctly framed).**
Make rigorous the claim that, per run, a single macroscopic record physically *resides* in a region $R$ containing apparatus + environment after measurement. The mechanism is *not* a dynamical concentration of amplitudes: the global wave function evolves by exact unitary dynamics and (FQ) trims no amplitude. A machine-verified audit (`NoConcentration.lean`) confirms that *linear unitary decoherence alone* conserves branch weights and cannot push them toward $0$ or $1$. The framework's position is that the single resident record arises from two non-dynamical ingredients acting together — the (FQ) **resolution-equivalence on regional states** (a finite-capacity residence/individuation constraint, §5.1) and **microscopic-IC selection** of which record the actual per-run universe carries — with the Schrödinger evolution left exactly unitary throughout. Turning this residence-plus-selection mechanism into a theorem (and characterizing the regional-capacity condition under which co-residence of macroscopically distinct records is excluded) is the open problem.
