...
5. **Poincaré-invariant typicality.** The measure $\mu$ over $\lambda$ (Open Problem 1) must itself be Lorentz-invariant, $\mu(g\cdot S) = \mu(S)$, so that "Born frequencies across runs" is a frame-independent notion. This couples the relativistic problem to the Born problem: a covariant $\mu$ is strictly harder than $\mu$, and a non-covariant $\mu$ would re-import a preferred frame through the statistics even if $\lambda$ itself is clean. This is the relativistic analogue of the difficulty Bohmian mechanics has with frame-dependence of $|\psi|^2$-equilibrium. Open, and entangled with Open Problem 1.
