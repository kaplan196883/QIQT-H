...
**Formal verification.** The deductive core of this framework — Theorems 3, 6, 7, Lemma 1, Donald's identity, the no-signaling chain, the Bell/CHSH inequality (with rigorous singlet construction reaching $2\sqrt{2}$), seven structural audits (independence of (H2) from (H1) plus its sharp reference-weight reformulation; unitary decoherence does not imply concentration; the modular-local entropy bound is distinct from the CPW renormalized entropy; branch-summed cost is not a holographic consequence; literal-finite (FQ) admissibility gives trivial dynamics; compression-locality leakage and the local-projection constraint on (FQ); $\mu$-selection is load-bearing for Born statistics; support preservation $\neq$ Born equivariance), and three sub-theorems for the Canonical IC Measure Principle (typicality Mackey-Gleason; operational data insufficient; FQ-equivariance uniqueness) — is machine-verified in Lean 4 with Mathlib. A subsequent **A1+A2+A4+A6 strengthening pass** further (i) discharges the locality sub-axiom of the Canonical IC Measure Principle as a theorem about equivariant measures (`MarginalLocality.lean`), (ii) upgrades Goldstein-Struyve sub-lemma 1c from a literally-false bare-coefficient axiom to a proved theorem under explicit permutation symmetry (`GoldsteinStruyveStep1.step1c_collapse_of_perm_symmetric`) and provides the foundational matrix-conjugation identity $P_\sigma \cdot E_{ij} \cdot P_\sigma^* = E_{\sigma(i),\sigma(j)}$ as a concrete proof, (iii) proves Chebyshev-style frequency concentration on finite Bernoulli trials (`BornConcentration.lean`), and (iv) packages the three remaining Born sub-axioms with their independence countermodels in a unified minimality table (`BornMinimalityTable.lean`). The `FQEquivarianceUniqueness` module was consolidated to route through the concrete `GoldsteinStruyveFinDim` proof, eliminating 12 redundant axioms; the headline theorem `canonical_ic_measure_principle` now depends on exactly two project-specific axioms (Schur classification + tensor multiplicativity), both load-bearing. The complete axiom audit is in `lean/mathlib/QIQTH/AxiomAudit.lean`. See `lean/mathlib/QIQTH/` in the project repository.
...
Machine-verified audits flag the difficulty: `NoBornFromNothing.lean` (any distribution realizable; the structural axioms do not pick out Born), `EquivarianceGap.lean` (support preservation $\neq$ Born equivariance), `OperationalNoGo.lean` (operational frequency data alone insufficient). The independence package `BornMinimalityTable.lean` re-exports these three countermodels and shows that the locality sub-axiom is *not* independent — it reduces to a theorem about equivariant measures (`MarginalLocality.alice_marginal_unchanged_by_bob_dynamics`). Hence the Canonical IC Measure Principle's irreducible content is exactly three named sub-axioms (canonical measure, equivariance, operational sufficiency), each provably necessary by concrete finite countermodel.
...
- **Projection locality.** Need precise assumptions on the local algebraic structure: tensor product factorisation (or its absence in Type III), commuting local algebras (microcausality), split property, finite corners. The `CompressionLocality.lean` audit isolates the compression-locality leakage identity and identifies when the FQ projection preserves microcausality at the restricted level. *Post A1 strengthening* (`MarginalLocality.alice_marginal_unchanged_by_bob_dynamics`): given measure equivariance under Bob's local dynamics, the Alice-marginal locality follows as a theorem, not as an independent postulate. Locality is therefore no longer counted among the load-bearing sub-axioms of the Canonical IC Measure Principle.
...
- **Frequency concentration (PROVED, single-trial).** The Lean module `BornConcentration.lean` proves Chebyshev's tail bound on finite probability spaces ($\Pr(|X-\mu| \ge \varepsilon) \le \mathrm{Var}/\varepsilon^2$), computes the Bernoulli variance ($p(1-p)$), and combines them to give the single-trial concentration inequality. By the standard product-measure scaling, the $N$-trial frequency satisfies $\Pr(|\mathrm{freq}_N - p| \ge \varepsilon) \le p(1-p)/(N\varepsilon^2)$, converging to $0$ at rate $1/N$. With this in place, the framework's typicality claim can honestly say "Born *frequencies*" — not merely "Born *means*". The $N$-trial product-measure scaling is standard probability theory left to the LLN interface axiom in `BornTypicality.lean`.
...
The independence package `BornMinimalityTable.lean` makes this explicit. After the A1 strengthening pass, the Canonical IC Measure Principle has exactly three irreducible sub-axioms, each with a concrete finite countermodel witnessing its necessity:
...
| P4 | Locality of Alice's marginal under Bob's local dynamics | **REDUCIBLE** — proved as theorem from P2 + algebraic locality (`MarginalLocality.alice_marginal_unchanged_by_bob_dynamics`) | (no independent axiom) |

This converts the standing concern "you still assume X" into a sharper audit conclusion: X is *necessary*. The minimal premise set is exactly $\{P1, P2, P3\}$ (down from four, after locality is discharged); each premise has a finite-dimensional countermodel; and Born follows uniquely when all three hold. This is the strongest honest statement available without resolving Sub-theorems A and C as theorems-in-Mathlib.
...
| 11.4.7 | Reducibility of locality (P4) to equivariance + algebraic locality | **PROVED** in `MarginalLocality.lean` (A1 strengthening) |

**Bottom line.** The Canonical IC Measure Principle is **not** a monolithic mystery. The hard parts are: the $II_\infty$ canonicality obstruction (11.4.3); the Type III correspondence (11.4.3a); the equivariance theorem for FQ-restricted dynamics (11.4.5); and the typicality justification (11.4.6). The mathematical core (11.4.1, 11.4.2, 11.4.4a) is standard given operator-algebra infrastructure; the empirical calibration (11.4.4b) is irreducible. **After the A1+A2+A4+A6 strengthening pass:** locality is no longer counted as an independent sub-axiom (theorem, not postulate); the irreducible sub-axiom set has exactly three named members (P1/P2/P3), each provably necessary by concrete finite countermodel; and Born frequency concentration is available as a finite-trial Chebyshev bound, with the $N$-trial scaling deferred only to the standard product-measure LLN.
...
**Strategic note.** Of the central open problems, the audit work suggests two bottleneck chains:
  ...
  • **Foundations bottleneck:** Open Problem 1 (canonical $\mu$-selection / equivariance). Now decomposed into three sub-theorems A, B, C (see above), with the A1+A2+A4+A6 strengthening pass further narrowing the load-bearing content. Two of the three sub-theorems are direct applications of existing operator-algebra machinery (Mackey-Gleason + noncommutative Radon-Nikodym); the third requires adapting the Goldstein-Struyve 2007 uniqueness theorem from Bohmian dynamics to QIQT-H's Type II / (FQ) setting. Concrete progress: Goldstein-Struyve Step 1 sub-lemma 1c is **PROVED** under explicit permutation symmetry (`step1c_collapse_of_perm_symmetric`) along with the foundational matrix-conjugation identity (`permutation_conj_matrixUnit`); locality is **PROVED** as a theorem reducing to equivariance + algebraic locality (`MarginalLocality`); Chebyshev frequency concentration is **PROVED** at the single-trial level (`BornConcentration`); and the duplicate axiomatization of Goldstein-Struyve sub-steps in `FQEquivarianceUniqueness` is **CONSOLIDATED** to route through the concrete `GoldsteinStruyveFinDim` proof (`canonical_ic_measure_principle` now depends on exactly two project-specific axioms, down from nine). Progress on the remaining load-bearing axioms transforms the Born section from conditional to substantive, and is the most pressing problem for foundations-of-QM defensibility.
...
| GS Step 1 (Schur classification) | `GoldsteinStruyveStep1.step1_via_sub_lemmas` | P | Decomposed into 5 sub-axioms; **sub-lemma 1c upgraded to PROVED theorem** (`step1c_collapse_of_perm_symmetric`) under explicit permutation symmetry; foundational matrix-conjugation lemma `permutation_conj_matrixUnit` also PROVED; sub-lemmas 1a/1b/1d/1e remain interface axioms |
| GS Step 1c (coefficient unification) | `GoldsteinStruyveStep1.step1c_collapse_of_perm_symmetric` | U | **NEW (A2):** proved via `Equiv.swap` transitivity on ordered pairs under explicit permutation-symmetry hypothesis; the prior bare-coefficient axiom was literally false |
...
| Marginal locality from equivariance | `MarginalLocality.marginal_invariant_of_local_dynamics` | U | **NEW (A1):** reindexing argument under $r \circ T = r$ |
| Alice marginal unchanged by Bob's local dynamics | `MarginalLocality.alice_marginal_unchanged_by_bob_dynamics` | U | **NEW (A1):** discharges locality from the Canonical IC sub-axiom list |
...
| Born single-trial frequency concentration | `BornConcentration.born_chebyshev_single_trial` | U | **NEW (A4):** upgrades $\S 11.4.6$ "Born means" $\to$ "Born frequencies" |
| Minimality witnesses for P1, P2, P3 | `BornMinimalityTable.{P1, P2, P3}_…_necessary` | N + U | **NEW (A6):** unified independence package showing each Born sub-axiom is provably necessary by concrete finite countermodel |
| Locality (P4) is reducible | `BornMinimalityTable.P4_locality_reducible_to_equivariance` | U | **NEW (A6):** locality discharged as theorem; not counted as independent sub-axiom |
