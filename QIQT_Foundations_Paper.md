...
*Lean status (`GleasonSelector.lean`) — axiom-free.* The architecture is formalized in the finite-dimensional record model. A first version named the Busch / POVM-Gleason step as an interface axiom; a soundness review (GPT-5.5-pro) showed that axiom was **false as stated** — it omitted *positivity*, and the explicit weight $w(E) = E_{00} + E_{01}$ on $\mathbb{C}^2$ satisfies normalization, additivity, homogeneity, and ray-certainty yet is not the Born functional. The axiom was therefore **retired**, and the module now contains **no project axiom**; everything is proved (standard axioms only, verified by `AxiomAudit.lean`):
...
- `proj_sandwich` — proved: $P_\psi\, E\, P_\psi = \langle\psi|E|\psi\rangle \cdot P_\psi$ on the ray projector (the algebraic heart of "ray-certainty $\Rightarrow$ Born");
- `born_is_forced` — proved (no axiom): a linear, **ray-supported** weight ($w(E) = w(P_\psi E P_\psi)$ — the explicit positivity/certainty hypothesis the counterexample violates) that is ray-certain *is* the Born functional;
...
The genuine Gleason content is thereby made an *overt hypothesis* (ray-support / positivity) rather than hidden in a false axiom. What remains future work — and is the honest residual — is the **Busch extension** (effect-additivity + positivity $\Rightarrow$ a positive linear functional, which would discharge the ray-support hypothesis from first principles) and the continuum Type II / history-net version.

*Honest verdict, and the position adopted.* A derivation of $\mu$ from unitarity plus the bare existence of $\lambda$ alone is impossible — **one** bridge principle is irreducibly required (here: non-contextuality + state-certainty), exactly the same status that quantum equilibrium has in Bohmian mechanics, and the same wall the Everettian probability problem hits. We therefore do **not** claim "probability from nothing." The adopted position is: *QIQT-H posits a selector typicality functional constrained by finite-record non-contextuality, covariance, tensoriality, and state-certainty; a Gleason-type theorem then shows the only canonical $\mu$ is the decoherence-functional measure $\mu_\Phi(\mathrm{Cyl}\,\alpha) = D_\Phi(\alpha,\alpha)$; the one remaining postulate is that the actual $\lambda$ is $\mu$-typical* — the relativistic analogue of quantum equilibrium. Strict Wood–Spekkens "no fine-tuning" in the causal-faithfulness sense is unachievable for any ontic-Parameter-Independence-violating model (a known no-go); what *is* achieved is the defensible weaker form — one canonical $\mu$ enforces operational no-signaling for every spacelike instrument pair, with no per-experiment tuning (`no_signaling_marginal`).
...
*Formalization note (Lean) — now partially carried out.* The linchpin's discrete skeleton is more amenable to machine verification than its analytic core, and the project's existing strategy — prove the order-theoretic / combinatorial scaffold, name the functional-analytic inputs as interface axioms (cf. `AxiomAudit.lean`) — applies directly. Three Lean modules now realize this, all compiling and axiom-audited (full build $\approx$ 2985 jobs; standard axioms only on every headline theorem):

- **`LorentzSelection.lean`** — the poset/sheaf layer: the causal-diamond poset $\mathsf{Diam}$, the record-sector presheaf $X_\Phi$ with functorial restriction, the global section $\lambda \in \Gamma(X_\Phi)$, `bulk_overlap_agreement` (Correction A; *no axioms*), and the **evaluation-gives-covariance theorem** $A_{gD}[U_g\Phi,g\lambda] = g\cdot A_D[\Phi,\lambda]$ — proved from equivariant naturality, depending on standard axioms only and on *none* of the deferred AQFT axioms. The Poincaré action is an `OrderIso` of diamonds; this makes the pushed-forward section's consistency a *proved theorem* (`actSection_consistent`) rather than an axiom — the earlier transport-bookkeeping axiom was thereby eliminated.
...
What remains deferred to named interface axioms (in `LorentzSelection.lean`): the existence of the finite record algebras with the holographic bound (linchpin hypotheses 1–3), the boundary-reconstruction isomorphism (2), the Born/decoherence-functional measure and its $\sigma$-additive extension (6), and the no-signaling marginal lemma (9) — the Type III$_1$ / Tomita–Takesaki / Haagerup-$L^p$ analysis that current Mathlib does not reach, and that the working physics literature also cites rather than reproves. The honest status: *equivariant naturality + Kolmogorov consistency $\Rightarrow$ a covariant single-outcome selection map* is machine-checked, the finite Type-I modular flow and the free-field finite-mode record structure (count, decoherence factorization, covariant boost action) are machine-checked, and the continuum AQFT existence inputs remain explicitly enumerated axioms — a finite skeleton proved, the continuum analysis named.
