...
*Formalization note (Lean).* The linchpin's discrete skeleton is more amenable to machine verification than its analytic core, and the project's existing strategy — prove the order-theoretic / combinatorial scaffold, name the functional-analytic inputs as interface axioms (cf. the Born pipeline's `AxiomAudit.lean`) — applies directly. Tractable now: the **poset / sheaf layer** (the causal-diamond poset $\mathsf{Diam}$, the presheaf $X_\Phi$, restriction maps, the global-section limit $\Gamma(X_\Phi)$, and the *Kolmogorov consistency* condition (7) as a statement about a finitely-additive set function on a projective system) can be stated and the evaluation-gives-covariance step (the one-line $A_{gD}[U_g\Phi,g\lambda] = g\cdot A_D[\Phi,\lambda]$) *proved* in Lean from equivariant-naturality hypotheses — exactly as `MarginalLocality.lean` already proves the finite marginal-locality step of the Born pipeline. Deferred to named interface axioms: the existence of the finite record algebras with the holographic bound (hypotheses 1–3), the boundary-reconstruction isomorphism (2), the Born/decoherence-functional measure and its $\sigma$-additive extension (6), and the no-signaling marginal lemma (9) — all of which sit on Type III$_1$ / Tomita–Takesaki / Haagerup-$L^p$ analysis that current Mathlib does not reach. The honest deliverable is therefore the same shape as the existing Lean corpus: a machine-checked proof that *equivariant naturality + Kolmogorov consistency ⟹ a covariant single-outcome selection map*, conditional on an explicitly enumerated set of AQFT interface axioms. That is a genuine, scoped, and immediately-startable formalization target.
