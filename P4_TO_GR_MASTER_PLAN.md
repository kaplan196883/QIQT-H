...
- [x] **M2 ✅ COMPLETE** — `K` operator + `Δ^{it}=e^{−itK}` (JLMS Stage 1). `K=∫f dE` operator + operator-level
  ...
  **The modulation operator / `e^{isX}` unitary group ✅** (`QIQTH/Spectral/ModulationFlow.lean`, axiom-free,
  budget 0): `modulationLp s = M_{e^{isx}}` (`(e^{isX}f)(x)=e^{isx}f(x)`), a one-parameter **unitary** group —
  `norm_modulationLp` (`‖e^{isX}f‖=‖f‖`, unitary since `|e^{isx}|=1`, via `norm_mulOp_of_norm_one`),
  `coeFn_modulationLp` (pointwise action), `modulationLp_add` (group law `e^{isX}∘e^{is'X}=e^{i(s+s')X}` via
  `mulOp_mul` + `Complex.exp_add`). Generated by the **position operator `X`** — the Fourier-dual of the
  translation group `e^{itP}`. **The canonical X–P pair is now both realized as one-parameter unitary groups.**
  Recorded next target: the **Weyl CCR** `e^{itP}e^{isX}=e^{ist}e^{isX}e^{itP}` (the integrated Heisenberg relation).
  **The Weyl canonical commutation relation ✅** (`ModulationFlow.lean`, axiom-free, budget 0): `weyl_relation` —
  `τ_t (e^{isX} f) = e^{ist} • (e^{isX} (τ_t f))`, i.e. `e^{itP} e^{isX} = e^{ist} · e^{isX} e^{itP}`. The translation
  group `e^{itP}` and the modulation group `e^{isX}` fail to commute by exactly the phase `e^{ist}` — **the
  integrated (Weyl) form of `[X,P]=i`**. Proof: both sides act a.e. as `e^{is(x+t)}f(x+t)`, the phase peeled via
  ...
  **The position Born expectation value ✅** (`PositionPVM.lean`, axiom-free, budget 0): `positionPVM_diagInt` —
  `diagInt f x = ∫ f(a)·‖x a‖² dμ`: the expectation of any bounded function `f` of the position observable `X` in
  the (unnormalized) state `x` is `∫ f(a)|x(a)|² da` — the **Born expectation rule for position**, read off the
  spectral measure (`diagInt = ∫ f d(scalarMeasure x)` against the `|x|²` density, via
