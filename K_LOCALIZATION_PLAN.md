...
| **2b-iii** Schwartz ⟹ memLp (the boundedness PROOF) | `Krep_memLp` for Schwartz `f` | ~weeks — confirmed hard: convention-match my `minkowskiFourier` to Mathlib `𝓕`/`fourierIntegral` (2π + Minkowski pairing) + 2D-Schwartz-on-hyperbola decay → L². Currently the named domain obligation (the `memLp` field). |
