...
| **2b-iii(c)** The decay bound (last mile) | `‖Krep m f θ‖ ≤ C/cosh θ` for compactly-supported-smooth / Schwartz `f` | the ONLY remaining boundedness obligation. Route: one integration by parts in `x₀` gives `(m cosh θ)·minkowskiFourier f(p_m θ) = ±i·minkowskiFourier(∂_{x₀}f)(p_m θ)`, bounded by `‖∂_{x₀}f‖₁`. Needs Fubini-on-`Fin 2→ℝ` + 1D Bochner IBP (boundary vanishes for compact support). The sharp Fourier-decay content; everything else is done. |
