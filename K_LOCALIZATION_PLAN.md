...
| **2b-ii** `K` bounded + symplectic identity | `LocalTest`, `Krep_memLp` (Schwartz decay → L²), `K`/`K_boost` (L² lift, match boostUnitary sign), **`2·Im⟪Kf,Kg⟫ = Pauli–Jordan bilinear`** | ~weeks (Schwartz-Fourier decay analysis + convention-matching to Mathlib `fourierIntegral`) |
