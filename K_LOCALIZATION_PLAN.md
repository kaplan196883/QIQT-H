...
| **2b-iii(c)** ★ Boundedness CLOSED (Gaussian) | `minkowskiFourier_gaussian`, `Krep_gaussian_eq`, **`gaussian_Krep_memLp`**, **`gaussianLocalTest`** | **DONE** 2026-06-09 (commit e7696d6, axiom-free, no sorry) — a GENUINELY NON-DEGENERATE `LocalTest`: the Gaussian's localization is provably in `L²(ℝ)`. Explicit Fourier (Fubini + complex Gaussian Fourier) ⇒ real-cast amplitude `2^{−½}π exp(−m²cosh2θ/4)` ⇒ `‖Krep‖²=(π²/2)exp(−(m²/2)cosh2θ)` integrable. |
| **2b-iv** ★★ Boundedness on ALL Schwartz (GPT #1) | `minkBilin`, **`minkowskiFourier_eq_fourierIntegral`** (bridge), `abs_sinh_le_cosh`, **`schwartz_Krep_memLp`**, **`schwartzLocalTest`** | **DONE** 2026-06-09 (file `QIQTH/Fock/SchwartzDecay.lean`, axiom-free, no sorry) — the honest local test class. For EVERY Schwartz `f`: `‖Krep θ‖ ≤ C_f/cosh θ`, `C_f=4π(‖f‖₁+‖∂f‖₁)/(√2|m|)`, hence `Krep f ∈ L²`. Route: express the bespoke no-2π transform as `VectorFourier.fourierIntegral` for `L v w=(v₀w₀−v₁w₁)/(2π)`, apply Mathlib `pow_mul_norm_iteratedFDeriv_fourierIntegral_le` (does the IBP), test vector `v=(p₀,−p₁)` ⇒ `L v p=(p₀²+p₁²)/(2π)`, shell `p₀²+p₁²≥m²cosh²θ`. Residual (not a gap): bundle as a single `𝒮→L²` CLM operator norm. |
...
| **5b** ★★ Oscillatory keystone + pointwise Δ_m vanishing | **`integral_sinh_div_cosh_sq`**, **`abs_integral_sin_sinh_le`** (the IBP bound `|∫_a^b sin(c·sinh u)du|≤3/(|c|cosh a)`), `integral_sin_sinh_symm_zero`, `abs_integral_shifted_le`, `tendsto_inv_cosh_atTop`, **`pauliJordan_spacelike_tendsto_zero`** | **DONE** 2026-06-09 (`PauliJordan.lean`, axiom-free, no sorry) — the genuine SPACELIKE VANISHING of Δ_m, pointwise: for spacelike z, `lim_R ∫_{−R}^R sin(η(p_mθ,z))dθ = 0`. Route: reparam (odd kernel) ⇒ symmetric part exactly 0 + the IBP `1/cosh` tail bound (the keystone GPT flagged) squeezed to 0. |
| **5c-i** ★ Bilinear assembly INGREDIENTS | **`abs_integral_sin_sinh_le_uniform`** (uniform dominating bound 6/\|c\|), **`reflect_integral_sin_sinh`**, **`exists_pos_lower_bound_slSq`** (r₀>0 on compact spacelike sets), **`symm_intervalIntegral_tendsto_integral`** (truncation limit) | **DONE** 2026-06-09 (`PauliJordan.lean`, axiom-free, no sorry) — ALL four reusable ingredients for the DCT assembly. |
| **5c-ii(a)** ★ Glue part (a) DONE | **`Krep_prod_im`** (fixed-θ double-integral rep), `im_exp_mul_of_real` | **DONE** 2026-06-09 (commit d596f5c) — `Im(conj(K f θ)·K g θ) = ½∫∫ sin(η(p_mθ,x−y))·(f x·g y).re` for real continuous compact-support tests. Via conj-real + integral_prod_mul + integral_comp_comm (Complex.imCLM). |
| **5c-ii(b)** ★ Per-R kernel bound DONE | **`abs_pauliJordan_trunc_le`** (+ reparam c²=m²slSq) | **DONE** 2026-06-09 (commit 8388516) — `\|∫_{−R}^R sin(η(p_mθ,z))dθ\| ≤ 6/(\|m\|√slSq)` for spacelike z, the per-point DCT dominating bound. |
| **5c-ii(d)** ★★ DCT DONE | **`Kform_im_trunc_tendsto_zero`** | **DONE** 2026-06-09 (commit 7750dd1) — `∫∫ (fg).re·(∫_{−R}^R sin(η))→0` via dominated convergence. |
| **5c-ii(c)** ★★ Fubini DONE | **`Kform_im_trunc_eq`** | **DONE** 2026-06-09 (commit c3ff0b8) — finite-R interval×2D Fubini, `∫_{−R}^R Im(conj(Kf)·Kg)=½∫∫(fg).re·∫_{−R}^R sin(η)`. |
| **5c-ii(final)** ★★★ MICROCAUSALITY DONE | **`Kform_im_eq_zero_of_spacelike`** | **DONE** 2026-06-09 (commit c3ff0b8, axiom-free, no sorry) — `Im⟨Kf,Kg⟩=0` for spacelike-separated compact supports (carries hKint convergence). **THE WALL IS DOWN** — the whole Pauli–Jordan microcausality is machine-checked. |
| **6a** ★ Interface bridge — BOTH physics inputs DONE | **`Kform_eq_inner`** (⟪Kf,Kg⟫_L²=Kform), **`K_im_inner_eq_zero_of_spacelike`** (= pauli_jordan field), **`boostUnitary`**/**`boostLocalTest`**/**`K_boost_equivariant`** (= K_equivariant field) | **DONE** 2026-06-09/10 (commits cb7e555, 59bbb51, axiom-free) — the two physics properties the interface requires (microcausality + Poincaré equivariance) discharged for the concrete K. |
| **6b** ★★ Orbit/spacelike tension RESOLVED | the interface's (boostT + relabel + pairwise-spacelike) design is over-rigid (boost orbit is TIMELIKE). FIX: boost-covariance of the MEASURE = isometry-invariance (depends only on the Gram matrix), NO closure needed. **`localized_typicality_boost_invariant`** (`LocalizedCovariance.lean`): μ∞ for pairwise-spacelike `{K(region i)}` = μ∞ for boosted `{U₁(a)·K(region i)}={K(boost_a·region i)}`; **`localized_hiso_of_spacelike`** (spacelike family ⇒ microcausal modes); **`boosted_localized_modes_eq`** (=K_boost_equivariant). | **DONE** 2026-06-10 (commit 111bb3d, axiom-free) — boost-covariance of the localized μ∞, tension gone. Region family only needs pairwise spacelike (freely satisfiable). |
| **6c** ★★★ Non-vacuity witness — SEALED | **`localized_microcausality_nonvacuous`**, **`bumps_spacelike`**, `bumpC`/`bumpLocalTest`/`bumpC_hKint` (`LocalizedWitness.lean`) | **DONE** 2026-06-10 (commit 1811492, axiom-free) — a CONCRETE 2-region pairwise-spacelike family (smooth compact-support bumps at (0,±5), built as a product of 1D ContDiffBumps), all analytic hypotheses discharged (Krep∈L² via toSchwartzMap+schwartz_Krep_memLp; hKint via Cauchy–Schwarz; spacelike (Δx)²≥36>16≥(Δt)²). Im⟪K Lf,K Lg⟫=0 NON-vacuously. **THE PRIZE IS SEALED.** |
