...
| **5c** Bilinear assembly (remaining) | from pointwise Δ_m=0 to `Im⟪Kf,Kg⟫=0` for spacelike-separated compact supports: finite-R Fubini (bounded θ, compact support) + DCT via `r≥r₀>0` on compact spacelike sets | the final remaining step; mechanical-but-substantial (Fubini + DCT plumbing) |
