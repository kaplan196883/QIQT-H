...
| **6b** Full instantiation — BLOCKED (geometric, not wiring) | needs a region family BOTH pairwise spacelike (distinct_spacelike) AND boost-relabel-closed (region_equivariant). **A boost orbit FAILS**: distinct orbit points are TIMELIKE (Δη²=2r²(1−cosh Δθ)<0). No single-boost-closed family is pairwise spacelike ⇒ the interface's (one boostT + one relabel + pairwise spacelike) design is too rigid for a non-vacuous instance. | a genuine research/design question — reformulate boost-covariance (measure invariance under the full boost action / modular structure), or generalize the interface. NOT mechanical. |
