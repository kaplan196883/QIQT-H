...
| **6** Interface wiring (remaining) | (1) `Kform m f g = ⟪K f, K g⟫_{L²}` (integral form = Lp inner product); (2) discharge `hKint` from smooth compact support (Krep∈L² via SchwartzMap + Cauchy–Schwarz); (3) instantiate `SpacetimeLocalization` with the concrete K + the proven `pauli_jordan` ⇒ the literal covariant μ∞. | mechanical wiring; no remaining analytic difficulty. |
