...
| **5c-ii** Bilinear GLUE (remaining) | one large interconnected theorem: (a) `Im(Kform m f g) = ½∫_θ∫∫ f(x)g(y) sin(η(p_mθ,x−y))` (Im∘∫, Krep expansion, product-of-integrals=double integral); (c) finite-R interval×2D Fubini; (d) `tendsto_integral_filter_of_dominated_convergence` along atTop using 5c-i; ⇒ `Im⟪Kf,Kg⟫=0` for spacelike-separated compact supports | the final glue; dedicated multi-session measure-theory effort (mixed Fubini + filter-DCT + integrability infra). NOT decomposable into quick lemmas. |
