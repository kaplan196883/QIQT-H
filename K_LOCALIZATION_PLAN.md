...
| **2b-iii(b)** Boundedness, L² bound | `Krep_memLp` (the eLpNorm < ∞ part) for Schwartz `f` | ~weeks — confirmed hard: J-flip → match `minkowskiFourier` to Mathlib `𝓕`/`fourierIntegral` (2π + Minkowski pairing) → `SchwartzMap.fourierTransform` is Schwartz → `SchwartzMap.decay` → mass-shell `‖p_m θ‖≥m cosh θ` → integrability of `(1+m cosh θ)^{−2N}`. The named domain obligation (`LocalTest.memLp` field); NOT faked. |
