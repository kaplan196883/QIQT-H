...
| **6c** Non-vacuity witness (optional) | exhibit a concrete ≥2-region pairwise-spacelike family (smooth compact-support bumps at spatially-separated locations) + discharge hKint from smoothness (Krep∈L² via SchwartzMap + Cauchy–Schwarz) ⇒ a fully non-conditional localized covariant μ∞. | analytic filling-in (bump construction + L² + Cauchy–Schwarz); the conceptual prize is already closed. |
