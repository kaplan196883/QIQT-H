...
| **5c-ii(c,d)** GLUE remaining (DCT + Fubini) | DCT lemma `Tendsto (fun R => ∫_{VV} (fg).re·D_R(z.1−z.2)) atTop (𝓝 0)` via `tendsto_integral_filter_of_dominated_convergence` (deps all confirmed: parametric-integral continuity, exists_pos_lower_bound_slSq, abs_pauliJordan_trunc_le, pauliJordan_spacelike_tendsto_zero); + the finite-R interval×2D Fubini (`integral_integral_swap`) tying it to `∫_{−R}^R (conj(Kf)·Kg).im`; + the truncation (`symm_intervalIntegral_tendsto_integral`) giving `Im(Kform)=lim` (needs θ-integrand L¹, i.e. Krep∈L² for smooth compact support). ⇒ `Kform_im_eq_zero_of_spacelike`. | the final wiring; one interconnected DCT+Fubini theorem (~150 lines, all Mathlib deps confirmed available). |
