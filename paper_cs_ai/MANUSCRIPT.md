...
- **Headline result: the Einstein field equations, machine-checked and axiom-free, from a
  finite-information bound.** Running the loop on a researcher's own framework (QIQT-H, not a
  benchmark and not a published lemma), we obtained a single end-to-end Lean theorem deriving the
  Einstein field equations $a\,T_{\mu\nu}=G_{\mu\nu}+\Lambda g_{\mu\nu}$ from the framework's
  Bekenstein-type capacity bound: a Jacobson-style equation of state in which the thermodynamic
  (area-law) input is *derived* from the framework rather than assumed, with only
  Bisognano-Wichmann wedge-modular flux and Raychaudhuri focusing cited as explicit hypotheses and
  all the differential geometry machine-checked. The same axiom-free development also carries a
...
result the loop produced (a machine-checked, axiom-free derivation of the Einstein field equations),
...
with $G$ the genuine Einstein tensor built from the metric. It is axiom-free and `sorry`-free,
...
All of the differential geometry is machine-checked and axiom-free: the second Bianchi identity, the
contracted-Bianchi conservation law $\nabla^\mu G_{\mu\nu}=0$ (divergence-freeness of the Einstein
tensor), metric compatibility, the trace identities, the null-cone-to-tensor step for a general
Lorentzian metric, and the constancy of $\Lambda$. Table 1 sorts the chain into what
QIQT-H derives, what is cited, and what is machine-checked geometry.

**Table 1. The QIQT-H + Bekenstein ⇒ Einstein-field-equations chain, by status.**
...
| Null-cone→tensor, 2nd/contracted Bianchi, $\nabla^\mu G_{\mu\nu}=0$, constant $\Lambda$ | Machine-checked, axiom-free |
...
axiom-free Lean derivation of the Einstein field equations $a\,T_{\mu\nu}=G_{\mu\nu}+\Lambda g_{\mu\nu}$
from the framework's finite-information (Bekenstein-type) capacity bound: a Jacobson-style equation of
state in which the thermodynamic input is itself derived, with only Bisognano-Wichmann wedge-modular
flux and Raychaudhuri focusing cited as explicit hypotheses and all the differential geometry
machine-checked. The same axiom-free development carries a no-collapse measurement core; in total it
...
demonstration on a hard physics target, held throughout under an explicit honesty boundary: the
formalization certifies that the derivation is correct and free of hidden axioms, with its physics
inputs labelled; it does not, and cannot, establish those cited inputs or the framework's physical
