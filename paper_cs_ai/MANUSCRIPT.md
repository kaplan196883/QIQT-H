...
false axiom and a vacuous one that a green build and a zero `sorry` count do not — not a measured
catch rate. We did not log every reviewer critique over the project, so we report no denominator: no
true-positive/false-positive rate, no count of faults the reviewer missed, and no controlled
comparison against an expert human reviewer or a richer static check (all of which we flag as future
work in §5.3). What the two cases do establish is that the failure mode is real and that the
instrument can catch it; quantifying how often it does is a separate empirical question this single
case study does not answer. With that caveat, the loop did not merely yield a green build; it yielded
an audited, shrinking-to-zero conditional base with concrete, checkable saves.
...
The fifth limit is evidential: we report no controlled comparison. We do not show that the two-model
loop outperforms a single agent, a human-only review, or ordinary CI plus `#print axioms`, and we
report no quantitative process metrics — LLM-call counts, human hours, reviewer true- and
false-positive rates, or time relative to manual formalization. The claim is therefore an audited
