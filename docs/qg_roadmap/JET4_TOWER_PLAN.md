...
## J4-789 — SYMBOLIC (sympy) INDEPENDENT VERIFICATION of the mixed-sliver campaign's target identities [AF, docs-only]
- User suggested using symbolic computation to de-risk the remaining concrete chart-surface verification work. sympy 1.14.0 available locally.
- Extracted the exact mathematical claims from `ChartJetHessianMixed.gaussComp_amp_pd_pd_mixed` (the on-gate mTerm-form target, J4-788's next sub-task) and independently re-derived them via direct symbolic differentiation (not trusting the Lean proof, re-deriving from first principles).
- **ALL VERIFIED, ZERO discrepancies found**: mixed 4-term decomposition residual EXACTLY 0 (tested 4 index combinations across dims 2-4); diagonal `gaussComp_amp_pd_pd` residual 0; underlying pure-Gaussian partials residual 0; Gaussian moment identities confirmed via `sympy.integrate` (`M1=0` odd-parity, diagonal `M2/M0=2τ` matches `gaussianMoment_diag`, off-diagonal `∫zᵢzⱼ=0` — the exact parity cancellation J4-781's proof relies on).
- Documentation `docs/qg_roadmap/SYMBOLIC_VERIFICATION_MIXED_SLIVER.md` (commit `08a16e71`), scripts in scratchpad.
- ★★★ VALUE: this is INDEPENDENT confirmation (a from-scratch symbolic re-derivation, not a trust-the-Lean-proof check) that every algebraic identity underlying the mixed-sliver campaign's target statements (J4-780→788) is mathematically sound — no sign errors, no spurious terms, no algebraic bugs anywhere in the chain. De-risks the remaining Lean formalization work (the "on-gate mTerm-form match" and subsequent concrete verification) by confirming in advance that what's being formalized is correct, before spending Lean-proof effort on it.
- a₁=R/6 remains CONDITIONAL — this is a verification/documentation artifact, not new Lean content, but a valuable complementary cross-check to the machine-checked axiom-budget audit this campaign has relied on all session.

## J4-790 — on-gate mTerm-form match FORMALIZED — sympy pre-check (J4-789) held, no new obstruction [AF]
- New file `QIQTH/MixedNormalFormOnGate.lean` (commit `ba2ec3ba`), `witnessMixed_gate_eq_mTerm` — on the open gate, the concrete `∂ⱼ∂ᵢ` mixed second field partial of the gated van-Vleck witness equals the 4-term `mTerm0+mTerm1+mTerm1+sTerm2` decomposition, in the EXACT shape `witness_sliver2_xuniform_mixed`'s `hNormalForm` needs. Std-3, budget raw 0, vacuum clean.
- Method exactly as scoped: `witnessMixed_gate_eq` (i/j-swapped) supplies the on-gate closed form; unfolding + one cross-jet symmetry (`∑Pi·Pj=∑Pj·Pi`, the ONLY real content, reflecting the mixed moment being a PRODUCT of 2 factors) closes by `ring`. NO new obstruction — the J4-789 sympy pre-check held.
- **Important scope note for future work**: this is the POINTWISE on-gate equality only. Extending to the sliver's FULL `∀ζ` `hNormalForm` needs reconciling OFF-gate behavior — off the gate the witness partial is 0, but `mTerm0` uses a nonzero Gaussian, so the amplitude fields must be the GATED ones (vanishing off `S z`) for both sides to agree everywhere. This reconciliation is part of the step-2 "chart surface" wall, not yet done.
- Remaining chain unchanged in structure: step 2 (concrete chart-surface hypothesis verification, the large wall) + step 3 (hcomp wiring, needs the analogous unbuilt diagonal kPrime link + off-gate reconciliation) + step 4 (hD1_concrete→hCConv). a₁=R/6 remains CONDITIONAL on `{hDuhamel, hDConv, hCConv}`.

## J4-791+792 — diagonal kPrime link + full off-gate-reconciled ∀ζ mTerm normal form BOTH LANDED [AF]
- **J4-791** (commit `8ee1ead1`, `QIQTH/KPrimeDiagPdBridge.lean`): `kPrime_apply_single_eq_diagPd` — the diagonal (j=i) twin of J4-788's mixed kPrime link. KEY FINDING: J4-788's mixed link carries NO i≠j hypothesis — it's fully index-generic, so the diagonal link is literally its j:=i instance, no new kPrime-level math needed. Built as a self-contained diagonal-labelled entry point for clarity. Sympy-checked (∂ᵢ∂ᵢ = mixed ∂ⱼ∂ᵢ at j=i, residual 0). Std-3, budget raw 0.
- **J4-792** (commit `2e2f4c6c`, `QIQTH/MixedNormalFormFull.lean`): `witnessMixed_hNormalForm_full` — the FULL `∀ζ` mixed hNormalForm (confirmed genuinely needed: `hNormalForm` is a literal unconditional `∀τ∀ζ` equality, NOT naturally gated as hoped). Found: `chartFieldAmp` has a radialCutoff factor but is NOT S-gated, while the witness IS hard-S-gated — the raw amplitude does NOT reconcile off-gate. FIX: feed S-gated amplitudes (`gateAmp`, a `Set.indicator (S z₀)`-gated amplitude) into the mTerm decomposition. Proof: on-gate via J4-790's match (indicator transparent); off-gate BOTH sides derived to vanish via germ-congruence (`pd_pd_mixed_eq_zero_of_eventuallyZero`, the mixed analogue of an existing diagonal germ-vanishing lemma) from an off-gate value germ `hOffNhd` — NOT assumed, derived. Sole carried residue: `hOffNhd` (witness≡0 on a neighborhood of off-gate points) — satisfiable (vacuous if S=univ), the actual chart-surface condition that radialCutoff kills the parametrix before the gate boundary. Sympy pre-check confirmed (docs/qg_roadmap/SYMBOLIC_VERIFICATION_OFFGATE_RECON.md). Std-3, budget raw 0.
- ★★★ Link 2 of J4-788's chain (on-gate + off-gate mTerm-form match) is now FULLY CLOSED for the mixed case. Link 1 (kPrime→pd∘pd) is closed for BOTH mixed (J4-788) and diagonal (J4-791) cases.
- REMAINING for the diagonal leg specifically: an analogous 3-term sTerm-form gate identity wired at kPrime's D2H level (the diagonal counterpart of J4-790, likely similarly mechanical given the pattern).
- REMAINING OVERALL: step 2's large concrete chart-surface hypothesis verification (dozens of geometric/amplitude/integrability carries, shared by both diagonal and mixed legs) + proving hOffNhd from the actual S/chart geometry + final hcomp wiring + hD1_concrete→hCConv.
- a₁=R/6 remains CONDITIONAL on `{hDuhamel, hDConv, hCConv}`.

## J4-793,794,795 — diagonal sTerm identity + hOffNhd geometry + 5 of ~10 chart-surface hypotheses discharged [AF]
- **J4-793** (commit `534deae4`, `QIQTH/MixedSliverAmpBounds.lean`): all 4 amplitude sup-bounds (hA0bdd/hA1ibdd/hA1jbdd/hA2bdd) discharged. Key find: sliver demands GLOBAL ∀τ∀ζ bounds, but concrete amplitudes are `gateAmp` (Set.indicator-gated) — 0 off-gate — so the global bound reduces to a pure ON-GATE bound. Std-3.
- **J4-794** (commit `f4dfade7`, `QIQTH/MixedSliverFdom.lean`): hFdom (two-point Gaussian domination) discharged. Corrects an initial worry about a 1/√s singularity — the banked `LeviSeriesLocalData.hFenv` is EXACTLY hFdom's shape at α=0, a one-rewrite consequence via `baseKernelW_zero_apply`. Std-3.
- **J4-795 TASK A** (commit `ebb4732d`, `QIQTH/DiagNormalFormOnGate.lean`): diagonal sTerm-form on-gate match — confirmed simpler than the mixed case as expected: `SecondDerivEnvelope.witnessFieldDeriv2_gate_eq` already supplies the raw form, no cross-jet symmetry needed (gradient terms pre-merged). Sympy-checked, residual 0.
- **J4-795 TASK B** (`QIQTH/OffGateCutoffDischarge.lean`): hOffNhd DISCHARGED to a strictly smaller, PARAMETRIX-FREE residue. Key find: `radialCutoff` is an OUTRIGHT LEADING FACTOR in the witness (→zero_mul off-gate), and `B2MeasurabilityDissolution.radialCutoff_zero_on_frontier_collar` establishes cutoff=0 pointwise on the frontier collar (`b≤r<c`). The far/open-exterior case discharged with NOTHING carried. Honest remaining residue: `hCut`, a chart-continuity germ at the gate BOUNDARY radius specifically (chart smoothness is only banked on inner balls, not at the boundary) — smaller and parametrix-independent, a real improvement.
- **J4-795 (parallel scoping)**: systematic per-hypothesis check found the campaign's working assumption ("diagonal case already discharged, just port") was WRONG for this specific wall — `AssemblyLadderR5` confirms the diagonal `witness_sliver2_xuniform` itself "demands ~30 fresh concrete carries" and was NEVER concretely instantiated anywhere. No diagonal template existed; each hypothesis traced from first principles. Result: 5 of ~10+ hypotheses discharged (the 4 amplitude bounds + hFdom); hNormalForm already done (J4-792). REMAINING GENUINELY OPEN: hco/hVdisp/hJ3i/hJ3j/hJ3Q (5 RNC chart-geometry Taylor estimates — confirmed via `AmpGeometryBundle.hjets_residual_carries` as carried residuals with NO global-base bound existing anywhere — genuinely hard analytic content, not a port); hqLip (partially portable, Lipschitz sub-part is new content); 7 integrabilities (reducible in principle via gate-compact-support but each needs a multi-lemma effort, not a one-step port).
- ★★★★★ HONEST UPDATED ASSESSMENT: the "dozens of concrete facts" wall is materially smaller (5+ facts down, hNormalForm done, hOffNhd narrowed) but the REMAINING core — 5 RNC chart-geometry Taylor estimates — is confirmed as genuinely hard, not-yet-built content (no supplier anywhere in the repo), distinct from this session's dominant "already exists, just find it" pattern. This is a genuine, precisely-scoped open frontier.
- a₁=R/6 remains CONDITIONAL on `{hDuhamel, hDConv, hCConv}`.

## J4-796 — hco DISCHARGED via reusable pointwise near-isometry primitive; the true remaining wall named: J4-556 chart-spec substrate gap [AF]
- New file `QIQTH/RNCNearIsometryPointwise.lean` (commit `9739ecee`), std-3, budget raw 0.
- CORRECTION to J4-795's "no supplier anywhere": local concrete `hco`/`hVdisp` ALREADY EXIST (`InverseChartDisplacement.chartW0_nearIsometry`/`chartW0_displacement`) for `V=W₀z` over a ball — what's missing is specifically the GLOBAL ∀z form + the jet gaps.
- **Sympy derivation** (docs/qg_roadmap/rnc_sympy/): from-scratch RNC Taylor expansion confirmed ZERO discrepancy in the expected exponent/constant shapes for all 5 estimates — `hVdisp` exponent 2 (`C_W~½sup‖Γ‖`), `hJ3` exponent 1 (`C_P~sup‖Γ‖`), `hJ3Q` bounded (`~sup‖Γ‖`). Also derived a clean sufficient radius condition (`n·C_W·‖z‖≤1/4`) for the near-isometry lower bound.
- **hco DISCHARGED**: `nearIsometry_lower_of_quadraticDisplacement` — a chart-AGNOSTIC reusable pointwise primitive (any `V`, `hVdisp`+radius condition ⟹ `hco`), plus `chartW0_hco_ball` routing it through the existing `chartW0_displacement` for the concrete van-Vleck chart.
- **★★★★★ THE TRUE REMAINING WALL, PRECISELY NAMED**: `hJ3i`/`hJ3j`/`hJ3Q` need a UNIFORM-IN-z second-order Taylor/Jacobian-Lipschitz bound of the inverse chart, one Fréchet order above `hVdisp`. The frozen chart existence spec (`uniformChart_exists` = ApproximatesLinearOn first-order + pointwise `ContDiffAt 2` only) does NOT expose this — identified as **the J4-556 substrate-rebuild wall** (a PRE-EXISTING, already-named gap in this repo's chart infrastructure, not new to tonight). The forward geodesic Taylor machinery EXISTS (`GeodesicTaylorCompact.geodesicField_taylor_remainder_uniform`) but is NOT plumbed into the inverse chart's spec — confirmed genuine multi-session substrate work, not a port, not this session's dominant pattern.
