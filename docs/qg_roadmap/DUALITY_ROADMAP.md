...
| **D3 — strong decoupling (THE duality theorem)** | 🟢 SKELETON COMPLETE (5 rungs, 2026-07-12) — the conjecture's every term machine-checked; the Prop awaits its cited physical inputs. D3a `7393d3af` (continuum entropy) · D3b `04c22cb2` (heat-kernel form) · D3c `a1d3a65e` (conical coefficient, cosecant-sum Mathlib-first) · D3d `0aa98ee3` (Susskind–Uglum: entropy renormalizes 1/G) · D3e/f `22bbd7b2` (saturation bridge + non-commuting-limit diagram). CITED-INPUTS DISCHARGE PROGRAM (turning the five physical inputs into finite theorems): **input #1 — the Gaussian one-loop determinant — DISCHARGED AT THE FINITE LEVEL, G1 `2e286419`** (`OneLoopDeterminant.lean`: the Frullani subtracted proper-time log-det `log det A = ∫(N e^{−t}−Tr e^{−tA})/t` — a genuine convergent Lebesgue integral, the subtraction killing the raw `∫ Tr K dt/t` UV divergence — + the diagonal Gaussian + the `log Z` assembly; finite diagonal spectrum, continuum functional det / ζ-reg / arbitrary PosDef Gaussian stay cited). **input #2 — the replica n→1 continuation — DISCHARGED AT THE FINITE LEVEL, G2 `41f35b90`** (`ReplicaContinuation.lean`: the replica free energy `w(n)=log ∑ pᵢⁿ` smooth, `w(1)=0`, `w'(1)=∑ pᵢ log pᵢ`, so `replica_entropy_hasDerivAt` = the tight headline `S = −∂ₙ log Zₙ|₁ =` the von Neumann entropy; `renyi_tendsto_shannon` = the Rényi n→1 limit via the slope characterization; finite full-support spectrum AFTER `Zₙ=Tr ρⁿ=∑ pᵢⁿ`, the integer-n-geometric→analytic identification stays cited). REMAINS (all standard-QFT, none QIQT-H-specific, none a clean tractable Lean brick): #3 the curved a₁=R/6 (Mathlib Riemannian-heat-kernel-gated, research-grade), #4 the same-regulator assumption (physical modeling stipulation), #5 the cutoff identification D_eff~1/x (modeling choice; the log-matching itself is proved in D3e/f) — discharge those and `FlatSpaceRecordGravityCorrespondence` closes. ⟨old status below⟩ 🟡 two rungs landed | D3a: the conjecture's first two terms TOUCH — finite record entropy → the exact c = 1 continuum thermal entropy π²/(3β) (Bose integral + Riemann-sum theorem from scratch, both Mathlib-gaps). D3b 04c22cb2 (heat-kernel form) + D3c a1d3a65e (CONICAL leg: the exact orbifold excess (1/12)(n−1/n), t-independent, + the c/6 replica coefficient; the cosecant sum a Mathlib-first) — the winding proper-time sum on the thermal cylinder = π/(6β) exactly, agrees with the canonical Bose free energy (one object, two descriptions), entropy = the D3a record entropy; the Euclidean-factor guard machine-checked. *Remains (the core):* the CONICAL leg (Sommerfeld/cone kernel — where the Hawking/cone layer's `cone_flat_iff` machinery may feed in), induced-G, the β → 0 saturation bridge, the limit interchange. |
