...
- M4a (analytic Jacobi formula) in flight — the abstract analytic toolkit `d/dτ log det W = tr(W⁻¹ W')` (the regularized expansion θ=tr(W⁻¹W')). Mathlib has `Continuous.matrix_det` but NOT the analytic derivative of det, so this is build-from-scratch: matrix_det_contDiff (det is C^∞, guaranteed) + hasDerivAt_matrix_det (Jacobi formula d/dτ det = tr(adj(W)·W'), the cofactor-derivative crux — checkpoint if intractable) + hasDerivAt_log_det_matrix (⟹ tr(W⁻¹W')). Self-contained (no geometry); on the M4 critical path. M2b (geometric Y=D exp_p, off-radial Jacobi eqn) + M4-full uniform van-Vleck ODE remain the deep walls.
