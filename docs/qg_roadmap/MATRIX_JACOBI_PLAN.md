...
- M4b (abstract matrix Raychaudhuri/Riccati) in flight — from the matrix Jacobi ODE Y''=−A(τ)Y (A abstract, Y invertible), Θ:=Y'Y⁻¹ satisfies the matrix Riccati Θ'=−A−Θ² (via hasFDerivAt_inv' for (Y⁻¹)'=−Y⁻¹Y'Y⁻¹), hence θ:=tr Θ satisfies θ'=−tr A−tr(Θ²) (Raychaudhuri, via Matrix.traceLinearMap). Abstract (A plays the curvature role); the geometric A=R(τ)/tr A=Ric identification is M2b.
