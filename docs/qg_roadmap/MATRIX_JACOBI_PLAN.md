...
- M3 ✅/in flight — det Y = τⁿ det W (Matrix.det_smul) + log split log det Y = n log τ + log det W (isolating the singular n·log τ from the finite log det W). ⚠ the K1/K2 bridge to det g̃ (det g̃=J²·det(g∘exp)) requires the GEOMETRIC identification Y=D exp_p (M2b) — deferred; M3 lands the ABSTRACT det/log split reachable now.
