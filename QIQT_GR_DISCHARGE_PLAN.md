...
- **Item A (`hKMS`) FULLY CLOSED**: the free-field BW KMS condition (RvD Def 3.4) is machine-checked
  ...
  the wedge subspace (the `S`-construction) — is now ALSO discharged axiom-free (2026-06-23):** `niceWedgeCyclic_pos_mass`
  (cyclic, via the Wiener–Tauberian theorem + width-scaled bump) and `niceWedgeSeparating_pos_mass` (separating /
  Pauli–Jordan, via a pure modular/KMS uniqueness argument) combine into `oneParticleBW_niceWedge_unconditional` =
  `modUnitary S t = boostUnitary(2πt)` with NO Reeh–Schlieder hypotheses. The free-field one-particle BW is a
  fully closed, axiom-free theorem — what was "research-grade, not in Mathlib" is now machine-checked.
...
**★★★★★★★ END-TO-END CAPSTONE (`177634c`, 2026-06-23, axiom-free): `qiqt_gr_explicit_kg`** — the abstract
`qiqt_gr_from_wedge_kms_complete` specialised to `T = kgStress`, so BOTH the abstract matter `T` AND the
`conserv` hypothesis are ELIMINATED from the top-level statement: `conserv` is discharged internally
(`kg_conserv_of_contDiff`) and the stress-tensor symmetry `hT_symm` is proved from metric symmetry. For a smooth
free scalar `φ` on a smooth Lorentzian metric satisfying the EOM `□φ=m²φ`, plus the genuinely physical inputs
...
5. **REMAINING — THREADING `stripKMSrvd_pair` ⟹ `StripKMSrvd boostUnitary 𝒦_W`** (abstract functional-analysis
   ...
   - **REMAINING (honest boundary — NOT the labelled KMS anymore):**
     (a) **`S`-construction = the Reeh–Schlieder FRONTIER.** `oneParticleBW_niceWedge` takes `S : StandardSubspace`
         ...
         **(8d assembly CHAIN) BUILT** (`6172950`): `fourierL2_toLp_ne_zero_of_ne_zero` (generic: `g∈L¹∩L²`, `𝓕g`
         real-analytic, `g≢0` ⟹ `⇑(𝓕_{L²}(g.toLp))≠0` a.e. — `g≢0 ⟹ ∃x 𝓕g(x)≠0` [brick-6b contrapositive] ⟹ `𝓕g≠0`
         ...
         **FACTORIZATION + θ=0 VALUE + REDUCTION BUILT** (`deecc60`, `fdec690`): `minkowskiFourier_bumpC`
         (`minkowskiFourier (bumpC cT cX) p = (∫ e^{−ip₀y}bump1 cT)·(∫ e^{ip₁y}bump1 cX)`, Fubini); `Krep_bumpC_zero`
         (`Krep m (bumpC cT cX) 0 = (1/√2)·A(m)·∫bump1 cX`, shell `(m,0)` ⟹ 2nd phase `e⁰=1`); `Krep_bumpC_ne_zero_of`
         (`Krep ≢ 0 ⟸ A(m)≠0`, since `∫bump1 cX>0` `ContDiffBump.integral_pos` + `Krep` continuous); and the capstone
