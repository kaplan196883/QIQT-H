...
trailing `f`).*
**Then (the converse half — the genuine remaining wall):** the **generator** `A x := −i d/dt U_t x|₀` (the
infinitesimal generator on its Gårding domain) and the identification `U_t = exp(it A)` with `A` the Cayley-defined
self-adjoint operator (`stoneGen`), i.e. that THIS `U` is the Stone group of its own generator — Stone's
correspondence, both directions. This is the derivative-through-strong-limit + essential-self-adjointness step
flagged from the start (Phase 3.2) as the hard analytic core; the entire unitary-group side beneath it is now
