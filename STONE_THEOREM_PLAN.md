...
Also ✅ `cayleyStoneU_comm_cayleyUnitary` — **★★★★ THE STONE GROUP IS A FUNCTION OF `V`** (`StoneExp.lean`,
axiom-free, budget 0): `U_t(V y) = V(U_t y)`. Each cutoff operator `cfc(g_{t,N}) V` commutes with `V` (it is a
continuous function of `V` — `Mathlib`'s `Commute.cfc`, fed `Commute.refl V` + the normality `Commute (star V) V` =
`(cayley_isStarNormal …).star_comm_self`); passing to the strong limit (`cayleyStoneU_tendsto` on the left,
`V`-continuity on the right, `tendsto_nhds_unique`) gives `U_t V = V U_t`. So `U_t = exp(it A)` lies in the **abelian
von Neumann algebra generated by the Cayley unitary `V`** — the precise sense in which the modular flow is generated
by its own spectral data. *Built green (2 fixes: `Commute.cfc` needs a 2nd hyp `Commute (star V) V` and an explicit
trailing `f`).*
Also ✅ `cayleyStoneU_cfc` — **★★★★ THE SPECTRAL ACTION ON THE cfc CORE** `U_t (cfc φ V z) = cfc(e_t·φ) V z`
(`StoneExp.lean`, axiom-free, budget 0): on the spectral-calculus vectors `cfc φ V z` (with `φ` and every `e_r·φ`
ContinuousOn `σ(V)` — e.g. `φ` vanishing at the excluded point `1`), the abstract strong-limit group **acts by
multiplying the symbol by `e_t(ω)=exp(it·c(ω))`**. Proof: `cfc(g_{t,N})V(cfc φ V z) = cfc(g_{t,N}·φ)V z` (`cfc_mul`)
`→ U_t(cfc φ V z)` (`cayleyStoneU_tendsto`); and `cfc(g_{t,N}·φ)V z → cfc(e_t·φ)V z` because the `L²`-defect
`∫‖(g_{t,N}−e_t)φ‖² = ∫ψ_N²|φ|² ≤ M²∫ψ_N² → 0` (`cayley_cfc_sub_norm_sq_integral` + `cayleyExpBump_sub_norm`
`‖g_{t,N}−e_t‖=ψ_N` + `cayleyCutoff_sq_integral_tendsto_zero`; `M = sup|φ|` on the compact `σ(V)` via
`IsCompact.exists_bound_of_continuousOn`); uniqueness. *Built green (1 fix: evaluate `√` at `M²·0` not `0` for the
`.comp`).* This **identifies the abstract `U_t` with the bounded-Borel functional calculus `cfc(e_t·)` on a core** —
the concrete spectral meaning of `U_t = exp(it A)`, and the gateway to the generator.
**Then (the converse half — the genuine remaining wall):** the **generator** `A x := −i d/dt U_t x|₀` and the
identification `U_t = exp(it A)`. With `cayleyStoneU_cfc` the next concrete step is to **differentiate
`t ↦ cfc(e_t·φ) V z` at `0`**: formally `d/dt e_t|₀ = i·c·e_0 = i·c`, so `d/dt U_t(cfc φ V z)|₀ = cfc(i·c·φ)V z =
i·(A acting on cfc φ V z)` — i.e. the generator is multiplication by `i·c(ω)` (the spectral value of the Cayley
