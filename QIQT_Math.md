...
3. **A non-dynamical run-index $\lambda$** — a global, atemporal actuality fact about *which* of $\Phi$'s macroscopic realizations the whole run is (fixed for the entire 4-dimensional history; *not* past-localized "initial data," to avoid any setting-correlation / Conway–Kochen reading) — selecting which single record is realized.
