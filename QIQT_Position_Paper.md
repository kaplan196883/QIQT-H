...
**Honest qualifications.** Under the modular-local refactor (technical paper §7.6), three precise statements replace earlier informal ones: (a) macroscopic definiteness is an *effective* statement — the relevant bound is on $H(\{p_k\})$, hence on $N_{\rm eff} := \exp H(\{p_k\}) \le \exp(C(R) - I_0)$, not on the raw cardinality of records, with single-record-per-run exact only at saturation $I_0 \approx C(R)$; (b) admissibility applies *causally* — a future joint diamond's capacity constrains the future joint state only, never retroactively deleting past separately-admissible branches; (c) instruments must be *branchwise* admissibility-preserving, not merely on-average. The lab-scale Born deviation is now bounded as $\delta_R \le \mathbb{E}[\chi_R]/C(R) \sim 10^{-27}$ for a 1-m, 1-kg region — region- and energy-dependent, not a universal constant. These refinements make the framework mathematically tighter without changing its empirical predictions in laboratory regimes.
