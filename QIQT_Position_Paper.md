...
**Formal verification.** The deductive core of the framework — the no-signaling chain, the Donald-bound effective-definiteness theorem, the Bell/CHSH inequality with a rigorous singlet construction reaching $2\sqrt{2}$, and two structural audits (independence of the record-cost postulate (H2) from the other structural axioms plus its sharp reference-weight reformulation; unitary decoherence alone does not imply single-record concentration) — is machine-verified in Lean 4 with Mathlib. See `lean/mathlib/QIQTH/` in the project repository.
