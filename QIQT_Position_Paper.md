...
The bound has pedigree across the quantum-gravity literature. Bekenstein's argument from black-hole thermodynamics. Hawking's derivation of $S_{\rm BH} = A/(4\ell_P^2)$. 't Hooft's dimensional-reduction arguments. Susskind's holographic principle. Bousso's covariant generalization. Banks (2025) arguing that finite entropy implies finite Hilbert-space dimension for bounded subsystems.

The values are enormous in conventional terms — $\sim 10^{70}$ nats for a one-meter region — but **finite**, and they **scale as boundary area rather than volume**. This is the *holographic* character of the bound.
...
**The framework's central new mathematical claim — the Macroscopic Definiteness Conjecture** — is then formulated on top of this CPW scaffolding. It states: *under (FQ), the physically realized state on the macroscopic record subalgebra of any region (the commutative subalgebra of decoherence-stable macroscopic observables) is a single point of its spectrum — a single macroscopic record. Multi-record states are not physically realizable, because they would require encoding multiple macroscopic configurations whose joint information content exceeds the regional holographic capacity.* This conjecture, if established, gives single-world per run as a structural consequence of (FQ) at the macroscopic observable level — *without* needing to "select" a branch from many physically real branches and *without* invoking a collapse postulate. The Everett branches of the universal wave function are mathematical artifacts of the unrestricted Hilbert-space apparatus; they don't enter the macroscopic observable content; they're not part of physics. We don't have direct physical access to the universal wave function's branch structure — only to macroscopic observable content; (FQ) constrains that content to be single-record per region per run. The technical paper (§§6.8 and 7.6) formalizes the conjecture and identifies what existing math (decoherent histories, einselection, algebraic state restriction) provides scaffolding and what new math is needed (cost-counting that macroscopic records saturate $Q_R$; additivity for multi-record states; (FQ) violation for multi-record states). We develop the formal apparatus in the companion technical paper; here we proceed informally.
